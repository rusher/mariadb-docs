...
This lets one check how close the optimizer's estimates about the query plan are to the reality. `ANALYZE` produces an overview, while the [ANALYZE FORMAT=JSON](https://mariadb.com/kb/en/analyze-formatjson/) command provides a more detailed view of the query plan and the query execution.
...
where the statement is any statement for which one can run [EXPLAIN](https://mariadb.com/kb/en/explain/).

### Command Output <a href="#command-output" id="command-output"></a>
...
### Interpreting the Output <a href="#interpreting-the-output" id="interpreting-the-output"></a>
...
* **orders.filtered=100, orders.r\_filtered=30.03**. The optimizer didn't have any way to estimate which fraction of records will be left after it checks the condition that is attached to table orders (it's orders.o\_totalprice > 200\*1000). So, it used 100%. In reality, it is 30%. 30% is typically not selective enough to warrant adding new indexes. For joins with many tables, it might be worth to collect and use [column statistics](https://mariadb.com/kb/en/engine-independent-table-statistics/) for columns in question, this may help the optimizer to pick a better query plan.

#### Meaning of NULL in r\_rows and r\_filtered <a href="#meaning-of-null-in-r_rows-and-r_filtered" id="meaning-of-null-in-r_rows-and-r_filtered"></a>
...
### ANALYZE FORMAT=JSON <a href="#analyze-formatjson" id="analyze-formatjson"></a>

[ANALYZE FORMAT=JSON](https://mariadb.com/kb/en/analyze-formatjson/) produces JSON output. It produces much more information than tabular `ANALYZE`.
...
* The [EXPLAIN in the slow query log](https://mariadb.com/kb/en/explain-in-the-slow-query-log/) feature allows MariaDB to have `ANALYZE` output of slow queries printed into the [slow query log](https://mariadb.com/kb/en/slow-query-log/) (see [MDEV-6388](https://jira.mariadb.org/browse/MDEV-6388)).
