...
Our GIS functionality is limitted compared to other DBMSes. Given that MariaDB looks to facilitate migration from MySQL, we should be on par. We have a list of functions that are missing in MariaDB compared to MySQL, as described in [function-differences-between-mariadb-10-10-and-mysql-8-0.md](../../../../release-notes/mariadb-community-server/compatibility-and-differences/function-differences-between-mariadb-and-mysql/function-differences-between-mariadb-and-mysql-unmaintained-series/function-differences-between-mariadb-10-10-and-mysql-8-0.md).
...
1. Formulating a clear plan to facilitate:

  1. Starting MariaDB with ColumnStore
  1. Upgrading MariaDB with ColumnStore on a version upgrade
  1. Creating health checks to validate the health of the ColumnStore service nodes
  1. Setting configuration variables via environment switches where appropriate
  1. Declaratively (likely docker-compose yml file) state the system's architecture.
  1. Documenting the necessary steps to deployment
  1. Producing a blog of its operation
  1. Optionally enable deployment via Kubernetes
1. Implementing the plan and creating a CI/CD pipeline for testing.


**Skills needed**: Ability to develop durable bash scripts, understanding of container runtime and ability to confirm to container best practices. Able to incrementally develop and test functionality.
**Mentors**: Daniel Black (primary - containers) / Andrew Hutchings (secondary - ColumnStore)
...
1. small
1. has no or almost no branches
1. optimized for the specific platform it is run at
...
1. demonstrates a negative and positive effects of using JIT
...
1. C++
1. at least basic compilers internals knowledge
...
1. produce a feature
1. support the feature with unit and integration tests using existing frameworks
...
1. a set of parser threads that populates the buffer
1. a set of writer threads that takes values that makes a single SQL record and puts them into the corresponding files
...
1. C++
1. production development tooling like git, Cmake
...
1. compares performance for cases:

  1. vectorized execution with vertical data
  1. vectorized execution with horizontal data
  1. scalar execution with horizontal data
...
1. C++
1. ASM knowledge to manage with low-level part of this project
...
1. Write a code which integrates C++ MariaDB Connectors and libFuzzer.
1. Prepare a valid corpus with SQL sripts suitable for Columnstore.
1. Create a fuzzing infrastructure.
