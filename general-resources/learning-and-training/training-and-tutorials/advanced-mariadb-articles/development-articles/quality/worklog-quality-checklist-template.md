...
* Does the feature change the format of the system tables in the `<code>mysql</code>` database?
...
* Does the feature change any result formatting or data types?
  ...
  * If yes, testing via the `<code>mysql</code>` client is required to verify that the new format or data type properly arrives at the client
* Does the feature change the syntax or the output of a `<code>SHOW</code>` command?

  * If yes, clients, connectors and tools that issue that `<code>SHOW</code>` command and process its output should be tested
* Does the feature change anything with respect to `<code>INFORMATION_SCHEMA</code>` and `<code>PERFORMANCE_SCHEMA</code>` ?
...
*Does the feature generate or transmit data that may be sent or processed by the slave or tools such as `<code>mysqlbinlog</code>`?*
...
* If code can only be reproduced by an RQG test or other external test,
 ...
 reproducible;
* All code should either be tested or reviewed and marked with `<code class="fixed" style="white-space:pre-wrap"><span class="cm">/* purecov inspected */</span>
</code>`
