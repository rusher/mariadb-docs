...
As a general rule, for new installations it's better to choose the [latest Generally Available (GA) version](/en/download-latest-releases/).
...
If you need to work with an existing production instance, you should of course use the same version in testing. However, deprecated versions should not be used in production, because they could be exposed to vulnerabilities that will never be fixed. See [deprecation policies](/en/deprecation-policy/) if you are not sure about the version you are using.
...
It is necessary to run [mysql_install_db.exe](/en/mysql_install_dbexe/) to install the data directory.
