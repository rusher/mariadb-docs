...
For more information: [Account Management SQL Commands](/kb/en/account-management-sql-commands/).
...
MariaDB supports [authentication plugins](/kb/en/authentication-plugins/). These plugins implement user's login and authorization before they can use MariaDB.
...
* There is no `SHOWPLAN` privilege in MariaDB. Instead, [EXPLAIN](../../../../clients-and-utilities/explain-analyzer.md) requires the `SELECT` privilege for each accessed table and the `SHOW VIEW` privilege for each accessed view.
