...
If you installed MariaDB on Linux using [RPMs](/en/rpm/), then the `mysql.server` script will be installed into the `/etc/init.d` directory with the name `mysql`. You need not install it manually.
...
See [mariadbd startup options](mariadbd-startup-options) for information on configuration options for `mariadbd`.