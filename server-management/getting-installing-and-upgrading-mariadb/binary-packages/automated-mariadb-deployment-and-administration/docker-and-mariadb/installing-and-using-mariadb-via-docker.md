...
Sometimes we want to install a specific version of MariaDB, [MariaDB ColumnStore](https://mariadb.com/kb/en/mariadb-columnstore/), or [MaxScale](https://app.gitbook.com/s/0pSbu5DcMSW4KwAkUcmX/mariadb-maxscale-25/mariadb-maxscale-25-changelog) on a certain system, but no packages are available. Or maybe, we simply want to isolate MariaDB from the rest of the system, to be sure that we won't cause any damage.
...
Now we are ready to [install MariaDB](/kb/en/getting-installing-and-upgrading-mariadb/) in the way we prefer.
