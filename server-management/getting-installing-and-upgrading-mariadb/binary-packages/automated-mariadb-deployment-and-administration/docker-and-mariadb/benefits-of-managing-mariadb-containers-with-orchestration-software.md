...
In this page we'll discuss why automating [containers](/en/docker-and-mariadb/) with software like [Ansible](/en/ansible-and-mariadb/) or [Puppet](/en/automated-mariadb-deployment-and-administration-puppet-and-mariadb/) may be desirable in some cases. To talk about this, we'll first need to discuss why containers are defined *ephemeral*, and how this applies to containerized database servers (particularly MariaDB).
...
* Manually. See [Installing and Using MariaDB via Docker](installing-and-using-mariadb-via-docker.md). This is not recommended for production, or for complex environments. However, it can easily be done for the simplest cases. If we want to make changes to our [custom images](/en/creating-a-custom-docker-image/), we'll need to modify the Dockerfiles, destroy the containers and recreate them.
