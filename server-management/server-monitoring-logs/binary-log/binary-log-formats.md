...
There are three supported formats for [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) events:

* Statement-Based Logging
* Row-Based Logging
* Mixed Logging

Regardless of the format, [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) events are always stored in a binary format, rather than in plain text. MariaDB includes the [mariadb-binlog](../../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md) utility that can be used to output [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) events in a human-readable format.
...
In [MariaDB 10.2.3](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-102-series/mariadb-1023-release-notes) and before, statement-based logging was the default. [Mixed logging](binary-log-formats.md#mixed-logging) is now the default.

When statement-based logging is enabled, statements are logged to the [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) exactly as they were executed. Temporary tables created on the primary will also be created on the replica.
...
This mode can be enabled by setting the [binlog_format](/en/replication-and-binary-log-server-system-variables/#binlog_format) system variable to `STATEMENT`.
...
* [ALTER TABLE](../../../reference/sql-statements-and-structure/sql-statements/data-definition/alter/alter-table.md) of a table using a storage engine that stores data remotely, such as the [S3 storage engine](../../../reference/storage-engines/s3-storage-engine/s3-storage-engine-status-variables.md), to another storage engine.
* One is using [SEQUENCEs](/en/sequences/) in the statement or the [CREATE TABLE](../../../reference/sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md) definition.

In certain cases, a statement may not be deterministic, and therefore not safe for [replication](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/mariadb-releases/compatibility-differences/replication-compatibility-between-mariadb-and-mysql). If MariaDB determines that an unsafe statement has been executed, then it will issue a warning. For example:
...
This mode can be enabled by setting the [binlog_format](/en/replication-and-binary-log-server-system-variables/#binlog_format) system variable to `MIXED`.

#

## Row-Based Logging

When row-based logging is enabled, DML statements are **not** logged to the [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md). Instead, each insert, update, or delete performed by the statement for each row is logged to the [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) separately. DDL statements are still logged to the [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md).
...
If one wants to be able to see the original query that was logged, one can enable [annotated rows events](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/annotate_rows_event.md), that is shown with [mariadb-binlog](../../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md), with [--binlog-annotate-row-events](/en/replication-and-binary-log-server-system-variables/#binlog_annotate_row_events). This option is on by default.

This mode can be enabled by setting the [binlog_format](/en/replication-and-binary-log-server-system-variables/#binlog_format) system variable to `ROW`.
...
[Compression of the binary log](compressing-events-to-reduce-size-of-the-binary-log.md) can be used with any of the binary log formats, but the best results come from using mixed or row-based logging. You can enable compression by using the [--log_bin_compress](/en/replication-and-binary-log-server-system-variables/#log_bin_compress) startup option.

#

# Configuring the Binary Log Format

The format for [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) events can be configured by setting the [binlog_format](/en/replication-and-binary-log-server-system-variables/#binlog_format) system variable. If you have the [SUPER](../../../reference/sql-statements-and-structure/sql-statements/account-management-sql-commands/grant.md#global-privileges) privilege, then you can change it dynamically with [SET GLOBAL](../../../server-usage/replication-cluster-multi-master/standard-replication/setting-up-replication.md#global-session). For example:

```
SET GLOBAL binlog_format='ROW';
```

You can also change it dynamically for just a specific session with [SET SESSION](../../../server-usage/replication-cluster-multi-master/standard-replication/setting-up-replication.md#global-session). For example:
...
If the mysql database is edited directly, logging is performed as expected according to the [binlog_format](/en/replication-and-binary-log-server-system-variables/#binlog_format). Statements that directly edit the mysql database include [INSERT](../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md), [UPDATE](../../../reference/sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/update.md), [DELETE](../../../reference/sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/delete.md), [REPLACE](../../../clients-and-utilities/replace-utility.md), [DO](../../getting-installing-and-upgrading-mariadb/downgrading-between-major-versions-of-mariadb.md), [LOAD DATA INFILE](../../../reference/sql-statements-and-structure/sql-statements/data-manipulation/inserting-loading-data/load-data-into-tables-or-index/load-data-infile.md), [SELECT](../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md), and [TRUNCATE TABLE](../../../reference/sql-statements-and-structure/sql-statements/table-statements/truncate-table.md).

If the `mysql` database is edited indirectly, statement logging is used regardless of [binlog_format](/en/replication-and-binary-log-server-system-variables/#binlog_format) setting. Statements editing the `mysql` database indirectly include [GRANT](../../../reference/sql-statements-and-structure/sql-statements/account-management-sql-commands/grant.md), [REVOKE](../../../reference/sql-statements-and-structure/sql-statements/account-management-sql-commands/revoke.md), [SET PASSWORD](../../../reference/sql-statements-and-structure/sql-statements/account-management-sql-commands/set-password.md), [RENAME USER](../../../reference/sql-statements-and-structure/sql-statements/account-management-sql-commands/rename-user.md), [ALTER](../../../server-usage/programming-customizing-mariadb/triggers-events/event-scheduler/alter-event.md), [DROP](../../../security/user-account-management/catalogs/drop-catalog.md) and [CREATE](../../../security/user-account-management/catalogs/create-catalog.md) (except for the situation described below).

CREATE TABLE ... SELECT can use a combination of logging formats. The [CREATE TABLE](../../../reference/sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md) portion of the statement is logged using statement-based logging, while the [SELECT](../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) portion is logged according to the value of `binlog_format`.
