---
noIndex: true
layout:
  title:
    visible: true
  description:
    visible: true
  tableOfContents:
    visible: true
  outline:
    visible: true
  pagination:
    visible: false
---

# MariaDB Connector/Python 1.0.8

{% @marketo/form formId="4279" %}

## About this Release

* MariaDB Connector/Python 1.0.8 was released on 22 Oct 2021.
* Please note: Newer releases exist within the MariaDB Connector/Python 1.0 release series.
* [What's new in this release?](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/connectors/python/mariadb-connector-python-1-0-release-notes/mariadb-connector-python-1-0-8-release-notes)
* This release is Stable (GA) maturity.
* For more information, see the [product documentation](https://app.gitbook.com/s/CjGYMsT2MVP4nd3IyW2L/mariadb-connector-python/).

## Release History

* The prior release in this series was MariaDB Connector/Python 1.0.7, released on 8 Jun 2021.
* This release (MariaDB Connector/Python 1.0.8) was released on 22 Oct 2021.
* The next release in this series is MariaDB Connector/Python 1.0.9, released on 21 Dec 2021.
* The latest release in this series is MariaDB Connector/Python 1.0.11, released on 12 Apr 2022.

## Useful Resources

* [MariaDB Server Documentation](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/)
* [MariaDB Connector Documentation](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/connectors/)
* [Contact Us](https://app.gitbook.com/s/WCInJQ9cmGjq1lsTG91E/about/contact-us/)
* [Learn about the MariaDB Community](https://app.gitbook.com/s/WCInJQ9cmGjq1lsTG91E/community/)

CC BY-SA / Gnu FDL


{% @marketo/form formId="4316" %}
