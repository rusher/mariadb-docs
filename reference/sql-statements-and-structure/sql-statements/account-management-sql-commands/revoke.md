...
`REVOKE` is also used to remove a [role](../../../../security/user-account-management/roles/roles_overview.md) from a user or another role that it's previously been assigned to. If a role has previously been set as a [default role](set-default-role.md), `REVOKE` does not remove the record of the default role from the [mysql.user](/en/mysqluser-table/) table. If the role is subsequently granted again, it will again be the user's default. Use [SET DEFAULT ROLE NONE](set-default-role.md) to explicitly remove this.

Before [MariaDB 10.1.13](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-101-series/mariadb-10113-release-notes), the `REVOKE role` statement was not permitted in [prepared statements](/en/prepared-statements/).
