...
`mysql.help_topic` is one of the four tables used by the [HELP command](../../help-command.md). It is populated when the server is installed by the `fill_help_tables.sql` script. The other help tables are [help_relation](/en/mysqlhelp_relation-table/), [help_category](/en/mysqlhelp_category-table/) and [help_keyword](/en/mysqlhelp_keyword-table/).
