...
The [Information Schema](/en/information_schema/) `ALL_PLUGINS` table contains information about [server plugins](/en/mariadb-plugins/), whether installed or not.
...
It provides a superset of the information shown by the `[SHOW PLUGINS SONAME](../../../show/show-plugins-soname.md)` statement, as well as the `[information_schema.PLUGINS](/en/information-schema-plugins-table/)` table. For specific information about storage engines (a particular type of plugin), see the [Information Schema ENGINES table](information-schema-engines-table.md) and the `[SHOW ENGINES](../../../show/show-engines.md)` statement.
