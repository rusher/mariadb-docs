...
`session_connect_attrs` is a [Performance Schema](../performance-schema-status-variables.md) table that shows connection attributes for all sessions. The Performance Schema needs to be enabled for the table to be populated.
...
Connection attributes values are set using the option [connectAttributes](https://mariadb.com/kb/en/nodejs-connection-options/#other-options)
