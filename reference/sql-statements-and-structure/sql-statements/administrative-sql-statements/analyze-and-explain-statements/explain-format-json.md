...
`EXPLAIN FORMAT=JSON` is a variant of [EXPLAIN](../../../../../clients-and-utilities/explain-analyzer.md) command that produces output in JSON form. The output always has one row which has only one column titled "`JSON`". The contents are a JSON representation of the query plan, formatted for readability:
...
* MySQL's output has deficiencies. Some are listed here: [EXPLAIN FORMAT=JSON in MySQL](https://app.gitbook.com/s/iJPrPCGi329TSR8WIXJW/learning-and-training/training-and-tutorials/advanced-mariadb-articles/development-articles/outdated-pages/explain-formatjson-in-mysql)
