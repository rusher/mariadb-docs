...
`SHOW PLUGINS` displays information about installed [plugins](/en/mariadb-plugins/). The `Library` column indicates the plugin library - if it is `NULL`, the plugin is built-in and cannot be uninstalled.

The `[PLUGINS](/en/information_schemaplugins-table/)` table in the `information_schema` database contains more detailed information.
...
* [INFORMATION_SCHEMA.ALL_PLUGINS Table](/en/information-schema-all_plugins-table/) (all plugins, installed or not)
