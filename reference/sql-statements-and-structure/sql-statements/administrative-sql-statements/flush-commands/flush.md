...
By default, `FLUSH` statements are written to the [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) and will be [replicated](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/mariadb-releases/compatibility-differences/replication-compatibility-between-mariadb-and-mysql). The `NO_WRITE_TO_BINLOG` keyword (`LOCAL` is an alias) will ensure the statement is not written to the binary log.
...
| DES_KEY_FILE | Reloads the DES key file (Specified with the [--des-key-file startup option](/kb/en/mysqld-options-full-list/)). |
...
| [ERROR | ENGINE | GENERAL | SLOW | RELAY] LOGS | Close and reopen the specified log type, or all log types if none are specified. FLUSH RELAY LOGS [connection-name] can be used to flush the relay logs for a specific connection. Only one connection can be specified per FLUSH command. See [Multi-source replication](../../../../../server-usage/replication-cluster-multi-master/standard-replication/multi-source-replication.md). FLUSH ENGINE LOGS will delete all unneeded [Aria](../../../../../security/securing-mariadb/securing-mariadb-encryption/encryption-data-at-rest-encryption/aria-encryption/aria-enabling-encryption.md) redo logs. FLUSH BINARY LOGS DELETE_DOMAIN_ID=(list-of-domains) can be used to discard obsolete [GTID](../../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) domains from the server's [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) state. In order for this to be successful, no event group from the listed [GTID](../../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) domains can be present in existing [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) files. If some still exist, then they must be purged prior to executing this command. If the command completes successfully, then it also rotates the [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md). |
| BINARY LOGS | FLUSH BINARY LOGS rotates the current [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md). |
| BINARY LOGS DELETE_DOMAIN_ID=(list-of-domains) | FLUSH BINARY LOGS DELETE_DOMAIN_ID can be used to discard obsolete [GTID](../../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) domains from the server's [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) state. In order for this to be successful, no event group from the listed [GTID](../../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) domains can be present in existing [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) files. If some still exist, then they must be purged prior to executing this command. If the command completes successfully, then it also rotates the [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md). |
...
| [QUERY CACHE](flush-query-cache.md) | Defragment the [query cache](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/buffers-caches-and-threads/query-cache.md) to better utilize its memory. If you want to reset the query cache, you can do it with [RESET QUERY CACHE](../reset.md). |
| QUERY_RESPONSE_TIME | See the [QUERY_RESPONSE_TIME](/kb/en/query_response_time-plugin/) plugin. |
| QUERY_RESPONSE_TIME_READ | See the [QUERY_RESPONSE_TIME](/kb/en/query_response_time-plugin/) plugin. From [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115). |
| QUERY_RESPONSE_TIME_READ_WRITE | See the [QUERY_RESPONSE_TIME](/kb/en/query_response_time-plugin/) plugin. From [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115). |
| QUERY_RESPONSE_TIME_WRITE | See the [QUERY_RESPONSE_TIME](/kb/en/query_response_time-plugin/) plugin. From [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115). |
| SLAVE | Deprecated option, use [RESET REPLICA or RESET SLAVE](../replication-commands/reset-replica.md) instead. |
| SSL | Used to dynamically reinitialize the server's [TLS](/kb/en/data-in-transit-encryption/) context by reloading the files defined by several [TLS system variables](../../../../../security/securing-mariadb/securing-mariadb-encryption/data-in-transit-encryption/ssltls-system-variables.md). See [FLUSH SSL](#flush-ssl) for more information. |
| [ GLOBAL | SESSION ] STATUS | Resets all [server status variables](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-status-variables.md) that can be reset to 0. Not all global status variables support this, so not all global values are reset. From [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115), GLOBAL or SESSION can be specified. See [FLUSH STATUS](#flush-status) for more information. |
...
Until [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115), the variables flushed are mainly session, but also some global. Not all session (or global) variables are flushed - the decision was made per variable.

#

#### MariaDB starting with [11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115)

From [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115) it is possible to specify FLUSH GLOBAL or FLUSH SESSION. Flushing of global status variables has been moved to `FLUSH GLOBAL STATUS` (in [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115), this flushes only the global variables previously flushed by `FLUSH STATUS`. In a future release, it is intended for this to reset all global summary variables.) `FLUSH SESSION STATUS` is a synonym for `FLUSH STATUS`. 
...
The `FLUSH SSL` command can be used to dynamically reinitialize the server's [TLS](/kb/en/data-in-transit-encryption/) context. This is most useful if you need to replace a certificate that is about to expire without restarting the server.
