...
deprecated in favor of `SET` without `OPTION`, and was removed in [MariaDB 10.0](/kb/en/what-is-mariadb-100/).

Changing a system variable by using the SET statement does not make the change permanently. To do so, the change must be made in a [configuration file](/kb/en/mysqld-configuration-files-and-groups/).
