...
`REPLACE` works exactly like
 `[INSERT](../../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md)`, except that if an old row in the table
...
The RETURNING clause was introduced in [MariaDB 10.5.0](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1050-release-notes)
...
`REPLACE` is a MariaDB/MySQL extension to the SQL standard. It
 ...
 standard SQL --- that also handle duplicate values --- see [IGNORE](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/optimization-and-indexes/ignored-indexes.md) and [INSERT ON DUPLICATE KEY UPDATE](../inserting-loading-data/insert-on-duplicate-key-update.md).
...
To use `REPLACE`, you must have both the
 `INSERT` and `DELETE` [privileges](../../account-management-sql-commands/grant.md)
for the table.
