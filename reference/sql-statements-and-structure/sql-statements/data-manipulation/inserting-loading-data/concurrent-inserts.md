...
The [MyISAM](../../../../../clients-and-utilities/myisam-clients-and-utilities/myisamchk-table-information.md) storage engine supports concurrent inserts. This feature allows [SELECT](../../../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) statements to be executed during [INSERT](../../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md) operations, reducing contention.
...
If the [binary log](../../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) is used, [CREATE TABLE ... SELECT](../../data-definition/create/create-tablespace.md#create-table-select) and [INSERT ... SELECT](insert-select.md) statements cannot use concurrent inserts. These statements acquire a read lock on the table, so concurrent inserts will need to wait. This way the log can be safely used to restore data.

Concurrent inserts are not used by replicas with the row based [replication](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/mariadb-releases/compatibility-differences/replication-compatibility-between-mariadb-and-mysql) (see [binary log formats](../../../../../server-management/server-monitoring-logs/binary-log/binary-log-formats.md)).

If an [INSERT](../../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md) statement contain the [HIGH_PRIORITY](/en/high_priority-and-low_priority-clauses/) clause, concurrent inserts cannot be used. [INSERT ... DELAYED](insert-delayed.md) is usually unneeded if concurrent inserts are enabled.
...
[LOCK TABLES](/en/transactions-lock/) allows non-conflicting concurrent inserts if a `READ LOCAL` lock is used. Concurrent inserts are not allowed if the `LOCAL` keyword is omitted.
