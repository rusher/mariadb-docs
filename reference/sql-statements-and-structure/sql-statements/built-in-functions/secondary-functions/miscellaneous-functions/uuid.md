...
Returns a Universally Unique Identifier (UUID) version 1. Functions to generate v4 and v7 UUIDs are available from [MariaDB 11.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-117). See [UUIDv4](/kb/en/uuidv4/) and [UUIDv7](/kb/en/uuidv7/) respectively.
...
Statements using the UUID() function are not [safe for statement-based replication](/kb/en/unsafe-statements-for-replication/).

The function generates a UUIDv1 and the results are generated according to the "DCE 1.1:Remote Procedure Call" (Appendix A) CAE (Common Applications Environment) Specifications published by The Open Group in October 1997 ([Document Number C706](http://www.opengroup.org/public/pubs/catalog/c706.htm)).
