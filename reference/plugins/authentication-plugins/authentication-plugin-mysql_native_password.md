...
However, the `mysql_native_password` client authentication plugin is generally statically linked into client libraries like `libmysqlclient` or [MariaDB Connector/C](https://app.gitbook.com/s/CjGYMsT2MVP4nd3IyW2L/c++/mariadb-connector-cpp-1-0-release-notes/mariadb-connector-cpp-1-0-3-release-notes), so this is not usually necessary.
