...
A normal-size (double-precision) floating-point number (see [FLOAT](floating-point-accuracy.md) for a single-precision floating-point number).
...
REAL and DOUBLE PRECISION are synonyms, unless the REAL_AS_FLOAT [SQL mode](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/mariadb-releases/compatibility-differences/sql_modemssql) is enabled, in which case REAL is a synonym for [FLOAT](floating-point-accuracy.md) rather than DOUBLE.
