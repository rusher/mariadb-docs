...
[Indexing](/en/optimization-and-indexes/) is one of the main ways to optimize queries. Key columns, in
...
because [CONNECT](../../../../security/user-account-management/catalogs/connecting-to-a-server-configured-for-catalogs.md) indexes store the values in the order they appear into the
...
You can define them in the [CREATE TABLE](../../../sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md) statement, or either using the CREATE
...
reconstructed to prevent returning false or incomplete results. To do this, use the [OPTIMIZE TABLE](../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/optimizing-tables/optimize-table.md) command.
 ...
 *dept.dat* of type DOS is a file named *dept_PRIMARY.dnx*. This makes
...
[OPTIMIZE TABLE](../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/optimizing-tables/optimize-table.md) command.
...
enabled versions, file mapping is used according to the boolean [connect_indx_map](../connect-system-variables.md#connect_indx_map) system variable. Set it to 0 (file read) or 1 (file mapping).
...
be *scattered*, *clustered*, or *sorted*. In general only one column can be sorted. Block indexing is used
...
Remote indexing is specific to the [MYSQL](/en/connect-table-types-mysql-table-type-accessing-mysqlmariadb-tables/) table type. It is equivalent to what the [FEDERATED](/en/federatedx-storage-engine/)
storage does. A MYSQL table does not support indexes per se. Because access to the table is handled
remotely, it is the remote table that supports the indexes. What the MYSQL table does is just to add a WHERE clause to the [SELECT](../../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) command sent to the remote server allowing the remote server to use
...
See [Indexing of MYSQL tables](/en/connect-table-types-mysql-table-type-accessing-mysqlmariadb-tables/#indexing-of-mysql-tables) for more.
...
[SHOW INDEX](../../../sql-statements-and-structure/sql-statements/administrative-sql-statements/show/show-index.md) command.
