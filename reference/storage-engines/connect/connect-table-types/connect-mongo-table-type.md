...
1. As a MONGO table via the MongoDB Java Driver.
1. As a JDBC table using some commercially available MongoDB JDBC drivers.
1. As a JSON table via the MongoDB C or Java Driver.
...
1. Configure, compile and install MariaDB.
...
1. Add the path to it in the CLASSPATH environment variable or in the connect_class_path variable. This is like what is done to declare JDBC drivers.
...
In some case or some platforms, when CONNECT is set up for use with JDBC table types, this causes [mariadb-dump](../../../../clients-and-utilities/mariadb-dumpslow.md) with the --all-databases option to fail.
