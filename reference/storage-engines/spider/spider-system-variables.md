...
The following variables are available when the [Spider](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/spider-status-variables.md) storage engine has been installed.
...
* Description: Storage engine used with temporary tables when the [spider_bka_mode](#spider_bka_mode) system variable is set to `1`. Defaults to the value of the [table parameter](spider-table-parameters.md), which is [MEMORY](../memory-storage-engine.md) by default.
* Scope: Global, Session
* Dynamic: Yes
* Data Type: `string`
...
* Introduced: [MariaDB 10.5.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1054-release-notes)
...
* Description: Bulk update and delete mode. Note: If you use a non-default value for the [spider_bgs_mode](#spider_bgs_mode) or [spider_split_read](#spider_split_read) system variables, Spider sets this variable to `2`.
...
* Description: Casual Reads enables all isolation levels, (such as repeatable reads) to work with transactions on multiple backends. With auto-commit queries, you can relax read consistency and run on multiple connections to the backends. This enables parallel queries across partitions, even if multiple shards are stored on the same physical server. Deprecated in [MariaDB 11.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-115) due to the complexity of the code for little benefit.
...
* Dynamic: Yes
* Data Type: `numeric`
* Default Session Value: `2` (>= [MariaDB 11.8](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-118)), `1000` (<= [MariaDB 11.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-117))
...
* Description: Indexes cardinality statistics in the background. Disable when the [spider_crd_mode](#spider_crd_mode) system variable is set to `3` or when the [spider_crd_interval](#spider_crd_interval) variable is set to `0`.
...
* Description: Type of cardinality calculation. Only effective when the [spider_crd_mode](#spider_crd_mode) system variable is set to use `SHOW` (`1`) or to use the Information Schema (`2`).

 * `-1` Falls back to the default value, if the [table parameter](spider-table-parameters.md) is not set.
 * `0` Uses the value of the [spider_crd_weight](#spider_crd_weight) system variable, as a fixed value.
 * `1` Uses the value of the [spider_crd_weight](#spider_crd_weight) system variable, as an addition value.
 * `2` Uses the value of the [spider_crd_weight](#spider_crd_weight) system variable, as a multiplication value.
* Scope: Global, Session
* Dynamic: Yes
* Data Type: `numeric`
* Default Session Value: `2`
...
* Description: Manages duplicate key check for [REPLACE](../../../clients-and-utilities/replace-utility.md), [INSERT IGNORE](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/optimization-and-indexes/ignored-indexes.md) and [LOAD DATA LOCAL INFILE](../../sql-statements-and-structure/sql-statements/data-manipulation/inserting-loading-data/load-data-into-tables-or-index/load-data-infile.md) to remote servers. This can save on network roundtrips if the key always maps to a single partition. For bulk operations, records are checked for duplicate key errors one by one on the remote server, unless you set it to avoid duplicate checks on local servers (`1`).
...
* Data Type: `boolean`
* Default Value: `OFF`
* Introduced: [MariaDB 10.10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-10-10-series/mariadb-10-10-7-release-notes)
...
* Description: Whether to push [LOCK TABLES](/kb/en/transactions-lock/) statements down to the remote server.
...
* Description: 
Whether to convert [SELECT... LOCK IN SHARE MODE](../../sql-statements-and-structure/sql-statements/data-manipulation/selecting-data/lock-in-share-mode.md) and [SELECT... FOR UPDATE](../../sql-statements-and-structure/sql-statements/data-manipulation/selecting-data/for-update.md) statements into a [LOCK TABLE](../../sql-statements-and-structure/sql-statements/data-manipulation/selecting-data/lock-in-share-mode.md) statement.
 ...
 * `3` Log warning
 * `4` Log info (Added in [MariaDB 10.5.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1054-release-notes))
* Scope: Global
* Dynamic: Yes
* Data Type: `numeric`
* Default Value: `0`
 ...
 * `3` Local buffering, uses a temporary table on disk when the result-set is greater than the value of the [spider_quick_page_size](#spider_quick_page_size) system variable.
* Scope: Global, Session
* Dynamic: Yes
* Data Type: `numeric`
...
* Dynamic: Yes
* Data Type: `numeric`
* Default Session Value: `1024` (>=[MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107)), `-1` (<= [MariaDB 10.6](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-106))
* Default Table Value: `100`
* Range: `-1` to `9223372036854775807`
...
* Dynamic: Yes
* Data Type: `numeric`
* Default Session Value: `0` (>=[MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107)), `-1` (<= [MariaDB 10.6](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-106))
* Default Table Value: `0`
* Range: `-1` to `1`
...
* Description: Forces the specified session [character set](/kb/en/character-sets/) when connecting to the backend server. This can improve connection time performance.
* Scope: Global, Session
* Dynamic: Yes
* Data Type: `string`
...
* Default Value: `-1`
* Range: `-1` to `2147483647`
* Introduced: [MariaDB 10.4.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-104-series/mariadb-1045-release-notes)
...
* Description: Number of second read records on the backend server when the first records are found from the first read. Spider continues scanning a range, adding a `LIMIT` using the [spider_first_read](#spider_first_read) and [spider_second_read](#spider_second_read) variables.
...
* Description: Enables semi-table locking. This adds a [LOCK TABLES](/kb/en/transactions-lock/) statement to SQL executions sent to the remote backend server when using non-transactional storage engines to preserve consistency between roundtrips.
...
* Dynamic: Yes
* Data Type: `numeric`
* Default Session Value: `0` (>=[MariaDB 10.8](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-108)), `1` (<= [MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107))
* Range: `0` to `1`
...
Mode for table statistics. The [SHOW](../../../security/user-account-management/catalogs/show-create-catalog.md) command is used at the table level default.
...
* Data Type: `boolean`
* Default Value: `ON`
* Introduced: [MariaDB 10.4.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-104-series/mariadb-1047-release-notes)
...
* Data Type: `boolean`
* Default Session Value: `OFF`
* Removed: [MariaDB 10.3.9](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-10-3-series/mariadb-1039-release-notes)
...
* Description: Converts [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) SQL statements.
When the [spider_sync_trx_isolation](#spider_sync_trx_isolation) system variable is set to `0`, Spider disables [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) conversions to prevent use of the statement on the [SERIALIZABLE](../../sql-statements-and-structure/sql-statements/transactions/set-transaction.md#serializable) isolation level.

 * `-1` Falls back to the default value, if the [table parameter](spider-table-parameters.md) is not set.
 * `0` Converts [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) statements into [SELECT](../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) statements.
 * `1` Passes [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) to the remote backend server.
 * `2` Converts SQL statements to [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) statements.
 * `3` Converts SQL statements to [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) statements and [HANDLER](../../sql-statements-and-structure/nosql/handlersocket/handlersocket-external-resources.md) statements to SQL statements.
* Scope: Global, Session
* Dynamic: Yes
* Data Type: `numeric`
* Default Value: `0`
...
* Description: 
When using a UDF function in a condition and the [engine_condition_pushdown](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#engine_condition_pushdown) system variable is set to `1`, whether to execute the UDF function locally or push it down.
 ...
 * `0` : No encapsulation.
 * `1` : Encapsulates, only when the [spider_use_all_conns_snapshot](#spider_use_all_conns_snapshot) system variable i set to `1`.
 * `2` : 
...
* Description: Whether to use the local table [character set](/kb/en/data-types-character-sets-and-collations/) for the remote backend server connections.
...
* Description: The current Spider version. Removed in [MariaDB 10.9.2](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-10-9-series/mariadb-1092-release-notes) when the Spider version number was matched with the server version.
* Scope: Global
* Dynamic: No
* Data Type: `string`
* Removed: [MariaDB 10.9.2](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-10-9-series/mariadb-1092-release-notes)
