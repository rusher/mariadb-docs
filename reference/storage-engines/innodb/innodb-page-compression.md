...
However, on many distributions, the standard MariaDB builds do not support all InnoDB page compression algorithms by default. From [MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107), algorithms can be [installed as a plugin](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/optimization-and-tuning-compression/compression-plugins.md).

This system variable can be changed dynamically with [SET GLOBAL](../../../server-usage/replication-cluster-multi-master/standard-replication/setting-up-replication.md#global-session). For example:
...
The [zlib](https://www.zlib.net/) compression algorithm is always supported. From [MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107), algorithms can be [installed as a plugin](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/optimization-and-tuning-compression/compression-plugins.md).
...
| --- | --- |
| Status Variable | Description |
| [Innodb_have_lz4](/kb/en/innodb-server-status-variables/#innodb_have_lz4) | Whether InnoDB supports the [lz4](https://code.google.com/p/lz4/) compression algorithm. |
| [Innodb_have_lzo](/kb/en/innodb-server-status-variables/#innodb_have_lzo) | Whether InnoDB supports the [lzo](http://www.oberhumer.com/opensource/lzo/) compression algorithm. |
| [Innodb_have_lzma](/kb/en/innodb-server-status-variables/#innodb_have_lzma) | Whether InnoDB supports the [lzma](http://tukaani.org/xz/) compression algorithm. |
| [Innodb_have_bzip2](/kb/en/innodb-server-status-variables/#innodb_have_bzip2) | Whether InnoDB supports the [bzip2](http://www.bzip.org/) compression algorithm. |
| [Innodb_have_snappy](/kb/en/innodb-server-status-variables/#innodb_have_snappy) | Whether InnoDB supports the [snappy](http://google.github.io/snappy/) compression algorithm. |
...
See [Compiling MariaDB From Source](../../../server-management/getting-installing-and-upgrading-mariadb/compiling-mariadb-from-source/compiling-mariadb-from-source-mariadb-source-configuration-options.md) for more information.
...
InnoDB page compression is only supported if the InnoDB table uses the `Barracuda` [file format](/kb/en/xtradbinnodb-file-format/).Therefore, in [MariaDB 10.1](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-1011) and before, the [innodb_file_format](innodb-system-variables.md#innodb_file_format) system variable must be set to `Barracuda` to use InnoDB page compression.
...
This system variable's session value can be changed dynamically with [SET SESSION](../../../server-usage/replication-cluster-multi-master/standard-replication/setting-up-replication.md#global-session). For example:
...
InnoDB page compression can be enabled for individual tables by setting the [PAGE_COMPRESSED](../../sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md#page_compressed) table option to `1`. For example:
...
The compression level for individual tables can also be configured by setting the [PAGE_COMPRESSION_LEVEL](../../sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md#page_compression_level) table option for the table. For example:
...
In [MariaDB 10.3](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-103) and later, InnoDB uses the *punch hole* technique to create sparse files used automatically when the underlying file system supports sparse files.

In [MariaDB 10.2](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-102) and before, InnoDB can be configured to use the *punch hole* technique to create sparse files by configuring the [innodb_use_trim](innodb-system-variables.md#innodb_use_trim) and [innodb_use_fallocate](innodb-system-variables.md#innodb_use_fallocate) system variables. These system variables can be set in a server [option group](../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md#option-groups) in an [option file](../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md) prior to starting up the server. For example:
...
InnoDB page compression was originally developed by collaborating with [Fusion-io](http://fusionio.com). As a consequence, it was originally designed to work best on [FusionIO devices](../../../server-management/getting-installing-and-upgrading-mariadb/mariadb-performance-advanced-configurations/fusion-io/fusion-io-introduction.md) using [NVMFS](https://ieeexplore.ieee.org/document/6558434). [Fusion-io](http://fusionio.com) has since been acquired by [Western Digital](https://www.westerndigital.com/), and they have decided not to continue supporting [NVMFS](https://ieeexplore.ieee.org/document/6558434).
...
| --- | --- |
| Status Variable | Description |
| [Innodb_page_compression_saved](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_saved) | Bytes saved by compression |
| [Innodb_page_compression_trim_sect512](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect512) | Number of 512 sectors trimmed |
| [Innodb_page_compression_trim_sect1024](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect1024) | Number of 1024 sectors trimmed |
| [Innodb_page_compression_trim_sect2048](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect2048) | Number of 2048 sectors trimmed |
| [Innodb_page_compression_trim_sect4096](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect4096) | Number of 4096 sectors trimmed |
| [Innodb_page_compression_trim_sect8192](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect8192) | Number of 8192 sectors trimmed |
| [Innodb_page_compression_trim_sect16384](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect16384) | Number of 16384 sectors trimmed |
| [Innodb_page_compression_trim_sect32768](/kb/en/xtradbinnodb-server-status-variables/#innodb_page_compression_trim_sect32768) | Number of 32768 sectors trimmed |
| [Innodb_num_pages_page_compressed](/kb/en/xtradbinnodb-server-status-variables/#innodb_num_pages_page_compressed) | Number of pages compressed |
| [Innodb_num_page_compressed_trim_op](/kb/en/xtradbinnodb-server-status-variables/#innodb_num_page_compressed_trim_op) | Number of trim operations |
| [Innodb_num_page_compressed_trim_op_saved](/kb/en/xtradbinnodb-server-status-variables/#innodb_num_page_compressed_trim_op_saved) | Number of trim operations saved |
| [Innodb_num_pages_page_decompressed](/kb/en/xtradbinnodb-server-status-variables/#innodb_num_pages_page_decompressed) | Number of pages decompressed |
| [Innodb_num_pages_page_compression_error](/kb/en/xtradbinnodb-server-status-variables/#innodb_num_pages_page_compression_error) | Number of compression errors |
...
[Mariabackup](../../../server-usage/replication-cluster-multi-master/galera-cluster/state-snapshot-transfers-ssts-in-galera-cluster/mariabackup-sst-method.md) supports InnoDB page compression.
