...
Sometimes a [binary log](../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) event causes an error of some sort. A whole binary log file is sometimes impractical due to size or sensitivity reasons.
...
Binary logs have a header portion. Without the header [mariadb-binlog](../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md) won't be able to read it. The header also contains valuable session information
...
```
mariadb-binlog --base64-output=decode-rows --verbose mysql-bin.000687 | grep '^
...
Upload this to the [private uploads](/en/ftp/) or attach to the public bug report if nothing sensitive there.