...
This utility tool is used to read messages from a RabbitMQ broker sent by the [RabbitMQ Filter](maxscale-rabbitmq-filter.md) and forward these messages into an SQL database as queries.
...
To install the RabbitMQ Consumer Client you ca either use the provided packages or you can compile it from source code. The source code is included as a part of the MaxScale source code and can be found in the `<code>rabbitmq_consumer</code>` folder.
