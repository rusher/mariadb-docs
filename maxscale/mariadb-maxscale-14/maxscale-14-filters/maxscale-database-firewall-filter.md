...
This parameter is optional and determines what action is taken when a query matches a rule. The value can be either `<code>allow</code>`, which allows all matching queries to proceed but blocks those that don't match, or `<code>block</code>`, which blocks all matching queries, or `<code>ignore</code>` which allows all queries to proceed.
