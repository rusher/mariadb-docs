...
The debugcli supports two modes of operation, `<code>developer</code>` and `<code>user</code>`. The mode is set via the `<code>router_options</code>` parameter. The user mode is more suited to end-users and administrators, whilst the develop mode is explicitly targeted to software developing adding or maintaining the MaxScale code base. Details of the differences between the modes can be found in the debugging guide for MaxScale. The default is `<code>user</code>` mode. The following service definition would enable a developer version of the debugcli.
...
It should be noted that both `<code>user</code>` and `<code>developer</code>` instances of debugcli may be defined within the same instance of MaxScale, however they must be defined as two distinct services, each with a distinct listener.
