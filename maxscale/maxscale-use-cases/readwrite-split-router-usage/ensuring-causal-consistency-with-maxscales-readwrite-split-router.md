---
description: >-
  Prevent stale reads in replication clusters. Configure the causal_reads
  setting to ensure clients always see their own updates by waiting for replicas
  to catch up.
---
...
1. Set the causal\_reads and `causal_reads_timeout` parameters for the Read/Write Split Router in `maxscale.cnf`. The `causal_reads` parameter can be set to the following values:
