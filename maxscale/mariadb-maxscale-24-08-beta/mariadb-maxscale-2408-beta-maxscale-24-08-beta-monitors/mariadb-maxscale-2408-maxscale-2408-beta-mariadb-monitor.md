...
* [MariaDB Monitor](#mariadb-monitor)
  ...
  * [Cluster manipulation operations](#cluster-manipulation-operations)
    ...
    * [Manual activation](#manual-activation)
      ...
      * [Switchover with key-value arguments](#switchover-with-key-value-arguments)
  ...
  * [Primary server write test](#primary-server-write-test)
...
[Monitor Common](mariadb-maxscale-2408-maxscale-2408-beta-common-monitor-parameters.md) document.
...
[disk space check](mariadb-maxscale-2408-maxscale-2408-beta-common-monitor-parameters.md) is enabled. Added in
MaxScale 23.08.5.
...
parameters [disk_space_threshold](mariadb-maxscale-2408-maxscale-2408-beta-common-monitor-parameters.md) and
[disk_space_check_interval](mariadb-maxscale-2408-maxscale-2408-beta-common-monitor-parameters.md)
...
This alternative form of *switchover* accepts the following arguments:
...
The key-value syntax thus supports the same features as the old *switchover*,
*async-switchover* and *switchover-force*-commands.
...
In addition, key-value argument passing supports `<code>old_primary_maint</code>`. This
...
`<code>old_primary_maint=1</code>` to promote one of the replicas while leaving the old
...
Enable automatic primary failover. `<code>true</code>`, `<code>on</code>`, `<code>yes</code>` and `<code>1</code>` enable normal
failover. `<code>false</code>`, `<code>off</code>`, `<code>no</code>` and `<code>0</code>` disable the feature. `<code>safe</code>` enables
...
online during failover. If `<code>auto_rejoin</code>` is on, Replica A will quickly be
...
Also, [disk_space_check_interval](mariadb-maxscale-2408-maxscale-2408-beta-common-monitor-parameters.md)
...
[here](../mariadb-maxscale-24-08-beta-getting-started/mariadb-maxscale-2408-maxscale-2408-beta-mariadb-maxscale-configuration-guide.md). If no explicit unit
...
within [backend_read_timeout](mariadb-maxscale-2408-maxscale-2408-beta-common-monitor-parameters.md),
...
#### `<code>write_test_fail_action</code>`
...
Which action to take if primary server fails the write test. `<code>log</code>` means that
MaxScale will simply log the failure but perform no other action. This is mainly
useful for testing the feature.


If set to `<code>failover</code>`, the monitor will perform a failover if the primary server
fails the write test [failcount](#failcount) consecutive times. That is,
the first write test is performed after `<code>write_test_interval</code>` has passed without
writes. If the test fails, the monitor will repeat the test during the next
monitor tick. After `<code>failcount</code>` monitor ticks with failed write tests, failover
...
[Configuration guide](../mariadb-maxscale-24-08-beta-getting-started/mariadb-maxscale-2408-maxscale-2408-beta-mariadb-maxscale-configuration-guide.md). Setting
