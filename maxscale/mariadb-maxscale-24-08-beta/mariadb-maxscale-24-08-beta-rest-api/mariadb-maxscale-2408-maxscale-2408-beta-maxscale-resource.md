...
`<code>/v1/maxscale/logs/data</code>` endpoint except that this is a resource collection
...
Any PATCH requests done to this endpoint will be redirected to the
 `<code>/v1/maxscale</code>` endpoint. Due to the misspelling of the `<code>ms_timestamp</code>` and
...
Here is an example POST requests to the mariadbmon module command *reset-replication* with
