...
* [Hint Syntax](#hint-syntax)
  ...
  * [Hint body](#hint-body)
    ...
    * [Routing destination hints](#routing-destination-hints)
      ...
      * [Route to primary](#route-to-primary)
      * [Route to replica](#route-to-replica)
...
* [Examples](#examples)
  ...
  * [Routing SELECT queries to primary](#routing-select-queries-to-primary)
...
A `<code>master</code>` value in a routing hint will route the query to a primary server. This
...
A `<code>slave</code>` value will route the query to a replica server. Please note that the
...
`<code>mariadb_stmt_execute</code>` the result is always returned from the primary:
...
`<code>EXECUTE</code>` will be routed to the primary as a result of it having the `<code>route to
master</code>` hint.
...
## Routing `<code>SELECT</code>` queries to primary
