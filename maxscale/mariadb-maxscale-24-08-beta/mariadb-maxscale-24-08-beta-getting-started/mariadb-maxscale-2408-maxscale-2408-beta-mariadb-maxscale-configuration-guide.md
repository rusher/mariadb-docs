...
*true* values and `false`, `no`, `off` and `0` as *false* values. Starting with
...
[the session state information](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/4-server-response-packets/ok_packet#session-state-info)
...
1. Drop the `mysql.maxscale_config` table
...
1. Give MaxScale read permissions on it with `chown maxscale:maxscale /var/lib/maxscale/encryption.key`
