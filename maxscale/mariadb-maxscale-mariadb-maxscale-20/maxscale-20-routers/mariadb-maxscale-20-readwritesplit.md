...
The `<code>LEAST_GLOBAL_CONNECTIONS</code>` and `<code>LEAST_ROUTER_CONNECTIONS</code>` use the connections from MariaDB MaxScale to the server, not the amount of connections reported by the server itself.


`<code>LEAST_BEHIND_MASTER</code>` does not take server weights into account when choosing a server.
...
* With `<code>slave_selection_criteria=LEAST_GLOBAL_CONNECTIONS</code>` and `<code>max_slave_connections=1</code>`, each session picks
 one slave and one master
* With `<code>slave_selection_criteria=LEAST_CURRENT_OPERATIONS</code>` and `<code>max_slave_connections=100%</code>`, each session
 picks one master and as many slaves as possible
* With `<code>slave_selection_criteria=LEAST_CURRENT_OPERATIONS</code>` each read is load balanced based on how many
 queries are active on a particular slave
* With `<code>slave_selection_criteria=LEAST_GLOBAL_CONNECTIONS</code>` each read is sent to the slave with the least
...
that in *fail_on_write* or *error_on_write* mode, connections are accepted as long
...
syntax and functionality, please read [this](../../mariadb-maxscale-21-06/README.md) document.
...
For a list of readwritesplit limitations, please read the [Limitations](../about-maxscale-20/mariadb-maxscale-20-limitations-and-known-issues-within-mariadb-maxscale.md) document.
