...
can be influenced with the server priority mechanic described in the[Galera Monitor](../maxscale-22-monitors/mariadb-maxscale-22-galera-monitor.md) manual.
...
will always be routed to the master. See[strict\_multi\_stmt](../maxscale-22-routers/mariadb-maxscale-22-readwritesplit.md#strict_multi_stmt) for more\
details.

Execution of LOAD DATA LOCAL INFILE statements inside a multi-statement query is\
not supported. If one is executed MaxScale will most likely hang (see[MXS-1828](https://jira.mariadb.org/browse/MXS-1828)).
