...
Issues [MXS-710](https://jira.mariadb.org/browse/MXS-710) and[MXS-711](https://jira.mariadb.org/browse/MXS-711) relate to this.
...
can be influenced with the server priority mechanic described in the[Galera Monitor](../../../archive-of-2x.xx-versions/mariadb-maxscale-21-06/) manual.
...
recommended to set the wait\_timeout to a high value and let MaxScale handle the\
client timeouts. This can be achieved by using the[connection\_timeout](../maxscale-20-getting-started/mariadb-maxscale-20-mariadb-maxscale-configuration-usage-scenarios.md#connection_timeout)\
...
For more information, read the[ReadWriteSplit](../../../archive-of-2x.xx-versions/mariadb-maxscale-21-06/) router documentation.

#### Parsing related limitations

Galera Cluster variables, such as @@wsrep\_node\_name, are not resolved by\
...
of sending them just to one of server. These queries include `USE <db name>` and `SET autocommit=0` among many others. Readwritesplit sends a\
