...
{% hint style="warning" %}
The fallback to asynchronous replication is expected behavior in the setup this page describes. However, if multiple MaxScales monitor the cluster with `cooperative_monitoring_locks=majority_of_all`, leaving `rpl_semi_sync_master_timeout` at a low value exposes the setup to a risk: after the fallback, transactions can be successfully committed on the minority side of a network partition and are lost when the partition heals. In that configuration, set the timeout to its maximum value. See [Failover With Multiple MaxScales](failover-with-multiple-maxscales.md).
{% endhint %}
