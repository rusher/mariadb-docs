...
solution to [MaxCtrl](../reference/maxscale-maxctrl.md).
...
1. [MaxScale object](../maxscale-management/deployment/maxscale-configuration-guide.md). i.e.
...
can be viewed by mousing over the server name. A tooltip will be displayed with
the following information: replication\_state, seconds\_behind\_master,
...
Sessions created by the "Workspace", such as via the Query Editor can be found
in two places:
...
[mariadbmon](../reference/maxscale-monitors/mariadb-monitor.md) module in a card-like view.
...
4. Carousel navigation button. Viewing more information about the server in the
   next slide.
4. Collapse the carousel.
5. Anchor link of the server. Opening the detail page of the server in a new tab.
6. Collapse its children nodes.
7. Rejoin node. When the `auto_rejoin` parameter is disabled, the node can be
...
Right-click on the name of the schema to show the context menu, then select
the desired option.
...
To view the statement that creates the given object in the
[Schemas objects sidebar](#schemas-objects-sidebar),
right-clicking on schema or table node and select the `View Insights` option.
For other objects such as view, stored procedure, function and trigger,
select the `Show Create` option.
...
necessary configurations on the MaxScale server. Instruction can be found [here](../reference/maxscale-rest-api/maxscale-sql-resource.md)
and limitations [here](../maxscale-management/mariadb-maxscale-limitations-guide.md#etl-limitations).
...
From an empty new worksheet, clicking on the "Create an ERD" card will open
a connection dialog. After connecting successfully, the ERD worksheet will be
...
`Export as jpeg`. These options can be accessed using either of the following methods:
