...
using [mysql\_clear\_password](../../../server/reference/clientserver-protocol/1-connecting/connection.md#mysql_clear_password-plugin).
...
setting [user\_accounts\_file](../../maxscale-management/deployment/maxscale-configuration-guide.md#user_accounts_file).
...
One array, "users\_and\_passwords", is read from the file.
...
Anonymous mapping is only attempted if the client username is not found in the`mysql.user` table
...
Optionally, the PAM configuration on the backend servers can be weakened such that
