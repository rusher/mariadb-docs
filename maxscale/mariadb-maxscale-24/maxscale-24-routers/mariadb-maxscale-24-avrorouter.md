...
[Binlog Server](mariadb-maxscale-24-binlogrouter.md).
...
servers as the replication source. In this mode the `<code>source</code>` parameter is
...
The *server_id* used when replicating from the master in direct
...
**Note:** If the `<code>source</code>` parameter is defined the values for `<code>binlogdir</code>` and
 `<code>filestem</code>` are only read from the source service. This means that the
...
currently, if used with Avrorouter, the option `<code>mariadb10_master_gtid</code>` must be
...
[Building MaxScale from Source Code](../maxscale-24-getting-started/mariadb-maxscale-24-building-mariadb-maxscale-from-source-code.md)
document.
