...
The [MariaDB Monitor](../maxscale-23-monitors/mariadb-maxscale-23-mariadb-monitor.md) is not only capable
...
which `<code>server1</code>` is the initial master and the other servers are slaves.
...
and want to make `<code>server1</code>` master again, then we must issue the following
command:
...
want to make into master, and* `<code>server2</code>` is the third argument to the command, the name of the *current*
*master*.
