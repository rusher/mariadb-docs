...
is now replicating.
1. It was previously part of a multimaster group but is no longer, or the
...
The `<code>user</code>` requires privileges depending on which monitor features are used.
...
[Monitor Common](mariadb-maxscale-23-common-monitor-parameters.md) document.
...
a slave connection with `<code>Master_Host</code>` and `<code>Master_Port</code>` equal to B's address and
...
(`<code>auto_failover=true</code>`). If automatic failover is not on, the monitor will try to
...
parameters [disk_space_threshold](https://mariadb.com/kb/en/node:mariadb-maxscale-23-common-monitor-parameters#disk_space_threshold) and
[disk_space_check_interval](https://mariadb.com/kb/en/node:mariadb-maxscale-23-common-monitor-parameters#disk_space_check_interval)
...
1. If the new master has unprocessed relay log items, cancel and try again
later.
1. Prepare the new master:

  1. Remove the slave connection the new master used to replicate from the old
 master.
  1. Disable the read_only-flag.
  1. Enable scheduled server events (if event handling is on). Only events that were
...
1. Redirect all other slaves to replicate from the new master:
  ...
  1. CHANGE MASTER TO
  1. START SLAVE
1. Check that all slaves are replicating.
...
1. Prepare the old master for demotion:
  ...
  1. Kill connections from super-users since read_only does not affect them.
...
1. Reset gtid:s and delete binary logs on all servers:
 ...
 gtid_current_pos.
1. Prepare new master:

  1. Disable the read_only-flag.
  1. Enable scheduled server events (if event handling is on). Events are only enabled
...
1. Direct other servers to replicate from the new master as in the other
operations.
...
commands generally match their automatic versions. The exception is *rejoin*, in
...
for the [server](../../mariadb-maxscale-21-06/README.md)
or the [monitor](./node:mariadb-maxscale-23-common-monitor-parameters#disk_space_threshold).
Also, [disk_space_check_interval](./node:mariadb-maxscale-23-common-monitor-parameters#disk_space_check_interval)
...
feature and `<code>master_failure_timeout</code>` defines the timeout in seconds (default:
10).
...
`<code>master_failure_timeout</code>` seconds, the master is not considered down when
...
[Configuration guide](../../mariadb-maxscale-21-06/README.md). Setting
...
[Replication Proxy](../maxscale-tutorials/mariadb-maxscale-23-mariadb-maxscale-as-a-binlog-server.md)
tutorial.
