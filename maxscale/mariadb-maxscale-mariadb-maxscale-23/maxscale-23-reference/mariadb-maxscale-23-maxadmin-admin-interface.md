...
* [MaxAdmin - Admin Interface](#maxadmin-admin-interface)
...
* [Working With Administration Interface Users](#working-with-administration-interface-users)
  ...
  * [Administrative and Read-only Users](#administrative-and-read-only-users)
  * [What Users Have Been Defined?](#what-users-have-been-defined)
  * [Enabling a Linux account](#enabling-a-linux-account)
  * [Disabling a Linux account](#disabling-a-linux-account)
  * [Add A New User](#add-a-new-user)
  * [Delete A User](#delete-a-user)
  * [Creating Read-only Users](#creating-read-only-users)
* [Command Line Switches](#command-line-switches)

  * [Interactive Operation](#interactive-operation)
  * [Command Line Operation](#command-line-operation)
  * [The .maxadmin file](#the-maxadmin-file)
* [Getting Help](#getting-help)
* [Working With Services](#working-with-services)

  * [What Services Are Available?](#what-services-are-available)
  * [See Service Details](#see-service-details)
  * [Examining Service Users](#examining-service-users)
  * [Reloading Service User Data](#reloading-service-user-data)
...
* [Working With Servers](#working-with-servers)
  ...
  * [What Servers Are Configured?](#what-servers-are-configured)
  * [Server Details](#server-details)
  * [Setting The State Of A Server](#setting-the-state-of-a-server)
  * [Viewing the persistent pool of DCB](#viewing-the-persistent-pool-of-dcb)
* [Working With Sessions](#working-with-sessions)

  * [What Sessions Are Active in MariaDB MaxScale?](#what-sessions-are-active-in-mariadb-maxscale)
  * [Display Session Details](#display-session-details)
* [Descriptor Control Blocks](#descriptor-control-blocks)

  * [Finding DCB’s](#finding-dcbs)
  * [DCB Of A Client Connection](#dcb-of-a-client-connection)
  * [DCB Details](#dcb-details)
* [Working with Filters](#working-with-filters)

  * [What Filters Are Configured?](#what-filters-are-configured)
  * [Retrieve Details Of A Filter Configuration](#retrieve-details-of-a-filter-configuration)
  * [Filter Usage](#filter-usage)
* [Working With Monitors](#working-with-monitors)

  * [What Monitors Are Running?](#what-monitors-are-running)
  * [Details Of A Particular Monitor](#details-of-a-particular-monitor)
  * [Shutting Down A Monitor](#shutting-down-a-monitor)
  * [Restarting A Monitor](#restarting-a-monitor)
* [MaxScale Status Commands](#maxscale-status-commands)

  * [MariaDB MaxScale Thread Usage](#mariadb-maxscale-thread-usage)
  * [The Housekeeper Tasks](#the-housekeeper-tasks)
* [Administration Commands](#administration-commands)

  * [What Modules Are In use?](#what-modules-are-in-use)
  * [Enabling syslog and maxlog logging](#enabling-syslog-and-maxlog-logging)
  * [Rotating the log file](#rotating-the-log-file)
  * [Change MariaDB MaxScale Logging Options](#change-mariadb-maxscale-logging-options)
  * [Adjusting the Log Throttling](#adjusting-the-log-throttling)
  * [Reloading The Configuration](#reloading-the-configuration)
  * [Shutting Down MariaDB MaxScale](#shutting-down-mariadb-maxscale)
* [Runtime Configuration Changes](#runtime-configuration-changes)

  * [Servers](#servers)

    * [Creating a New Server](#creating-a-new-server)
    * [Adding Servers to Services and Monitors](#adding-servers-to-services-and-monitors)
    * [Removing Servers from Services and Monitors](#removing-servers-from-services-and-monitors)
    * [Altering Servers](#altering-servers)
    * [Destroying Servers](#destroying-servers)
  * [Listeners](#listeners)

    * [Creating New Listeners](#creating-new-listeners)
    * [Destroying Listeners](#destroying-listeners)
  * [Monitors](#monitors)

    * [Creating New Monitors](#creating-new-monitors)
    * [Altering Monitors](#altering-monitors)
    * [Destroying Monitors](#destroying-monitors)
  * [Services](#services)

    * [Altering Services](#altering-services)
  * [MaxScale Core](#maxscale-core)

    * [Altering MaxScale](#altering-maxscale)
  * [Other Modules](#other-modules)
* [MaxScale Internals](#maxscale-internals)
...
the network interface is `<code>admin</code>` with the password `<code>mariadb</code>`. The default
user for the UNIX domain socket interface is `<code>root</code>`.


Users that can only perform read-only operations are created with `<code>add
readonly-user</code>` command. These users can only perform operations that fetch data
...
Currently, the `<code>list</code>` and `<code>show</code>` type commands are the only operations that
read-only users can perform.


To create a read-only network user, use the `<code>add readonly-user</code>` command. To
enable a local Linux account as a read-only user, use the `<code>enable
readonly-account</code>` command. Both administrative and read-only users can be
deleted with the `<code>remove user</code>` and `<code>disable account</code>` commands.
...
The *show threads* command can be used to determine how many descriptors the
...
The `<code>Load (1s)</code>` column shows the load during the last measured second, an
...
`<code>show threads</code>` command was issued.


The load during the measured second is defined as
`<code>100 * (1 - time-spent-blocked-in-epoll_wait() / 1)</code>` which translates into
a load of `<code>0</code>` if the thread is blocked in `<code>epoll_wait()</code>` for the entire
second, waiting for events to process, and `<code>100</code>` if the thread spends no time
blocked in `<code>epoll_wait()</code>` but processing events for the entire duration.


The `<code>Load (1m)</code>` value is the moving average of the last 60 second load values
and the `<code>Load (1h)</code>` value is the moving average of the last 60 minute load
values.
...
**Note:** This command has been deprecated. Use the MaxScale REST API or the
 MaxAdmin `<code>alter</code>` commands to change configuration values at runtime.
...
To alter the common service parameters, use the `<code>alter service</code>` command. Module
...
the `<code>alter maxscale</code>` command.
...
long it has taken for events having been returned from `<code>epoll_wait</code>`
