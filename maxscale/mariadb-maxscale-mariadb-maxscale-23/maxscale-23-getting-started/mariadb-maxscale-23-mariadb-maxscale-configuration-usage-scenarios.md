...
**Note:** This parameter is deprecated and it is ignored by MaxScale versions
 ...
 directory with `<code>logdir</code>` in `<code>/dev/shm</code>`.


In older MaxScale versions, the actual log file was created in `<code>/dev/shm</code>` and a
...
The default is `<code>10, 1000, 10000</code>`, which means that if the same error is logged
...
The `<code>persistmaxtime</code>` parameter defaults to zero but can be set to an integer
