...
* there is an explicitly read-only transaction (that is,`START TRANSACTION
 READ ONLY`) active, or
...
That is, the `SELECT` is performed in the database server *before* the
...
* Type: [size](https://mariadb.com/kb/en/maxscale-2208-getting-started-mariadb-maxscale-configuration-guide/#sizes)
* Mandatory: No
* Dynamic: No
* Default: `0` (no limit)
...
[here](/en/maxscale-2208-getting-started-mariadb-maxscale-configuration-guide/#sizes).
...
[here](/en/maxscale-2208-getting-started-mariadb-maxscale-configuration-guide/#durations).
...
Currently there are two *shared* storages; `storage_memcached` and
...
However, when the
[query classifier cache](https://mariadb.com/kb/en/maxscale-2208-getting-started-mariadb-maxscale-configuration-guide/#query_classifier_cache_size)
