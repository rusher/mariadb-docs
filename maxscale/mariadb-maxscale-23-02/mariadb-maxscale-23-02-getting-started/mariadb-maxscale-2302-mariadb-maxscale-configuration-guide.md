...
### `<code>skip_permission_checks</code>`
...
#### `<code>log_unrecognized_statements</code>`
...
Network throttling is only enabled when both `<code>writeq_high_water</code>` and
`<code>writeq_low_water</code>` have a non-zero value. To disable throttling, set the value
to 0.
...
`<code>targets</code>` parameter, the `<code>connection_timeout</code>` for those is not used.


The value of `<code>connection_timeout</code>` should be lower than the lowest `<code>wait_timeout</code>`
...
`<code>connection_timeout</code>` and `<code>wait_timeout</code>`. This way the client application will
...
In MaxScale versions 6.2.0 and older, if long-running operations (e.g. `<code>ALTER
TABLE</code>`) were performed, MaxScale would close the connections if they look longer
than `<code>connection_timeout</code>` seconds to execute
...
primary. Read the
[readwritesplit documentation on server ranks](../mariadb-maxscale-23-02-routers/mariadb-maxscale-2302-readwritesplit.md)
...
[Server Priorities](../mariadb-maxscale-23-02-monitors/mariadb-maxscale-2302-galera-monitor.md#interaction-with-server-priorities)
...
See [Change Data Capture Protocol](../mariadb-maxscale-23-02-protocols/mariadb-maxscale-2302-change-data-capture-cdc-protocol.md) for more information.
...
Custom CA certificates can be defined with the `<code>ssl_ca</code>` parameter.
...
`<code>REQUIRE X509</code>` was defined for all users. For servers, this behaves like the
`<code>--ssl-verify-server-cert</code>` command line option for the `<code>mysql</code>` client.
...
done. To require peer certificates, use `<code>ssl_verify_peer_certificate</code>`.
...
* [ReadConnRoute](../mariadb-maxscale-23-02-routers/mariadb-maxscale-2302-readconnroute.md)


Read/Write aware statement based router:
* [ReadWriteSplit](../mariadb-maxscale-23-02-routers/mariadb-maxscale-2302-readwritesplit.md)
...
![](../../../.gitbook/assets/mariadb-corporation/MaxScale/23.02.13/Documentation/Getting-Started/images/filter_example.png.png)
...
* [Regular Expression Filter](../mariadb-maxscale-23-02-filters/mariadb-maxscale-2302-regex-filter.md)
* [Tee Filter](../mariadb-maxscale-23-02-filters/mariadb-maxscale-2302-tee-filter.md)
* [Top Filter](../mariadb-maxscale-23-02-filters/mariadb-maxscale-2302-top-filter.md)
* [Query Redirection Filter](../mariadb-maxscale-23-02-filters/mariadb-maxscale-2302-named-server-filter.md)
...
The current limitations of MaxScale are listed in the [Limitations](../mariadb-maxscale-23-02-about/mariadb-maxscale-2302-limitations-and-known-issues-within-mariadb-maxscale.md) document.
