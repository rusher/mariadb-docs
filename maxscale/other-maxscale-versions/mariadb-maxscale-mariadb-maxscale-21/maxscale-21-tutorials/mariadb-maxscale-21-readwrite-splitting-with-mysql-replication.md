...
The first part of this tutorial is covered in[MariaDB MaxScale Tutorial](mariadb-maxscale-21-setting-up-mariadb-maxscale.md). Please read it and follow the\
...
module to use and the monitored servers. The section `type` should be set to`monitor`. Parameters added include: the list of servers to monitor and the\
...
The final stage in the configuration is to add the service which used by the`maxadmin` command to connect to MariaDB MaxScale for monitoring and\
