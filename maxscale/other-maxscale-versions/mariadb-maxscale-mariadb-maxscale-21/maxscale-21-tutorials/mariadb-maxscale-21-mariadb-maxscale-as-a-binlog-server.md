# mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server
...
* [Introduction](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#introduction)
* [MariaDB as a Binlog Server](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#mariadb-as-a-binlog-server)
* [MariaDB MaxScale's approach](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#mariadb-maxscales-approach)
* [Configuring MariaDB MaxScale as a Binlog Server](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#configuring-mariadb-maxscale-as-a-binlog-server)
* [Service Configuration](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#service-configuration)
* [Listener Configuration](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#listener-configuration)
* [Configuring Replication](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#configuring-replication)
* [Binlog Router Compatibility](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#binlog-router-compatibility)
* [MariaDB MaxScale Replication Diagnostics](mariadb-maxscale-21-mariadb-maxscale-as-a-binlog-server.md#mariadb-maxscale-replication-diagnostics)
...
received via replication from the master server. This is done with th&#x65;_&#x6C;og\_slave\_updates_ configuration option of the server. In this case the server\
...
made on the master. If the _log\_slave\_updates_ option has been enabled, new\
...
service definition with the _maxscale.cnf_ file. The service requires a name\
...
replication proxies this router name is _binlogrouter_.
...
Documentation](../maxscale-21-routers/mariadb-maxscale-21-binlogrouter.md).
...
The protocol used by slaves for connection to MariaDB MaxScale is the sam&#x65;_&#x4D;ySQLClient_ protocol that is used for client applications to connect to\
...
Refer to the [Configuration-Guide](../maxscale-21-getting-started/mariadb-maxscale-21-mariadb-maxscale-configuration-usage-scenarios.md)\
...
Both the _MASTER\_LOG\_FILE_ and _MASTER\_LOG\_POS_ must be defined and the value o&#x66;_&#x4D;ASTER\_LOG\_POS_ must be 4.
...
A successful configuration change results in _master.ini_ being updated. Any\
...
Further details about level of encryption or certificates could be found in the[Configuration Guide](../maxscale-21-getting-started/mariadb-maxscale-21-mariadb-maxscale-configuration-usage-scenarios.md)

#### Slave servers setup

Examples of _CHANGE MASTER TO_ command issued on a slave server that wants to\
...
The latter example specifies a _MASTER\_LOG\_POS_ for the selecte&#x64;_&#x4D;ASTER\_LOG\_FILE_

**Note:**

* MASTER\_LOG\_FILE must be set to one of existing binlog files in MariaDB\
  MaxScale binlogdir
* If MASTER\_LOG\_POS is not set with CHANGE MASTER TO it defaults to 4
* Latest binlog file name and pos in MariaDB MaxScale can be found by executing`SHOW MASTER STATUS` on MaxScale.

#### Controlling the Binlogrouter

There are some constraints related to _MASTER\_LOG\_FILE_ and _MASTER\_LOG\_POS_._MASTER\_LOG\_FILE_ can be changed to next binlog in sequence wit&#x68;_&#x4D;ASTER\_LOG\_POS=4_ or to current one at current position.
...
This could be applied to current master\_host/port or a new one. If there is a\
...
replication stops and errors are reported via _SHOW SLAVE STATUS_ and in error\
logs.
...
This could be applied with current master\_host/port or a new one If transaction\
...
In some situations replication state could be _STOPPED_ and proper messages are\
displayed in error logs and in _SHOW SLAVE STATUS_. In order to resolve any\
mistake done with _CHANGE MASTER TO MASTER\_LOG\_FILE / MASTER\_LOG\_POS_, another\
...
This command removes _master.ini_ file, blanks all master configuration in\
memory and sets binlog router in unconfigured state: a _CHANGE MASTER TO_\
...
* In order to enable/re-enable Master SSL comunication the MASTER\_SSL=1 option\
...
* New certificate options changes take effect after maxScale restart or after\
  MASTER\_SSL=1 with the new options.
* SHOW SLAVE STATUS displays all the options but MASTER\_TLS\_VERSION value.
* Maxadmin, 'show services' or 'show service $binlog\_service' displays all the\
...
In order to use it with MySQL 5.6/5.7, the _GTID\_MODE_ setting must be OFF and\
connecting slaves must not use _MASTER\_AUTO\_POSITION = 1_ option. Additionally\
with MySQL 5.7 slaves the `send_slave_heartbeat` option must be set to on.

Binlog Router currently does not work for MySQL 5.5 due to missin&#x67;_@@global.binlog\_checksum_ variable.
