# MariaDB MaxScale 2.2 RabbitMQ Setup and Integration
...
them to this location, provide a different value for the CMAKE\_INSTALL\_PREFIX variable.
...
operations with amqp\_\* tools, located in the examples/ folder of the build directory,\
...
schema ‘test’ with target ‘my1’ the option logging\_strict must be set\
to TRUE and if we want to include those selects without schema name the\
option logging\_log\_all must be set to TRUE.
