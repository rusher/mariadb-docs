...
retrieved multiple times. Results can also be explicitly discarded with the new`DELETE /sql/:id/queries/:query_id` endpoint.
...
`maxctrl (list|show) sessions` and _MaxGUI_ now show for each session a&#x6E;_&#x69;/o activity_ number that gives an indication of the load of a particular\
