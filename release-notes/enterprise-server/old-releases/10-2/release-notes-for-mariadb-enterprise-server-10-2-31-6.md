...
* For [MariaDB Enterprise Cluster](https://github.com/mariadb-corporation/docs-release-notes/blob/test/en/galera-cluster/README.md) (Galera Library 25.3.29):
  * Setting [socket.recv\_buf\_size](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/reference/wsrep-variable-details/wsrep_provider_options#socketrecv_buf_size) was not effective because it was done after the socket was connected or accepted. The default value also caused TCP receive buffer auto-tuning to be disabled. This lead to sub-optimal performance in high bandwidth WAN clusters. The default value for [socket.recv\_buf\_size](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/reference/wsrep-variable-details/wsrep_provider_options#socketrecv_buf_size) has been changed to auto, which lets the kernel tune the TCP receive buffer. A new variable `socket.send_buf_size` with default value `auto` was added to allow send buffer tuning.
...
* Possible server crash if [binlog\_checksum=CRC32](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/standard-replication/binlog-event-checksums) (default) is set and if the value for parameter `pos` in [SHOW BINLOG EVENTS FROM pos](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/administrative-sql-statements/show/show-binlog-events) does not point to the beginning of an event in the binary log. ([MDEV-18046](https://jira.mariadb.org/browse/MDEV-18046))
...
* [MariaDB Enterprise Backup](../../10-2/broken-reference/) could encounter a deadlock if a backup is taken from a replica with [slave-parallel-threads](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#slave_parallel_threads) is set to a value greater than `0` ([MDEV-21255](https://jira.mariadb.org/browse/MDEV-21255))
* [MariaDB Enterprise Backup](../../10-2/broken-reference/) could crash when used in combination with `FLUSH TABLES` and `UNLOCK TABLES` (MENT-438)
...
* Dropping a partition with [wsrep\_OSU\_method=RSU](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#wsrep_OSU_method) and SESSION [sql\_log\_bin=0](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/standard-replication/replication-and-binary-log-system-variables#sql_log_bin) caused the [MariaDB Enterprise Cluster](https://github.com/mariadb-corporation/docs-release-notes/blob/test/en/galera-cluster/README.md) (Galera) node to hang. ([MDEV-21189](https://jira.mariadb.org/browse/MDEV-21189))

### Can result in unexpected behavior

* Executing [TRUNCATE](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-functions/numeric-functions/truncate) or [OPTIMIZE](https://app.gitbook.com/s/WCInJQ9cmGjq1lsTG91E/development-articles/mariadb-internals/mariadb-internals-documentation-query-optimizer/mariadb-internals-documentation-optimizer-trace) on [InnoDB](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb) tables could lead to an unexpected `SQL Error (1118): Row size too large`, when [innodb\_strict\_mode=ON](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#innodb_strict_mode) (default). ([MDEV-21429](https://jira.mariadb.org/browse/MDEV-21429))
...
* Using the --use-memory option with [mariadb-backup](../../10-2/broken-reference/) would cause it to run out of memory. ([MDEV-20679](https://jira.mariadb.org/browse/MDEV-20679))
* For [MariaDB Enterprise Cluster](https://github.com/mariadb-corporation/docs-release-notes/blob/test/en/galera-cluster/README.md) (Galera Library 25.3.29):
...
In alignment with the [enterprise lifecycle](../../about/enterprise-server-lifecycle.md), MariaDB Enterprise Server 10.2.31-6 is provided for:
