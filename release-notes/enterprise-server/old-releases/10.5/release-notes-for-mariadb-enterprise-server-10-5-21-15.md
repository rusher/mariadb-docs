---
description: >-
  MariaDB Enterprise Server 10.5.21-15 is a Stable (GA) maintenance release of
  MariaDB Enterprise Server 10.5, released on 2023-06-13
---
...
* When a view's definition contains a table-value constructor (TVC) as a single-value subquery, selecting from the view can fail with an error. ([MDEV-28603](https://jira.mariadb.org/browse/MDEV-28603))
  * In previous releases, queries could raise an error with the [ER\_VIEW\_INVALID error code](../10-5/broken-reference/) and the following error message:
...
* When the [JSON\_OBJECTAGG function](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-functions/aggregate-functions/json_objectagg) is called with a key value that includes a double quote, the double quote character is not escaped. ([MDEV-30412](https://jira.mariadb.org/browse/MDEV-30412))
