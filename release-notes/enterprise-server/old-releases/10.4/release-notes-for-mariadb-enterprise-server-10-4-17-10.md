...
* Galera wsrep library updated to 26.4.6 in [MariaDB Enterprise Cluster](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/).
...
* Better MariaDB GTID support for the [mariadb-backup](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/backup-and-restore/mariadb-backup) [--slave-info](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#slave-info) option. ([MDEV-19264](https://jira.mariadb.org/browse/MDEV-19264))
...
* A rolling upgrade for [MariaDB Enterprise Cluster](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/) from 10.3 to 10.4 can result in data loss. ([MDEV-22723](https://jira.mariadb.org/browse/MDEV-22723))
...
* [InnoDB](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb) fails to open the table during removal of VIRTUAL column DDL while using [FOREIGN\_KEY\_CHECKS=0](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#foreign_key_checks), due to lack of referenced index. ([MDEV-23387](https://jira.mariadb.org/browse/MDEV-23387))
...
* [MariaDB Enterprise Cluster](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/) node crash with Galera message `Assertion` server\_state\_.rollback\_mode() == wsrep::server\_state::rm\_async' failed\`. in the error log. (MENT-937)
...
* [MariaDB Enterprise Cluster](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/) crash if bulk updates are executed on Galera. ([MDEV-23872](https://jira.mariadb.org/browse/MDEV-23872))
...
* [MariaDB Enterprise Cluster](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/) node can crash with an error message `WSREP: MDL BF-BF conflict` in the error log. The error is related to tables with foreign keys and running [OPTIMIZE](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/optimization-and-tuning/optimizing-tables/optimize-table) or [REPAIR](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/table-statements/repair-table) on them. ([MDEV-21577](https://jira.mariadb.org/browse/MDEV-21577))

### Can result in unexpected behavior

* Defining a view with SQL syntax `ISNULL(ID)=0` incorrectly returns a syntax error. (MENT-1015)
* [MariaDB Enterprise Backup](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/backup-and-restore/mariadb-backup) reports an error that it cannot find an Aria log file `'./aria_log.00000000'` (MENT-907)
* [MariaDB Enterprise Backup](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/backup-and-restore/mariadb-backup) failure for incremental backups. ([MDEV-24026](https://jira.mariadb.org/browse/MDEV-24026))
...
* [Galera](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/) replication broken if only one timezone is loaded. ([MDEV-22626](https://jira.mariadb.org/browse/MDEV-22626))
...
* [MariaDB Enterprise Backup](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/backup-and-restore/mariadb-backup) SST fails for [MariaDB Enterprise Cluster](https://app.gitbook.com/s/3VYeeVGUV4AMqrA3zwy7/) if data-directory has `lost+found` directory. ([MDEV-21951](https://jira.mariadb.org/browse/MDEV-21951))
...
* `CREATE TEMPORARY TABLE .. LIKE` ([system versioned table](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-structure/temporal-tables/system-versioned-tables)) returns error if unique index is defined in the table. ([MDEV-23968](https://jira.mariadb.org/browse/MDEV-23968))
...
In alignment with the [enterprise lifecycle](../../enterprise-server-lifecycle.md), MariaDB Enterprise Server 10.4.17-10 is provided for:
