---
description: >-
  MariaDB Enterprise Server 11.6.12-8 is a Stable (GA) maintenance release of
  MariaDB Enterprise Server 10.6, released on 2023-05-24
---
...
* When InnoDB undo log truncation is enabled by setting [innodb\_undo\_log\_truncate=ON](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb/innodb-system-variables#innodb_undo_log_truncate), if some undo log tablespace exceeds [innodb\_max\_undo\_log\_size](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb/innodb-system-variables#innodb_max_undo_log_size), the server can hang. ([MDEV-30863](https://jira.mariadb.org/browse/MDEV-30863))
...
* InnoDB does not free undo logs when they are no longer needed. ([MDEV-31234](https://jira.mariadb.org/browse/MDEV-31234))
  * In the previous release, when [innodb\_undo\_tablespaces](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb/innodb-system-variables#innodb_undo_tablespaces) is set to 0, the undo logs would be stored in the InnoDB system tablespace (`ibdata1` by default), so this issue could cause the InnoDB system tablespace to grow indefinitely.
