...
MariaDB Enterprise Server 10.6.8-4 is a Stable (GA) maintenance release of [MariaDB Enterprise Server]({server}) 10.6. This release includes a variety of fixes.
...
* The server and [MariaDB Enterprise Backup]({server}/server-usage/backup-and-restore/mariadb-enterprise-backup) can raise unnecessary warnings about tablespace IDs in some scenarios. ([MDEV-27343](https://jira.mariadb.org/browse/MDEV-27343))
...
* This warning can be written to standard output (stdout) when preparing a backup with [MariaDB Enterprise Backup]({server}/server-usage/backup-and-restore/mariadb-enterprise-backup).
