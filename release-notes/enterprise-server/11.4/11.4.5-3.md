...
MariaDB Enterprise Server 11.4.5-3 is a Stable (GA) maintenance release of [MariaDB Enterprise Server](https://github.com/mariadb-corporation/docs-release-notes/blob/test/kb/en/mariadb-enterprise-server/README.md) 11.4. This release includes a variety of fixes.
...
* Starting with this release we generate a Software Bill of Materials (SBOM) JSON file provided in the downloads archive, which can be reached from the "All Files" link on the MariaDB Enterprise Server downloads page ([MENT-2234](https://jira.mariadb.org/browse/MENT-2234))
...
* [Galera](https://github.com/mariadb-corporation/docs-release-notes/blob/test/kb/en/galera-cluster/README.md) updated to [26.4.21](../../galera-cluster/26.4/26.4.21.md)
...
* Fix possible assertion failure when Galera cluster is in 'split-brain' state due to loss of communication between nodes (fix requires Galera library 26.4.21+) ([MENT-2175](https://jira.mariadb.org/browse/MENT-2175))
* Fix assertion failure when executing `XA PREPARE` (and possibly other XA statements) on Galera cluster nodes ([MENT-2212](https://jira.mariadb.org/browse/MENT-2212))
* Fix assertion failure when executing `XA` statements on Galera cluster nodes ([MENT-2214](https://jira.mariadb.org/browse/MENT-2214))
* In rare cases, an `ALTER TABLE` or other operation could previously hang when using `NBO` mode on a cluster with very low network latencies (for example, when both nodes are running on the same physical machine) ([MENT-2215](https://jira.mariadb.org/browse/MENT-2215))
...
* `Replicate_*` fields in `Show-Slave-Status` may be truncated, impacting replication monitoring. ([MENT-2238](https://jira.mariadb.org/browse/MENT-2238))
...
For the complete list of changes in this release, see the [changelog](changelog-11.4.5-3.md).
