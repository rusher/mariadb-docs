...
* [MariaDB Enterprise Backup](broken-reference/) could encounter a deadlock if a backup is taken from a replica with [slave-parallel-threads](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/optimization-and-tuning/system-variables/server-system-variables#slave_parallel_threads) is set to a value greater than `0` ([MDEV-21255](https://jira.mariadb.org/browse/MDEV-21255))
* [MariaDB Enterprise Backup](broken-reference/) could crash when used in combination with `FLUSH TABLES` and `UNLOCK TABLES` (MENT-438)
