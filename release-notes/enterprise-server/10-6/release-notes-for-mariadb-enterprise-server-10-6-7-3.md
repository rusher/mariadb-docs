...
* When a CTE is used in a subquery of a `DELETE` statement, the server incorrectly raises an error with the [ER\_NO\_DB\_ERROR](broken-reference) error code. ([MDEV-26470](https://jira.mariadb.org/browse/MDEV-26470))
...
* When [innodb\_buffer\_pool\_size](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb/innodb-system-variables#innodb_buffer_pool_size) is changed dynamically with [SET GLOBAL](https://mariadb.com/kb/en/SET-GLOBAL), InnoDB does not enforce the minimum value that is enforced on startup, which can result in an impossibly small buffer pool with some values of [innodb\_buffer\_pool\_chunk\_size](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb/innodb-system-variables#innodb_buffer_pool_chunk_size). ([MDEV-27467](https://jira.mariadb.org/browse/MDEV-27467))
...
* When a [system-versioned table](https://mariadb.com/kb/en/system-versioned-table) is partitioned by [SYSTEM\_TIME\` with a \`\`LIMIT clause, CHECK TABLE](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/table-statements/check-table) can incorrectly return an error. ([MDEV-25552](https://jira.mariadb.org/browse/MDEV-25552))
...
* [rocksdb\_ignore\_datadic\_errors](broken-reference) system variable added
