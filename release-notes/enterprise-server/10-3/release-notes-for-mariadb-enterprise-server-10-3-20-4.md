...
* [mariabackup](broken-reference) [--prepare](broken-reference) [--export](broken-reference) ... could overwrite binary logs if certain conditions were present. ([MDEV-20703](https://jira.mariadb.org/browse/MDEV-20703)) Conditions which must be present to trigger this bug:
...
* If unable to upgrade to MariaDB Enterprise Server 10.3.20-4, where this bug is fixed, a workaround is available: use the `--defaults` option to [mariabackup](broken-reference) to avoid the bug-triggering conditions by specifying a different configuration file.

#### Can result in a hang or crash

* Prior removal of a \`#FULLTEXT

## index from an [InnoDB](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb) table can cause a hang on startup. ([MDEV-19647](https://jira.mariadb.org/browse/MDEV-19647))\`
