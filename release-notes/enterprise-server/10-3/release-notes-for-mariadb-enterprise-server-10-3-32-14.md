...
* [Enterprise Spider](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/spider) no longer enables pushdown of UDFs and Stored Functions to the Data Node by default: ([MDEV-26545](https://jira.mariadb.org/browse/MDEV-26545))
  * The default value of [spider\_use\_pushdown\_udf](https://mariadb.com/kb/en/spider-system-variable/#spider_use_pushdown_udf) has changed from `-1` to `0`
...
* If two InnoDB tables have a [foreign key](https://mariadb.com/kb/en/foreign-key/) and an operation cascades from the parent table to the child table, an index on a virtual generated column in the child table can become corrupt. ([MDEV-26866](https://jira.mariadb.org/browse/MDEV-26866))
...
* [mariadb --binary-mode](https://mariadb.com/kb/en/binary-mode) is not able to replay some [mysqlbinlog](https://mariadb.com/kb/en/mariadb-binlogs) outputs if `0` is in the data. ([MDEV-25444](https://jira.mariadb.org/browse/MDEV-25444))
