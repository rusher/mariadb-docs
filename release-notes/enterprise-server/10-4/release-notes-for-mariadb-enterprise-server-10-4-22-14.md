...
* In packets sent from the server, a column's `"Original Name"` field can contain the column's alias instead of original name of the column. This metadata mismatch can cause [MariaDB Connectors](https://mariadb.com/kb/en/other-connectors-methods/) to raise an exception. ([MDEV-23519](https://jira.mariadb.org/browse/MDEV-23519))
