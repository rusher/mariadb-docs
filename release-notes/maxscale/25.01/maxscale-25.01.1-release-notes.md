...
This feature is an alternative to enabling [log\_info](../../mariadb-maxscale-release-notes/Getting-Started/Configuration-Guide.md#log_info) which is is not always feasible in a production system due to the high volume of log data that it creates. This overhead of writing large amounts of trace logging data could be mitigated by placing the log directory on a volatile in-memory filesystem but this risks losing important warning and error messages if the system were to be restarted.
