---
description: >-
  Release notes for MariaDB Connector/C 3.3.0.rc1, a Release Candidate (RC)
  release published 15 February 2022.
---
...
Added new option MARIADB\_OPT\_RESTRICTED\_AUTH (and corresponding "restricted-auth" option for configuration files) which specifies on or more comma separated authentication plugins which are allowed for authentication.
...
The connection string must contain at least one semicolon, otherwise it will be interpreted as hostname. Unknown or invalid keys will be ignored.

To connect via connection string, the following methods might be used:

* by specifying connection option in configuration file:
...
on each push, see the [changelog](../changelogs/3.3/3.3.0.md).
