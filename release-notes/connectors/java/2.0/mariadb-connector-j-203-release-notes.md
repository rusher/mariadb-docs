# MariaDB Connector/J 2.0.3 Release Notes

The most recent [_**Stable**_](../../../mariadb-release-criteria.md) _**(GA)**_ release of [MariaDB Connector/J](https://github.com/mariadb-corporation/docs-release-notes/blob/test/en/about-mariadb-connector-j/README.md) is:[**MariaDB Connector/J 3.5.3**](../3.5/mariadb-connector-j-3-5-3-release-notes.md)

[Download](https://downloads.mariadb.org/connector-java/2.0.3/)[Release Notes](mariadb-connector-j-203-release-notes.md)[Changelog](../changelogs/2.0/mariadb-connector-j-203-changelog.md)[Connector/J Overview](https://github.com/mariadb-corporation/docs-release-notes/blob/test/en/about-mariadb-connector-j/README.md)

**Release date:** 27 Jun 2017

MariaDB Connector/J 2.0.3 is a [_**Stable**_](../../../mariadb-release-criteria.md) _**(GA)**_\
release.

**For an overview of MariaDB Connector/J see the**[**About MariaDB Connector/J**](https://github.com/mariadb-corporation/docs-release-notes/blob/test/en/about-mariadb-connector-j/README.md) **page**

### Bug

* [CONJ-473](https://jira.mariadb.org/browse/CONJ-473) - when useServerPrepStmts is not set, the PREPARE statement must not be cached.
* [CONJ-494](https://jira.mariadb.org/browse/CONJ-494) - Handle PrepareStatement.getParameterMetaData() if query cannot be PREPAREd
* [CONJ-497](https://jira.mariadb.org/browse/CONJ-497) - escape string correction for big query

{% include "https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/~/reusable/pNHZQXPP5OEz2TgvhFva/" %}

{% @marketo/form formid="4316" formId="4316" %}
