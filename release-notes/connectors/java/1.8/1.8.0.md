---
description: >-
  Release notes for MariaDB Connector/J 1.8.0, a Stable (GA) release published
  11 February 2019.
---
...
[Download](https://mariadb.com/downloads/connectors) | **Release Notes** | [Changelog](../changelogs/1.8/1.8.0.md) | [About MariaDB Connector/J](https://app.gitbook.com/s/CjGYMsT2MVP4nd3IyW2L/mariadb-connector-j/about-mariadb-connector-j)
...
* \[[CONJ-675](https://jira.mariadb.org/browse/CONJ-675)] permit multiple alternative authentication methods for the same user (future [MariaDB 10.4](../../../community-server/old-releases/10.4/what-is-mariadb-104.md) feature)
...
* \[[CONJ-670](https://jira.mariadb.org/browse/CONJ-670)] ability to Refresh the SSL certificate
...
* \[[CONJ-623](https://jira.mariadb.org/browse/CONJ-623)] Increase connection logging when the Primary node connection fails
* \[[CONJ-384](https://jira.mariadb.org/browse/CONJ-384)] Add option permit having "affected" or "found" rows

h3. Bugfixes

* \[[CONJ-646](https://jira.mariadb.org/browse/CONJ-646)] possible NullPointerException when the connection is lost to the database using aurora configuration with one node
...
* \[[CONJ-673](https://jira.mariadb.org/browse/CONJ-673)] aborting a connection while fetching a query still reads the whole resultset
* \[[CONJ-669](https://jira.mariadb.org/browse/CONJ-669)] SQLSyntaxErrorException when querying on an empty column name
* \[[CONJ-674](https://jira.mariadb.org/browse/CONJ-674)] make dumpQueriesOnException = false by default as per documentation
* \[[CONJ-616](https://jira.mariadb.org/browse/CONJ-616)] correction on possible NPE on getConnection when using failover configuration and the master is down, not throwing a proper exception
...
* \[[CONJ-622](https://jira.mariadb.org/browse/CONJ-622)] The option "connectTimeout" must take into account DriverManager.getLoginTimeout() when set
* \[[CONJ-621](https://jira.mariadb.org/browse/CONJ-621)] wrong escaping when having curly brackets in table/field name
