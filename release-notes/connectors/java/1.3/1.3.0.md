---
description: >-
  Release notes for MariaDB Connector/J 1.3.0, a Stable (GA) release published
  16 November 2015.
---
...
[Download](https://downloads.mariadb.org/connector-java/1.3.0/) | **Release Notes** | [Changelog](../changelogs/1.3/1.3.0.md) | [About MariaDB Connector/J](https://app.gitbook.com/s/CjGYMsT2MVP4nd3IyW2L/mariadb-connector-j/about-mariadb-connector-j)
...
information on each push, see the [changelog](../changelogs/1.3/1.3.0.md).
...
```java
PreparedStatement pst = connection.prepareStatement("INSERT INTO example VALUES (?)");
int i=1000;
while(i>0) {
    pst.setInt(1, i--);
    pst.addBatch();
...
_When connecting, the driver will always try first host1, and if not available host2 and following._
_After a host fail, the driver will reconnect according to this order._
