---
description: >-
  MariaDB Enterprise Kubernetes Operator 25.08 is a Stable (GA) release of
  MariaDB Enterprise Kubernetes Operator, released on 2025-10-29
---
...
If you're updating from previous versions, __please follow the [UPDATE GUIDE](https://mariadb.com/docs/tools/mariadb-enterprise-operator/updates/update-25.10)__ to ensure a safe transition.
...
### Deprecated Embedded `maxScale` Property In MariaDB Custom Resource.

The Enterprise Kubernetes Operator has deprecated the use of the embedded `maxScale` property within the `MariaDB` Custom Resource (CR) to promote modularity and declarative best practices. Users must now provision `MaxScale` using the dedicated `MaxScale` CR. This transition ensures a clean separation of concerns, allowing the database and proxy layers to be managed and configured independently as well as allow us to better fit inside Helm/etcd size limitations.

Please refer to the [migration guide](../../tools/mariadb-enterprise-operator/migrations/migrate-embedded-maxscale-to-maxscale-resource.md).
