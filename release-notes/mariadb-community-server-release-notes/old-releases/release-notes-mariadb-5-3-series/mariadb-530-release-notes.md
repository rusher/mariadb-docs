...
One of the major performance improvements in [MariaDB 5.3.0](mariadb-530-release-notes.md) is that subqueries are finally usable in practice. It is no longer necessary to rewrite subqueries manually into joins or separate queries. The optimizer of [MariaDB 5.3](broken-reference) will do this, and more automatically. EXPLAIN for derived tables and materialized views is instantaneous. Both derived tables and views can be optimized by indexes created on the fly.
...
[MariaDB 5.3](broken-reference) implements group commit which works when using XtraDB with the binary log enabled.
