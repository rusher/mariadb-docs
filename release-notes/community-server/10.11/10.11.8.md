...
* Port backup features from Enterprise Server. This adds support for [BACKUP STAGE](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/administrative-sql-statements/backup-commands/backup-stage) to [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup), and obsoletes the [no-backup-locks]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#no-backup-locks) and [rsync]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#rsync) options ([MDEV-32932](https://jira.mariadb.org/browse/MDEV-32932))
