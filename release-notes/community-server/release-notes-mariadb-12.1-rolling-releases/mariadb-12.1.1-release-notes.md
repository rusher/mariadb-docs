...
* Dump DDLs of tables/views does not work for multi-delete (n=3) if there is data to delete ([MDEV-37207](https://jira.mariadb.org/browse/MDEV-37207))
...
* Command START SLAVE UNTIL would invalidly allow some CHANGE MASTER TO options (e.g. MASTER\_USE\_GTID, MASTER\_DEMOTE\_TO\_SLAVE) despite not being valid for a START SLAVE command. ([MDEV-35635](https://jira.mariadb.org/browse/MDEV-35635))
...
* Fix ha\_innobase::check() for sequences ([MDEV-36487](https://jira.mariadb.org/browse/MDEV-36487))
