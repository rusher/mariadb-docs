...
For a list of every change made in [MariaDB 5.3.0](5.3.0.md), including the various bugs that were fixed and links to detailed information on each push, see the [MariaDB 5.3.0 Changelog](../../changelogs/changelogs-mariadb-53-series/mariadb-530-changelog.md). These changes are compared against [MariaDB 5.2](../5.2/changes-improvements-in-mariadb-5-2.md), and it is worth noting that [MariaDB 5.3](changes-improvements-in-mariadb-5-3.md) is based on the MySQL 5.1 releases.
...
A few highlights (**for a more detailed list, please see the** [**What is MariaDB 5.3**](https://github.com/mariadb-corporation/docs-server/blob/test/release-notes/mariadb-community-server-release-notes/old-releases/release-notes-mariadb-5-3-series/broken-reference/README.md) **page**) in [MariaDB 5.3.0](5.3.0.md) follow:

## Subquery optimizations

One of the major performance improvements in [MariaDB 5.3.0](5.3.0.md) is that subqueries are finally usable in practice. It is no longer necessary to rewrite subqueries manually into joins or separate queries. The optimizer of [MariaDB 5.3](changes-improvements-in-mariadb-5-3.md) will do this, and more automatically. EXPLAIN for derived tables and materialized views is instantaneous. Both derived tables and views can be optimized by indexes created on the fly.
...
NoSQL is all the rage these days and [MariaDB 5.3.0](5.3.0.md) ships with the [HandlerSocket](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-structure/nosql/handlersocket) plugin.
