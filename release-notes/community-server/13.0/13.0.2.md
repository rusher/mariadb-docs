...
<!-- TODO(DOCS-6610): base page for an UNRELEASED round (TODO-6175, blocked by MDEV-40327).
     Before publishing:
       1. Replace every TBD with the real release date (the documented date tracks the
          `mariadb-13.0.2` git tag date, as 11.8.8/12.3.2/13.0.1 all do).
       2. Add a `### General` platform-deprecation item below if this round retires any
          platform (source: about/platform-deprecation-policy.md; the Aug 2026 round had none).
       3. Add `### Security` as the LAST H3 before `## Changelog` if the round fixes CVEs.
       4. Remove `hidden: true` here AND on the changelog page together, then do the
          release-day updates listed in the DOCS-6610 handoff comment.
     New Features / Notable Items below are copied from mariadb-13.0-changes-and-improvements.md,
     which is the convention for an INITIAL GA release (cf. 12.2.2, 12.1.2) rather than the
     notable-MDEV listing used for later releases in a series. Re-copy if that page changes
     before the release. -->
