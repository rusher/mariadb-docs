...
| [read\_buffer\_size](system-variables/server-system-variables.md#read_buffer_size)        | Used by some storage engines when doing bulk insert                                                                             |
...
There are two variables that dictates how memory are allocated by MariaDB while parsing and executing a query.[query\_prealloc\_size](system-variables/server-system-variables.md#query_prealloc_size) defines the standard buffer for memory used for query execution and [query\_alloc\_block\_size](system-variables/server-system-variables.md#query_alloc_block_size) that is size of memory blocks if `query_prealloc_size` was not big enough. Getting these variables right will reduce memory fragmentation in the server.
...
From [MariaDB 10.1](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/community-server/old-releases/release-notes-mariadb-10-1-series/changes-improvements-in-mariadb-10-1), [table\_open\_cache](system-variables/server-system-variables.md#table_open_cache) defaults to 2000.

### Query Cache

Short answer: [query\_cache\_type](system-variables/server-system-variables.md#query_cache_type) = OFF and [query\_cache\_size](system-variables/server-system-variables.md#query_cache_size) = 0
...
* Yes, the mutex is hit even if [query\_cache\_type](system-variables/server-system-variables.md#query_cache_type) = DEMAND (2).
...
To see how well your QC is performing, SHOW GLOBAL STATUS LIKE 'Qc%'; then compute the read hit rate: [Qcache\_hits](system-variables/server-status-variables.md#qcache_hits) / [Qcache\_inserts](system-variables/server-status-variables.md#qcache_inserts) If it is over, say, 5, the QC might be worth keeping.

If you decide the QC is right for you, then I recommend

* [query\_cache\_size](system-variables/server-system-variables.md#query_cache_size) = no more than 50M
* [query\_cache\_type](system-variables/server-system-variables.md#query_cache_type) = DEMAND
* [SQL\_CACHE or SQL\_NO\_CACHE](buffers-caches-and-threads/query-cache.md#sql_no_cache-and-sql_cache) in all SELECTs, based on which queries are likely to benefit from caching.
