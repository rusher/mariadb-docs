...
There are two encryption key identifiers that have special meanings in MariaDB. Encryption key `<code>1</code>` is intended for encrypting system data, such as InnoDB redo logs, binary logs, and so on. It must always exist when [data-at-rest encryption](../data-at-rest-encryption-overview.md) is enabled. Encryption key `<code>2</code>` is intended for encrypting temporary data, such as temporary files and temporary tables. It is optional. If it doesn't exist, then MariaDB uses encryption key `<code>1</code>` for these purposes instead.
