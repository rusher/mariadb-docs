...
MariaDB can encrypt data in tables that use the [Aria storage engine](aria-enabling-encryption.md). This includes both user-created tables and internal on-disk temporary tables that use the Aria storage engine. This ensures that your Aria data is only accessible through MariaDB.
...
In order to enable encryption for tables using the [Aria storage engine](aria-enabling-encryption.md), there are a couple server system variables that you need to set and configure. Most users will want to set `[aria_encrypt_tables](../../../../../reference/storage-engines/aria/aria-system-variables.md#aria_encrypt_tables)` and `[encrypt_tmp_disk_tables](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#encrypt_tmp_disk_tables)`.
