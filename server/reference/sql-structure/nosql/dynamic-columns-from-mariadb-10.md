...
It is possible to refer to column by names. Names can be used everywhere where in [MariaDB 5.3](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/community-server/old-releases/release-notes-mariadb-5-3-series/changes-improvements-in-mariadb-5-3) one could use only strings:
...
[COLUMN\_CHECK](../../sql-functions/special-functions/dynamic-columns-functions/column_check.md) is used to check a column's integrity. When it encounters an error\
...
[COLUMN\_JSON](https://mariadb.com/kb/en/column-json) converts all dynamic column record content to a JSON object.
...
* All API functions have the prefix mariadb\_dyncol\_ (the old prefix dynamic\_column\_ is deprecated
...
an interface to Apache Cassandra dynamic columns. The [Cassandra engine](../../../server-usage/storage-engines/legacy-storage-engines/cassandra/cassandra-storage-engine-overview.md) may pack all columns which were not mentioned in the MariaDB interface table definition\
...
* [Cassandra Storage Engine](../../../server-usage/storage-engines/legacy-storage-engines/cassandra/cassandra-storage-engine-overview.md)
