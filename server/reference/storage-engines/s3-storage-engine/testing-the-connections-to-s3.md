...
Key [S3 variables](3-storage-engine-system-variables) are:
...
[aria_s3_copy](aria_s3_copy.md) is a tool that allows you to copy [aria](../../../security/securing-mariadb/securing-mariadb-encryption/encryption-data-at-rest-encryption/aria-encryption/aria-enabling-encryption.md) tables to and from S3. It's useful for testing the connection as it allows you to specify all s3 options on the command line.
...
One can use the [MariaDB test system](../../../clients-and-utilities/mariadb-test/mariadb-test-overview.md) to run all default S3 test against your S3 storage.
...
* [Using MinIO with mysql-test-run to test the S3 storage engine](/en/installing-minio-for-usage-with-mysql-test-run/)