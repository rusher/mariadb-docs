...
[MyISAM](../../../../clients-and-utilities/myisam-clients-and-utilities/myisamchk-table-information.md) or [InnoDB](../../../../security/securing-mariadb/securing-mariadb-encryption/encryption-data-at-rest-encryption/innodb-encryption/innodb-encryption-troubleshooting.md) do by using the PARTITION engine that must be enabled for this to work. This type of partitioning is sometimes referred as “horizontal partitioning”.
...
1. Table partitioning. Each partition is stored in a separate table like in TBL tables.
...
1. The connection string, when specified for the table, is lost by the partition engine. The workaround is to specify the connection string in the option_list.
1. [MySQL upstream bug #71095](https://bugs.mysql.com/bug.php?id=71095). In case of list columns partitioning it sometimes causes a false “impossible where” clause to be raised. This makes a wrong void result returned when it should not be void. There is no workaround but this bug should be hopefully fixed.
...
1. Unlike multiple tables, partitioned tables are not read only.
1. Unlike multiple tables, partitioned tables can be indexable.
1. The file names are generated from the partition names.
1. Query pruning is automatically made by the partition engine.
...
1. The partitions can be tables driven by engines not currently supporting partitioning.
1. Partition tables can be located on remote servers, enabling table sharding.
1. Like for TBL tables, the columns of the partition table do not necessarily match the columns
of the sub-tables.
...
The column *id* is declared as a key, and the table type is now MYSQL. This makes Sub-tables accessed by calling a MariaDB server as MYSQL tables do. Note that this modifies only the way
...
1. For each partition sub-tables, the “%s” part of the connection string has been replaced by the
partition name.
1. It is not needed anymore to define the rt1, rt2, and rt3 tables (even it does not harm) and the
