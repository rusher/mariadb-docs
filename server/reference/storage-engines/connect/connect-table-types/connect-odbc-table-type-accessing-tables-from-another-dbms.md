...
ODBC (Open Database Connectivity) is a standard API for accessing database management systems (DBMS). CONNECT uses this API to access data contained in other DBMS without having to implement a specific application for each one. An exception is the access to MySQL that should be done using the [MYSQL table type](/en/connect-table-types-mysql-table-type-accessing-mysqlmariadb-tables/).
...
1. No indexing of ODBC tables (do not specify any columns as key). However,
 ...
 source, indexing will be used by the data source if it supports it. (Remote indexing is available with version 1.04, released with [MariaDB 10.1.6](/en/mariadb-1016-release-notes/))
1. CONNECT ODBC supports [SELECT](../../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) and [INSERT](../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md). [UPDATE](../../../sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/update.md) and [DELETE](../../../sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/delete.md) are also supported
...
In CONNECT version 1.03 (until [MariaDB 10.1.5](/en/mariadb-1015-release-notes/)) ODBC tables are not indexable. Version 1.04 (from [MariaDB 10.1.6](/en/mariadb-1016-release-notes/)) adds remote indexing facility to the ODBC table type.
...
With CONNECT version 1.04 (from [MariaDB 10.1.6](/en/mariadb-1016-release-notes/)), another way to provide random access is to specify some columns to be indexed. This should be done only when the corresponding column of the source table is also indexed. This should be used for tables too large to be stored in memory and is similar to the remote indexing used by the [MYSQL table type](/en/connect-table-types-mysql-table-type-accessing-mysqlmariadb-tables/) and by the [FEDERATED engine](/en/federatedx-storage-engine/).
...
The only data modifying operations are the [INSERT](../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md) , [UPDATE](../../../sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/update.md) and [DELETE](../../../sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/delete.md) commands.
...
1. Being executed by the data source, the (rephrased) command must also respect
 the data source syntax.
1. All data referenced in the SET and WHERE clause belongs to the data source.
...
We can populate it directly using the supported [INSERT](../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md) statement:
...
1. Using SQLConnect and a Data Source Name (DSN)
...
1. When specified, the DBNAME create table option is regarded by ODBC tables as
 the SCHEMA name.
1. Table names can be specified as “cat.sch.tab” allowing to set the catalog and
 schema info.
