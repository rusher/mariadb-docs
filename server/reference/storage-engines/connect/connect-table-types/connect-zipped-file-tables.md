...
1. Zipped tables are read-only. [UPDATE](../../../sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/update.md) and [DELETE](../../../sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/delete.md) are not supported. However, [INSERT](../../../sql-statements-and-structure/sql-statements/data-manipulation/inserting-loading-data/insert.md) is supported in a specific way when making tables.
1. The inside files are decompressed into memory. Memory problems may arise with huge files.
...
1. Making the table from the data of another table. This can be done by executing an “insert into table select * from another_table” or by specifying “as select * from another_table” in the create table statement.
1. Making the table from a file whose format enables to use the “load data infile” statement.
