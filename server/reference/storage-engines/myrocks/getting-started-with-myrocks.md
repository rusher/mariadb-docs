...
Note: When installed with a package manager, an option file that contains the `[--plugin-load-add](/en/mysqld-options/#-plugin-load-add)` option may also be installed. The RPM package installs it as `/etc/my.cnf.d/rocksdb.cnf`, and the DEB package installs it as `/etc/mysql/mariadb.conf.d/rocksdb.cnf`
