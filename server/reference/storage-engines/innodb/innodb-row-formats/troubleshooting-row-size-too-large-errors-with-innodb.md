...
The root cause is that InnoDB has a maximum row size that is roughly equivalent to half of the value of the [innodb_page_size](../innodb-system-variables.md) system variable. See [InnoDB Row Formats Overview: Maximum Row Size](innodb-row-formats-overview.md#maximum-row-size) for more information.
...
The JSON document can be queried and manipulated using MariaDB's [JSON functions](../../../sql-statements-and-structure/sql-statements/built-in-functions/special-functions/json-functions/README.md).
