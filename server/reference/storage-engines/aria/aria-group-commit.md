...
Since [MariaDB 5.2](/en/what-is-mariadb-52/), the [Aria storage engine](../../../security/securing-mariadb/securing-mariadb-encryption/encryption-data-at-rest-encryption/aria-encryption/aria-enabling-encryption.md) has included a feature to group commits to speed up concurrent threads doing many inserts into the same or different Aria tables.

By default, group commit for Aria is turned off. It is controlled by the 
[aria_group_commit](/en/aria-server-system-variables/#aria_group_commit) and [aria_group_commit_interval](/en/aria-server-system-variables/#aria_group_commit_interval) system variables.
