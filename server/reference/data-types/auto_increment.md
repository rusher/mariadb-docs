...
{% hint style="info" %}
Note that when changing the `auto_increment_increment` or `auto_increment_offset` on a server, you either have to restart the server, or ensure that all current connections are killed. This is needed to to force all connections to use the new values.&#x20;
