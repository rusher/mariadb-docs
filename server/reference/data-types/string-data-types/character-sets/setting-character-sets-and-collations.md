...
The [CREATE TABLE](../../../sql-statements/data-definition/create/create-table.md) and [ALTER TABLE](<../../../sql-statements/data-definition/alter/alter-table/README (1).md>) statements support optional character set and collation clauses, a MariaDB and MySQL extension to standard SQL.
...
When using [ALTER TABLE](<../../../sql-statements/data-definition/alter/alter-table/README (1).md>) to change a column's character set, you need to ensure the character sets are compatible with your data. MariaDB will map the data as best it can, but it's possible to lose data if care is not taken.
