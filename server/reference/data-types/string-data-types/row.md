...
`ROW` is a data type for [stored procedure](/kb/en/stored-procedures/) variables.
...
* Assignment is using the `:=` operator and the [SET](../../../server-usage/replication-cluster-multi-master/standard-replication/setting-up-replication.md) command:
