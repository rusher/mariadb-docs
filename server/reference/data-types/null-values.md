...
When a table is [created](../sql-statements/data-definition/create/create-table.md) or the format [altered](<../sql-statements/data-definition/alter/alter-table/README (1).md>), columns can be specified as accepting `NULL` values, or not accepting them, with the `NULL` and `NOT NULL` clauses respectively.
