...
If the user variable `@rpl_semi_sync_slave` is set, 2 extra bytes are added after the status byte of a [binlog network stream](3-binlog-network-stream.md) and before the normal binlog event header.
