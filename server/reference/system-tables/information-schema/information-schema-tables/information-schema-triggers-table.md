...
| ACTION\_ORDER                 | Indicates the order that the action will be performed in (of the list of a table's triggers with identical `EVENT_MANIPULATION` and `ACTION_TIMING` values). See the [FOLLOWS/PRECEDES](../../../../server-usage/triggers-events/triggers/create-trigger.md#follows-precedes-other_trigger_name) clauses.        |
...
Queries to the `TRIGGERS` table return information even if the user doesn't have `SELECT` [privileges](../../../sql-statements/account-management-sql-statements/grant.md#column-privileges) on the columns. The columns `ACTION_CONDITION`, `ACTION_STATEMENT` and `DEFINER` are `NULL` unless the user has the `TRIGGER` [privilege](../../../sql-statements/account-management-sql-statements/grant.md#table-privileges). Similar information is returned by the [SHOW TRIGGERS](../../../sql-statements/administrative-sql-statements/show/show-triggers.md) statement.
