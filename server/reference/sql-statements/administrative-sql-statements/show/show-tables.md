...
The `FULL` modifier is supported such that `SHOW FULL TABLES` displays a second output column. Values for the second column, `Table_type`, are `BASE TABLE` for a table, `VIEW` for a [view](../../../../server-usage/views/) and `SEQUENCE` for a [sequence](../../../sql-statements-and-structure/sequences/).
