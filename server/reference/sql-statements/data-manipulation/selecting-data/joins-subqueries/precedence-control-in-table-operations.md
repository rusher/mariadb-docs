...
Using parentheses in your SQL allows you to control the order of execution for `[SELECT](../select.md)` statements and [Table Value Constructor](../../../../sql-statements-and-structure/sql-language-structure/table-value-constructors.md), including `[UNION](union.md)`, `[EXCEPT](except.md)`, and `[INTERSECT](intersect.md)` operations. MariaDB executes the parenthetical expression before the rest of the statement. You can then use `[ORDER BY](../order-by.md)` and `[LIMIT](../limit.md)` clauses the further organize the result-set.
