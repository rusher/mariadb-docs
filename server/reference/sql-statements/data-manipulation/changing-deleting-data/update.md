...
columns of existing rows in the named table with new values. The`SET` clause indicates which columns to modify and the values\
they should be given. Each value can be given as an expression, or the keyword`DEFAULT` to set a column explicitly to its default value. The`WHERE` clause, if given, specifies the conditions that identify\
...
specified as described in [SELECT](../selecting-data/select.md).
...
* If you use the `LOW_PRIORITY` keyword, execution of\
  ...
  locking (MyISAM, MEMORY, MERGE). See [HIGH\_PRIORITY and LOW\_PRIORITY clauses](high_priority-and-low_priority.md) for details.
