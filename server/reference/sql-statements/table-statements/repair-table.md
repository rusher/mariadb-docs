...
`REPAIR TABLE` is also supported for [partitioned tables](../../../server-usage/partitioning-tables/) with the [ALTER TABLE ... REPAIR PARTITION](<../data-definition/alter/alter-table/README (1).md>) statement. However, the `USE_FRM` option cannot be used with this statement on a partitioned table. See [Repairing Partitions](../../../server-usage/partitioning-tables/partitioning-overview.md#repairing-partitions) for details.
