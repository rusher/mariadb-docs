...
[CURRENT\_USER](../built-in-functions/secondary-functions/information-functions/current_user.md) or `CURRENT_USER()` can also be used to alter the account logged into the current session. For example, to change the current user's password to `mariadb`:
