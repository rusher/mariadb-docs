...
The `CAST()` function takes a value of one [type](/en/data-types/) and produces a value of another type, similar to the [CONVERT()](convert.md) function.
...
In [MariaDB 10.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-104) and later, you can use the `CAST()` function with the `INTERVAL` keyword.
...
Note that when one casts to [CHAR](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/query-optimizations/charset-narrowing-optimization.md) without specifying the character set, the [collation_connection](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#collation_connection) character set collation will be used. When used with `CHAR CHARACTER SET`, the default collation for that character set will be used.
...
Using `CAST()` to order an `[ENUM](../../../../data-types/string-data-types/enum.md)` field as a `[CHAR](../../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/query-optimizations/charset-narrowing-optimization.md)` rather than the internal numerical value:
...
* [Supported data types](/en/data-types/)
