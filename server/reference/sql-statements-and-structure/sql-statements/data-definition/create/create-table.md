...
separately as ``db_name`.`tbl_name``. This is particularly useful for [CREATE TABLE ... SELECT](create-table.md#create-table-select), because it allows to create a table into a database, which contains data from other databases. See [Identifier Qualifiers](../../../sql-language-structure/identifier-qualifiers.md).
...
If the table is a [sequence](../../../sequences/), then it will have the `SEQUENCE` set to `1`.
...
`CREATE TABLE` can also be used to create a [SEQUENCE](../../../sequences/). See [CREATE SEQUENCE](../../../sequences/create-sequence.md) and [Sequence Overview](../../../sequences/sequence-overview.md).
