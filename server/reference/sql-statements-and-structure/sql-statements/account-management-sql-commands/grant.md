...
* [Global privileges priv_type](#global-privileges) are granted using `*.*` for
...
procedures. Global privileges are stored in [mysql.global_priv table](/kb/en/mysqlglobal_priv-table/).
* [Database privileges priv_type](#database-privileges) are granted using `db_name.*`
...
* [Table privileges priv_type](#table-privileges) are granted using `db_name.tbl_name`
...
* [Function privileges priv_type](#function-privileges) are granted using `FUNCTION db_name.routine_name`
for priv_level, or using just `FUNCTION routine_name` to specify a function
in the default database.
* [Procedure privileges priv_type](#procedure-privileges) are granted using `PROCEDURE db_name.routine_name`
...
Enables administration of the [binary log](../../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md), including the [PURGE BINARY LOGS](../administrative-sql-statements/purge-binary-logs.md) statement and setting the system variables:
...
When a user would upgrade from an older major release to a [MariaDB 10.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-105) minor release prior to [MariaDB 10.5.9](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1059-release-notes), certain user accounts would lose capabilities. For example, a user account that had the REPLICATION CLIENT privilege in older major releases could run [SHOW REPLICA STATUS](../administrative-sql-statements/show/show-replica-status.md), but after upgrading to a [MariaDB 10.5](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-105) minor release prior to [MariaDB 10.5.9](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1059-release-notes), they could no longer run [SHOW REPLICA STATUS](../administrative-sql-statements/show/show-replica-status.md), because that statement was changed to require the REPLICATION REPLICA ADMIN privilege.

This issue is fixed in [MariaDB 10.5.9](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1059-release-notes) with this new privilege, which now grants the user the ability to execute SHOW [ALL] (SLAVE | REPLICA) STATUS.
...
Synonym for [REPLICATION SLAVE](#replication-slave). From [MariaDB 10.5.1](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1051-release-notes).
...
Permits administering replica servers, including [START REPLICA/SLAVE](../administrative-sql-statements/replication-commands/start-replica.md), [STOP REPLICA/SLAVE](../administrative-sql-statements/replication-commands/stop-replica.md), [CHANGE MASTER](../administrative-sql-statements/replication-commands/change-master-to.md), [SHOW REPLICA/SLAVE STATUS](../administrative-sql-statements/show/show-replica-status.md), [SHOW RELAYLOG EVENTS](../administrative-sql-statements/show/show-relaylog-events.md) statements, replaying the binary log with the [BINLOG](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/binlog_checkpoint_event.md) statement (generated by [mariadb-binlog](/kb/en/mysqlbinlog/)), and setting the system variables:

* [gtid_cleanup_batch_size](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_cleanup_batch_size)
* [gtid_ignore_duplicates](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_ignore_duplicates)
* [gtid_pos_auto_engines](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_pos_auto_engines)
* [gtid_slave_pos](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_slave_pos)
* [gtid_strict_mode](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_strict_mode)
...
Enables setting the `DEFINER` when creating [triggers](../../../../server-usage/programming-customizing-mariadb/triggers-events/triggers/triggers-and-implicit-locks.md), [views](/kb/en/views/), [stored functions](/kb/en/stored-functions/) and [stored procedures](/kb/en/stored-procedures/). Added in [MariaDB 10.5.2](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-105-series/mariadb-1052-release-notes).
...
| CREATE ROUTINE | Create Stored Programs using the [CREATE PROCEDURE](../../../../server-usage/programming-customizing-mariadb/stored-routines/stored-procedures/create-procedure.md) and [CREATE FUNCTION](../../../../server-usage/programming-customizing-mariadb/user-defined-functions/create-function-udf.md) statements. |
| CREATE TEMPORARY TABLES | Create temporary tables with the [CREATE TEMPORARY TABLE](../data-definition/create/create-tablespace.md) statement. This privilege enable writing and dropping those temporary tables |
...
| CREATE | Create a table using the [CREATE TABLE](../data-definition/create/create-tablespace.md) statement. You can grant the CREATE privilege on tables that do not yet exist. |
...
| INDEX | Create an index on a table using the [CREATE INDEX](../data-definition/create/create-index.md) statement. Without the INDEX privilege, you can still create indexes when creating a table using the [CREATE TABLE](../data-definition/create/create-tablespace.md) statement if the you have the CREATE privilege, and you can create indexes using the [ALTER TABLE](../data-definition/alter/alter-table.md) statement if you have the ALTER privilege. |
| INSERT | Add rows to a table using the [INSERT](../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md) statement. The INSERT privilege can also be set on individual columns; see [Column Privileges](#column-privileges) below for details. |
| REFERENCES | Unused. |
| SELECT | Read data from a table using the [SELECT](../../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) statement. The SELECT privilege can also be set on individual columns; see [Column Privileges](#column-privileges) below for details. |
...
| --- | --- |
| Privilege | Description |
| INSERT (column_list) | Add rows specifying values in columns using the [INSERT](../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md) statement. If you only have column-level INSERT privileges, you must specify the columns you are setting in the INSERT statement. All other columns will be set to their default values, or NULL. |
| REFERENCES (column_list) | Unused. |
| SELECT (column_list) | Read values in columns using the [SELECT](../../../../server-usage/replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) statement. You cannot access or query any columns for which you do not have SELECT privileges, including in WHERE, ON, GROUP BY, and ORDER BY clauses. |
| UPDATE (column_list) | Update values in columns of existing rows using the [UPDATE](../data-manipulation/changing-deleting-data/update.md) statement. UPDATE statements usually include a WHERE clause to update only certain rows. You must have SELECT privileges on the table or the appropriate columns for the WHERE clause. |
...
| EXECUTE | Execute a [stored procedure](/kb/en/stored-procedures/) using the [CALL](../stored-routine-statements/call.md) statement. The privilege to call a procedure may allow you to perform actions you wouldn't otherwise be able to do, such as insert rows into a table. |
...
The `PROXY` privilege allows one user to proxy as another user, which means their privileges change to that of the proxy user, and the [CURRENT_USER()](../built-in-functions/secondary-functions/information-functions/current_user.md) function returns the user name of the proxy user.

The `PROXY` privilege only works with authentication plugins that support it. The default [mysql_native_password](../../../plugins/authentication-plugins/authentication-plugin-mysql_native_password.md) authentication plugin does not support proxy users.
...
The default `root` user accounts created by [mariadb-install-db](../../../../clients-and-utilities/mariadb-install-db.md) have this privilege. For example:
...
The optional `IDENTIFIED BY` clause can be used to provide an account with a password. The password should be specified in plain text. It will be hashed by the [PASSWORD](../built-in-functions/secondary-functions/encryption-hashing-and-compression-functions/password.md) function prior to being stored.
...
The only [authentication plugins](/kb/en/authentication-plugins/) that this clause supports are [mysql_native_password](../../../plugins/authentication-plugins/authentication-plugin-mysql_native_password.md) and [mysql_old_password](../../../plugins/authentication-plugins/authentication-plugin-mysql_old_password.md).
...
The optional `IDENTIFIED VIA authentication_plugin` allows you to specify that the account should be authenticated by a specific [authentication plugin](/kb/en/authentication-plugins/). The plugin name must be an active authentication plugin as per [SHOW PLUGINS](../administrative-sql-statements/show/show-plugins.md). If it doesn't show up in that output, then you will need to install it with [INSTALL PLUGIN](../administrative-sql-statements/plugin-sql-statements/install-plugin.md) or [INSTALL SONAME](../administrative-sql-statements/plugin-sql-statements/install-soname.md).
...
Per account resource limits are stored in the [user](/kb/en/mysqluser-table/) table, in the [mysql](/kb/en/the-mysql-database-tables/) database. Columns used for resources limits are named `max_questions`, `max_updates`, `max_connections` (for `MAX_CONNECTIONS_PER_HOUR`), and `max_user_connections` (for `MAX_USER_CONNECTIONS`).
...
#### MariaDB starting with [10.11.0](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-101-series/mariadb-10110-release-notes)
...
* [mysql.global_priv table](/kb/en/mysqlglobal_priv-table/)
* [mysql.user table](/kb/en/mysqluser-table/)
* [Password Validation Plugins](/kb/en/password-validation-plugins/) - permits the setting of basic criteria for passwords
* [Authentication Plugins](/kb/en/authentication-plugins/) - allow various authentication methods to be used, and new ones to be developed.