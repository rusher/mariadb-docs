...
Since storage engines are plugins, different information about them is also shown in the `[information_schema.PLUGINS](/kb/en/information_schemaplugins-table/)` table and by the `[SHOW PLUGINS](show-plugins.md)` statement.
...
* `Transactions`, `XA` and `Savepoints` indicate whether [transactions](../../transactions/transactions-unlock-tables.md), [XA transactions](../../transactions/xa-transactions.md) and [transaction savepoints](../../transactions/savepoint.md) are supported by the engine.
