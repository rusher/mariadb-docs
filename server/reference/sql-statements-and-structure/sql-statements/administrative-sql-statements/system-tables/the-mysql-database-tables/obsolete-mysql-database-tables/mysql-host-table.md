...
Until [MariaDB 5.5](/kb/en/what-is-mariadb-55/), the `mysql.host` table contained information about hosts and their related privileges. When determining permissions, if a matching record in the [mysql.db table](/kb/en/mysqldb-table/) had a blank host value, the mysql.host table would be examined.
...
| Create_priv | enum('N','Y') | NO | | N | Can [CREATE TABLEs](../../../../data-definition/create/create-tablespace.md). |
...
| Lock_tables_priv | enum('N','Y') | NO | | N | Acquire explicit locks using the [LOCK TABLES](/kb/en/transactions-lock/) statement; user also needs to have the SELECT privilege on a table in order to lock it. |
...
| Execute_priv | enum('N','Y') | NO | | N | Can execute [stored procedure](/kb/en/stored-procedures/) or functions. |
