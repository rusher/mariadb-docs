...
`<code>mysql.help_category</code>` is one of the four tables used by the [HELP command](../../help-command.md). It is populated when the server is installed by the `<code>fill_help_tables.sql</code>` script. The other help tables are [help_relation](mysql-help_relation-table.md), [help_topic](mysql-help_topic-table.md) and [help_keyword](mysql-help_keyword-table.md).
...
The `<code>mysql.help_category</code>` table contains the following fields:
