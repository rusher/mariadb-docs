...
The `<code>io_global_by_wait_by_bytes</code>` and `<code>x$io_global_by_wait_by_bytes</code>` views summarize global I/O consumers, displaying amount of I/O and time waiting for I/O, grouped by event. Rows are sorted by descending total I/O (bytes read and written) by default.


The `<code>io_global_by_wait_by_bytes</code>` view is intended to be easier for human reading, while the `<code>x$io_global_by_wait_by_bytes</code>` view provides the data in raw form, intended for tools that process the data.
