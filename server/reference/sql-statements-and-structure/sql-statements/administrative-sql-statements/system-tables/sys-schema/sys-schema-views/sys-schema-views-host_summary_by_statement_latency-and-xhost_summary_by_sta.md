...
The `<code>host_summary_by_statement_latency</code>` and `<code>x$host_summary_by_statement_latency</code>` views summarize statement statistics, grouped by host. Rows are sorted by descending total latency by default. The `<code>host_summary_by_statement_latency</code>` view is intended to be easier for human reading, while the `<code>x$host_summary_by_statement_latency</code>` view provides the data in raw form, intended for tools that process the data.
