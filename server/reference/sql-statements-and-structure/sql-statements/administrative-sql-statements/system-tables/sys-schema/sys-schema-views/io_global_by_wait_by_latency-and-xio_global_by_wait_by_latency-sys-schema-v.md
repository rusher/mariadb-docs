...
The `<code>io_global_by_wait_by_latency</code>` and `<code>x$io_global_by_wait_by_latency</code>` views summarize global I/O consumers, displaying I/O and time waiting for I/O, grouped by event. Rows are sorted by descending total latency by default.


The `<code>io_global_by_wait_by_latency</code>` view is intended to be easier for human reading, while the `<code>x$io_global_by_wait_by_latency</code>` view provides the data in raw form, intended for tools that process the data.
