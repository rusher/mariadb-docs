...
This statement removes a single installed [plugin](/en/mariadb-plugins/). To uninstall the whole library which contains the plugin, use [UNINSTALL SONAME](uninstall-soname.md). You cannot uninstall a plugin if any table that uses it is open.

`plugin_name` must be the name of some plugin that is listed
in the [mysql.plugin](/en/mysqlplugin-table/) table. The server executes the plugin's deinitialization
...
[DELETE](../../account-management-sql-commands/grant.md) privilege for the [mysql.plugin](/en/mysqlplugin-table/) table.
