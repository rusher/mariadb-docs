...
The `DELAYED` option for the `[INSERT](../../../../../server-usage/programming-customizing-mariadb/views/inserting-and-updating-with-views.md)`
...
* `INSERT DELAYED` works only with [MyISAM](/en/myisam-storage-engine/), [MEMORY](../../../../storage-engines/memory-storage-engine.md), [ARCHIVE](/en/archive/),
...
* `DELAYED` is ignored on slave replication servers, so that 
 ...
 the master. `INSERT DELAYED` statements are not [safe for replication](/en/unsafe-statements-for-replication/).
...
* `INSERT DELAYED` is not supported for [partitioned tables](/en/managing-mariadb-partitioning/).
* `INSERT DELAYED` is not supported within [stored programs](/en/stored-programs-and-views/).
