...
[table_options](../vectors/create-table-with-vectors.md#table-options)
```

The options for `<code>CREATE SEQUENCE</code>` can be given in any order, optionally followed by `<code>table_options</code>`.


*table_options* can be any of the normal table options in [CREATE TABLE](../vectors/create-table-with-vectors.md) but the most usable ones are `<code>ENGINE=...</code>` and `<code>COMMENT=</code>`.


`<code>NOMAXVALUE</code>` and `<code>NOMINVALUE</code>` are there to allow one to create SEQUENCEs using the Oracle syntax.
...
values when called with `<code>NEXT VALUE FOR sequence_name</code>`. It's an alternative to [AUTO INCREMENT](../../storage-engines/innodb/auto_increment-handling-in-innodb.md) when one wants to have more control of how the numbers are generated. As the SEQUENCE caches values (up to `<code>CACHE</code>`) it can in some cases be much faster than [AUTO INCREMENT](../../storage-engines/innodb/auto_increment-handling-in-innodb.md). Another benefit is that one can access the last value generated by all used sequences, which solves one of the limitations with [LAST_INSERT_ID()](../sql-statements/built-in-functions/secondary-functions/information-functions/last_insert_id.md).
...
int type, i.e. one of [TINYINT](../../data-types/data-types-numeric-data-types/tinyint.md), [SMALLINT](../../data-types/data-types-numeric-data-types/smallint.md), [MEDIUMINT](../../data-types/data-types-numeric-data-types/mediumint.md), [INT](../../../../general-resources/learning-and-training/video-presentations-and-screencasts/interviews-related-to-mariadb.md), [INTEGER](../../data-types/data-types-numeric-data-types/integer.md), [BIGINT](../../data-types/data-types-numeric-data-types/bigint.md). Can be signed or unsigned. Maximum value is based on the data type. The use of BIGINT UNSIGNED with this option extends the possible maximum value from 9223372036854775806 to 18446744073709551614. Default BIGINT. From [MariaDB 11.5](../../../../release-notes/mariadb-community-server/what-is-mariadb-115.md).


#### INCREMENT


Increment to use for values. May be negative. Setting an increment of 0 causes the sequence to use the value of the [auto_increment_increment](../../../server-usage/replication-cluster-multi-master/standard-replication/replication-and-binary-log-system-variables.md) system variable at the time of creation, which is always a positive number. (see [MDEV-16035](https://jira.mariadb.org/browse/MDEV-16035)). Default 1.


#### MINVALUE


Minimum value for the sequence. From [MariaDB 11.5](../../../../release-notes/mariadb-community-server/what-is-mariadb-115.md), parser permits much smaller numbers, such as -9999999999999999999999999999, but converts to the minimum permitted for the int type, with a note. Default 1 if INCREMENT > 0 and -9223372036854775807 (or based on int type) if INCREMENT < 0.
...
|   |
| --- |
| Max value for sequence. From [MariaDB 11.5](../../../../release-notes/mariadb-community-server/what-is-mariadb-115.md), parser permits much larger numbers, such as 9999999999999999999999999999 used in Oracle examples, but converts to the maximum permitted for the int type, with a note. Default 9223372036854775806 (or based on int type) if INCREMENT > 0 and -1 if INCREMENT < 0. |
...
* MAXVALUE > MINVALUE
* START >= MINVALUE
* MAXVALUE <= 9223372036854775806 (LONGLONG_MAX-1). From [MariaDB 11.5](../../../../release-notes/mariadb-community-server/what-is-mariadb-115.md), the parser accepts values beyond this, and converts based on the int type.
* MINVALUE >= -9223372036854775807 (LONGLONG_MIN+1). From [MariaDB 11.5](../../../../release-notes/mariadb-community-server/what-is-mariadb-115.md), the parser accepts values beyond this, and converts based on the int type.
...
[MariaDB 10.6.1](../../../../release-notes/mariadb-community-server/release-notes-mariadb-10-6-series/mariadb-1061-release-notes.md) supports [Atomic DDL](../sql-statements/data-definition/atomic-ddl.md) and `<code>CREATE SEQUENCE</code>` is atomic.
...
From [MariaDB 11.5](../../../../release-notes/mariadb-community-server/what-is-mariadb-115.md):
...
One can use sequences instead of `<code>AUTO_INCREMENT</code>` to generate values for a table
