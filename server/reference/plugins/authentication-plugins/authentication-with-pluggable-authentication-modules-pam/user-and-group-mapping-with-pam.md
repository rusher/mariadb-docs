...
On RHEL, CentOS, and other similar Linux distributions that use [RPM packages](/en/rpm/), you need to install `gcc`, `pam-devel` and `MariaDB-devel`. For example:
...
Note that the `''@'%'` account is a special catch-all [anonymous account](../../../sql-statements-and-structure/sql-statements/account-management-sql-commands/create-user.md#anonymous-accounts). Any login by a user that has no more specific account match in the system will be matched by this anonymous account.
...
In case any user mapping is performed, the original user name is returned by the SQL function `[USER()](../../../../security/user-account-management/user-password-expiry.md)`, while the authenticated user name is returned by the SQL function `[CURRENT_USER()](../../../sql-statements-and-structure/sql-statements/built-in-functions/secondary-functions/information-functions/current_user.md)`. The latter actually defines what privileges are available to a connected user.
