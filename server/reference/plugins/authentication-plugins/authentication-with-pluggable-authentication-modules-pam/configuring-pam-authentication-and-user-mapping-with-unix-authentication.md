...
On RHEL, CentOS, and other similar Linux distributions that use [RPM packages](/en/rpm/), we need to install `gcc` and `pam-devel`:
...
The [server needs to be restarted](/en/starting-and-stopping-mariadb-starting-and-stopping-mariadb/) for this change to take affect.
