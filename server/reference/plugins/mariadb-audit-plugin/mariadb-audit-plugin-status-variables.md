...
There are a few status variables related to the [MariaDB Audit Plugin](/en/server_audit-mariadb-audit-plugin/), once it has been [installed](/en/mariadb-audit-plugin-entitymdashentity-installation/). These variables can be displayed using the [SHOW STATUS](../../sql-statements-and-structure/sql-statements/administrative-sql-statements/show/show-status.md) statement like so:
