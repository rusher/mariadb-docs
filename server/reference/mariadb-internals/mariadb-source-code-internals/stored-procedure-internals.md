...
There are three structures involved in the execution of a query which are of
 interest to the [stored procedure](../../../server-usage/programming-customizing-mariadb/stored-routines/stored-procedures/README.md) implementation:
...
1. Upon first CALL, read from the database, parse it, and cache the
 resulting Lex in memory. This time we can do more error checking.
1. Upon subsequent CALLs, use the cached Lex.
...
1. Create a new runtime context. The information about the required size
 is in sp_head's parse time context.
1. Push each parameter (from the CALL's Lex->value_list) to the new
 context. If it's an OUT or INOUT parameter, the parameter's offset
 in the caller's frame is set in the new context as well.
1. For each instruction, call its execute() method.
 The result is a pointer to the next instruction to execute (or NULL)
 if an error occurred.
1. On success, set the new values of the OUT and INOUT parameters in
 the caller's frame.
...
1. At each DROP or ALTER, increase the counter by one.
1. Each cache has its own copy of the counter, copied at the last read.
1. When looking up a name in the cache, first check if the global counter
