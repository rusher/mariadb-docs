...
In a [sequence](../../../../sql-structure/sequences/), once the MAXVALUE has been reached, and the [CYCLE](../../../../sql-structure/sequences/create-sequence.md#cycle-nocycle) argument has not been used, further attempts to set the next value will fail:
...
The underlying table is only updated with a new sequence value the first time the value is created, and then each time the [cache](../../../../sql-structure/sequences/create-sequence.md#cache-nocache) expires. By default the cache is set to `1000`. For example:
