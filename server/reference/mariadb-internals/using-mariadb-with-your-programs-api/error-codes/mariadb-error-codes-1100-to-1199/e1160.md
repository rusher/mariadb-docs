...
* `<code>5461</code>` is the connection id that got the warning/error.
* `<code>fd: 406</code>` 406 is the file descriptor that had a problem.
* `<code>state: 2</code>`
* `<code>length: 34</code>` is how many characters the server tried to wrote to the file.
* `<code>errno 32</code>` is the system error code (more below).
* `<code>vio_errno: 1160</code>` is our internal error code from the vio library, which stands for ER_NET_WRITE_ERROR "Got an error writing communication packets".
