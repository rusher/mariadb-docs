...
The error was picked up at the end of the `<code>DECLARE x TINYINT;</code>` line, guiding one to the delimiter found there. The solution is to specify a distinct delimiter for the duration of the process, using the DELIMITER command, for example:
...
The error was picked up after the `<code>accessible</code>` identifier, leading one to suspect that it's causing the problem. And indeed, it's a [reserved word](../../../../sql-statements-and-structure/sql-language-structure/reserved-words.md). There are a number of possible solutions.
...
The error message directs one to the identifier `<code>t</code>`. Double quotes cannot be used to [quote identifiers](../../../../sql-statements-and-structure/sql-language-structure/identifier-names.md#quote-character) unless ANSI_MODE is set, which in this case it isn't:
...
[Oracle mode](../../../../../../release-notes/mariadb-community-server/compatibility-and-differences/sql_modeoracle.md) in particular has a large number of differences to the default [SQL_MODE](../../../../../server-management/variables-and-modes/sql-mode.md), for example:
...
See [Oracle mode](../../../../../../release-notes/mariadb-community-server/compatibility-and-differences/sql_modeoracle.md) for a full list of syntax differences.
