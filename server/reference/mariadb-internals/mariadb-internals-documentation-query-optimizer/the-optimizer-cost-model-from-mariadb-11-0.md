...
* If using [ANALYZE TABLE](../../sql-statements/table-statements/analyze-table.md) made your plans worse (or not good enough).
...
* To keep things fast, engine-specific costs are stored in the table definition (TABLE\_SHARE). One effect of this is that if one changes the cost for an engine, it will only take effect when new, not previously cached tables are accessed. You can use [FLUSH TABLES](../../sql-statements/administrative-sql-statements/flush-commands/flush.md) to force the table to use the new costs at next access.
...
* User changes to engine costs are stored in the data pointed to by `handlerton->costs`. This is why [FLUSH TABLES](../../sql-statements/administrative-sql-statements/flush-commands/flush.md) is needed to activate new engine costs.
