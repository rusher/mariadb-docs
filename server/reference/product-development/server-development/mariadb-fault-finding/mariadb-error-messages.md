...
The `perror 11` is 'Resource temporarily unavailable'. This is an indication that an operating system lock couldn't be obtained on the ariia\_log\_control file. This is a strong indication that an existing MariaDB server is already running on the datadir.
...
* Error 28 - ENOSPC - no space left on device. The filesystem where writing is occurring is full and cannot fit any more data.
...
MariaDB cannot startup on MySQL-8.0 data directory. MySQL should be run on the MySQL data directory and if the objective is to migrate, take logical dumps from MySQL before recreating a MariaDB database, perhaps in a different location, or after the MySQL data directory has been moved.
