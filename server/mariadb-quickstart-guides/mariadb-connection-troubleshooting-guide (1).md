...
or can connect to a particular database, but not another, for example`mariadb -uname -p -u name db1` works but not `mariadb -uname -p -u name db2`, you have not been granted permission for the particular database. See the [GRANT](../reference/sql-statements-and-structure/sql-statements/account-management-sql-commands/grant.md) article.

#### Option Files and Environment Variables

It's possible that option files or environment variables may be providing incorrect connection parameters. Check the values provided in any option files read by the client you are using (see [mariadbd Configuration Files and Groups](../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md) and the documentation for the particular client you're using - see [Clients and Utilities](../clients-and-utilities/)).
