...
| --add-locks | Add locks around [INSERT](../../reference/sql-statements-and-structure/sql-statements/built-in-functions/string-functions/insert-function.md) statements, which results in faster inserts when the dump file is reloaded. Use --skip-add-locks to disable. |
...
| -c, --complete-insert | Use complete [INSERT](../../reference/sql-statements-and-structure/sql-statements/built-in-functions/string-functions/insert-function.md) statements that include column names. |
...
| --delayed-insert | Insert rows with [INSERT DELAYED](../../reference/sql-statements-and-structure/sql-statements/data-manipulation/inserting-loading-data/insert-delayed.md) instead of [INSERT](../../reference/sql-statements-and-structure/sql-statements/built-in-functions/string-functions/insert-function.md). |
...
| --hex-blob | Dump binary strings in hexadecimal format (for example, ´abc´ becomes 0x616263). The affected data types are [BINARY](../../reference/storage-engines/innodb/binary-log-group-commit-and-innodb-flushing-performance.md), [VARBINARY](../../reference/data-types/string-data-types/varbinary.md), the [BLOB](../../reference/data-types/string-data-types/blob.md) types, and [BIT](../../reference/sql-statements-and-structure/temporal-tables/bitemporal-tables.md). |
...
| --no-autocommit | Enclose the [INSERT](../../reference/sql-statements-and-structure/sql-statements/built-in-functions/string-functions/insert-function.md) statements for each dumped table within [SET autocommit = 0](../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#autocommit) and [COMMIT](../../reference/sql-statements-and-structure/sql-statements/transactions/commit.md) statements. On by default from [MariaDB 11.8](../../../release-notes/mariadb-community-server/what-is-mariadb-118.md) to allow faster data loading by InnoDB, writing only one undo log for the whole operation. |
...
| -d, --no-data | Do not write any table row information (that is, do not dump table contents). This is useful if you want to dump only the [CREATE TABLE](../../reference/sql-statements-and-structure/vectors/create-table-with-vectors.md) statement for the table (for example, to create an empty copy of the table by loading the dump file). See also --ignore-table-data . |
| --no-data-med | Do not dump rows for engines that manage external data (i.e. [MRG_MyISAM](../../reference/storage-engines/merge.md), MRG_ISAM, [CONNECT](../../../connectors/mariadb-connector-nodejs/connector-nodejs-pipelining.md), [OQGRAPH](../../reference/storage-engines/oqgraph-storage-engine/README.md), [Spider](../../reference/storage-engines/spider/spider-functions/spider_copy_tables.md), VP, [Federated](../../reference/storage-engines/federatedx-storage-engine/README.md)). This option is enabled by default. If you want to dump data for these engines, then you would need to set --no-data-med=0. |
...
| --order-by-primary | Sorts each table's rows by primary key, or first unique key, if such a key exists. This is useful when dumping a [MyISAM](../../reference/storage-engines/myisam-storage-engine/myisam-system-variables.md) table to be loaded into an [InnoDB](../../../general-resources/learning-and-training/training-and-tutorials/advanced-mariadb-articles/development-articles/quality/innodb-upgrade-tests/README.md) table, but will make the dump itself take considerably longer. |
...
| --replace | Use [REPLACE INTO](../../reference/sql-statements-and-structure/sql-statements/built-in-functions/string-functions/replace-function.md) statements instead of [INSERT INTO](../../reference/sql-statements-and-structure/sql-statements/built-in-functions/string-functions/insert-function.md) statements. |
...
it is often useful to rotate the [binary log](../../reference/storage-engines/innodb/binary-log-group-commit-and-innodb-flushing-performance.md) or at least know the binary log coordinates to which the dump corresponds:
