...
In this field, you may provide any [configuration option](../../server-management/getting-installing-and-upgrading-mariadb/starting-and-stopping-mariadb/mariadbd-options.md) or [system variable](../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md) supported by MariaDB.
...
In the case of `MariaDB`, it is recommended to set the `innodb_buffer_pool_size` system variable to a value that is 70-80% of the available memory. This can be done via the [myCnf field](#mycnf):
