...
| replicaThreads integer | ReplicaThreads is the number of replica threads used to apply Galera write sets in parallel.More info: [galera-cluster-system-variables.md#wsrep_slave_threads](../../server-usage/replication-cluster-multi-master/galera-cluster/galera-cluster-system-variables.md#wsrep_slave_threads). |  |  |
| providerOptions object (keys:string, values:string) | ProviderOptions is map of Galera configuration parameters.More info: [galera-cluster-system-variables.md#wsrep_provider_options](../../server-usage/replication-cluster-multi-master/galera-cluster/galera-cluster-system-variables.md#wsrep_provider_options). |  |  |
