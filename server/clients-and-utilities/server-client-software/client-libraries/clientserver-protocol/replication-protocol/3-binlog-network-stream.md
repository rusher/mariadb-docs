...
Binlog Network streams are requested with COM\_BINLOG\_DUMP and each Binlog Event is prepended with a status byte.\
The data sent over network is then MariaDB network protocol (4 bytes) + 1 byte status flag + event data.
...
**Note**: due to the 1 byte status flag the effective data payload is event\_size + 1\
...
### Example of an Event Transmission [HEARTBEAT\_LOG\_EVENT](heartbeat_log_event.md)
