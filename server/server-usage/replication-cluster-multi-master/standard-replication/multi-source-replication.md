...
![multi_source_replication_small](/kb/en/multi-source-replication/+image/multi_source_replication_small "multi_source_replication_small")
...
[default_master_connection](/kb/en/replication-and-binary-log-server-system-variables/#default_master_connection) to the connection you want to work with.
...
* `[CHANGE MASTER ['connection_name'] TO ...](../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md)`. This creates or modifies a connection to a primary.
...
* `[RESET SLAVE ['connection_name'] [ALL](/kb/en/reset-slave/)]`. This is used to reset a replica's replication position or to remove a replica permanently.
...
* `[SHOW SLAVE ['connection_name'] STATUS](/kb/en/show-slave-status/)`
* `[SHOW ALL SLAVES STATUS](/kb/en/show-slave-status/)`
* `[START SLAVE ['connection_name'](/kb/en/start-slave/)...]]`
* `[START ALL SLAVES ...](/kb/en/start-slave/)`
* `[STOP SLAVE ['connection_name'] ...](/kb/en/stop-slave/)`
* `[STOP ALL SLAVES ...](/kb/en/stop-slave/)`

The original old-style connection is an empty string `''`.
You don't have to use this connection if you don't want to.

You create new primary connections with [CHANGE MASTER](../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md).
...
| --- | --- | --- |
| Type | Name | Description |
| Variable | [max_relay_log_size](/kb/en/replication-and-binary-log-server-system-variables/#max_relay_log_size) | Max size of relay log. Is set at startup to max_binlog_size if 0 |
| Variable | [replicate_do_db](/kb/en/replication-and-binary-log-server-system-variables/#replicate_do_db) | Tell the replica to restrict replication to updates of tables whose names appear in the comma-separated list. For statement-based replication, only the default database (that is, the one selected by USE) is considered, not any explicitly mentioned tables in the query. For row-based replication, the actual names of table(s) being updated are checked. |
| Variable | [replicate_do_table](/kb/en/replication-and-binary-log-server-system-variables/#replicate_do_table) | Tells the replica to restrict replication to tables in the comma-separated list |
| Variable | [replicate_ignore_db](/kb/en/replication-and-binary-log-server-system-variables/#replicate_ignore_db) | Tell the replica to restrict replication to updates of tables whose names do not appear in the comma-separated list. For statement-based replication, only the default database (that is, the one selected by USE) is considered, not any explicitly mentioned tables in the query. For row-based replication, the actual names of table(s) being updated are checked. |
| Variable | [replicate_ignore_table](/kb/en/replication-and-binary-log-server-system-variables/#replicate_ignore_table) | Tells the replica thread to not replicate any statement that updates the specified table, even if any other tables might be updated by the same statement. |
| Variable | [replicate_rewrite_db](replication-and-binary-log-system-variables.md#replicate_rewrite_db) | From [MariaDB 10.11](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-1011). Allows one to configure a replica to rewrite database names. It uses the format primary_database->replica_database. If a replica encounters a binary log event in which the default database (i.e. the one selected by the USE statement) is primary_database, then the replica will apply the event in replica_database instead. |
| Variable | [replicate_wild_do_table](/kb/en/replication-and-binary-log-server-system-variables/#replicate_wild_do_table) | Tells the replica thread to restrict replication to statements where any of the updated tables match the specified database and table name patterns. |
| Variable | [replicate_wild_ignore_table](/kb/en/replication-and-binary-log-server-system-variables/#replicate_wild_ignore_table) | Tells the replica thread to not replicate to the tables that match the given wildcard pattern. |
...
| Variable | [Sql_slave_skip_counter](/kb/en/replication-and-binary-log-server-system-variables/#sql_slave_skip_counter) | How many entries in the replication log that should be skipped (mainly used in case of errors in the log). |
...
If you need to set [gtid_slave_pos](https://mariadb.com/kb/en/global-transaction-id/#gtid_slave_pos) you need to set this for all primaries at the same time.
...
| --- | --- |
| Name | Description |
| [Com_start_all_slaves](/kb/en/replication-and-binary-log-server-status-variables/#com_start_all_slaves) | Number of executed START ALL SLAVES commands. |
| [Com_start_slave](/kb/en/replication-and-binary-log-server-status-variables/#com_start_slave) | Number of executed START SLAVE commands. This replaces Com_slave_start. |
| [Com_stop_slave](/kb/en/replication-and-binary-log-server-status-variables/#com_stop_slave) | Number of executed STOP SLAVE commands. This replaces Com_slave_stop. |
| [Com_stop_all_slaves](/kb/en/replication-and-binary-log-server-status-variables/#com_stop_all_slaves) | Number of executed STOP ALL SLAVES commands. |

`[SHOW ALL SLAVES STATUS](/kb/en/show-slave-status/)` has the following new columns:
...
* The output of [SHOW ALL SLAVES STATUS](/kb/en/show-slave-status/) has one more column than `SHOW SLAVE STATUS`, since it includes the `connection_name` column.
* `[RESET SLAVE](/kb/en/reset-slave/)` now deletes all relay-log files.
...
* One can change [max_relay_log_size](/kb/en/replication-and-binary-log-server-system-variables/#max_relay_log_size) for any active connection, but new connections will always use the server startup value for `max_relay_log_size`, which can't be changed at runtime.
