...
API. At the time of writing, the supporting engines are [XtraDB](/en/xtradb-and-innodb/) and [PBXT](pbxt-storage-engine). In
...
(In [MariaDB 5.2](/en/what-is-mariadb-52/) and earlier, and MySQL at least up to 5.5, `START TRANSACTION
...
transactions have been committed since the snapshot was taken (`SHOW MASTER
STATUS` always shows the position of the last committed transaction). This works for
...
this is loaded on a new slave server, the binlog position is used in a `CHANGE
...
The [mariadb-dump](../../../clients-and-utilities/mariadb-dumpslow.md) program was extended to use these status
...
5.2](/en/what-is-mariadb-52/)-rpl, 5.3, or higher). In other cases, it will fall back to the old
