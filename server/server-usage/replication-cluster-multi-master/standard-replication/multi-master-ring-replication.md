...
![](/kb/en/multi-master-ring-replication/+image/multi-master-ring-replication1)

One should [setup replication](setting-up-replication.md) on each master like one does in [standard MariaDB replication](/kb/en/standard-replication/). The replication setup among the masters should be a ring. In other words, each master should replicate to one other master and each master should only have one other master as a slave.
...
* Use [global transaction id](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) (as described above)
* Give each master a unique [gtid_domain_id](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_domain_id). This will allow replication to apply transactions from a different master in parallel independent from other masters.

Add the following into your [my.cnf](/kb/en/configuring-mariadb-with-mycnf/) file for **all masters** and restart the servers.
...
* Check if there are any events on the old master that have not been sent to the slave. You can use [mariadb-binlog](../../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md) to read the old master binary log files and apply them to the slave.
...
* Enable the option [--gtid-ignore-duplicates](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_ignore_duplicates) on the servers.
...
The most common way to solve issues is to skip the conflicting log events with [SET GLOBAL SQL_SLAVE_SKIP_COUNTER](../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/set-global-sql_slave_skip_counter.md).
...
* Use [SET GLOBAL SQL_SLAVE_SKIP_COUNTER](../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/set-global-sql_slave_skip_counter.md) to skip the error.
...
![](/kb/en/multi-master-ring-replication/+image/multi-master-ring-replication2)
...
* Promote slave1 as the new master3. Applications should now be moved to use master3. Note [MaxScale](https://app.gitbook.com/s/0pSbu5DcMSW4KwAkUcmX/maxscale-troubleshooting) can do this step automatically.
...
* For non-semi-synchronous setups, one can use option [CHANGE MASTER TO MASTER_DEMOTE_TO_SLAVE=1](../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md#master_demote_to_slave) (requires [MariaDB 10.11](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-1011) or higher).
