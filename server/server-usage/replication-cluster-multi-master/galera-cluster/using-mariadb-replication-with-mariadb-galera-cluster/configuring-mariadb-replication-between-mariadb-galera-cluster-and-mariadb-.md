...
[MariaDB replication](/en/high-availability-performance-tuning-mariadb-replication/) can be used to replicate between [MariaDB Galera Cluster](../galera-cluster-status-variables.md) and MariaDB Server. This article will discuss how to do that.
...
If you want to use [GTID](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) replication, then you also need to configure some things to [enable wsrep GTID mode](using-mariadb-gtids-with-mariadb-galera-cluster.md#enabling-wsrep-gtid-mode). For example:

* `[wsrep_gtid_mode=ON](../galera-cluster-system-variables.md#wsrep_gtid_mode)` needs to be set on all nodes in the cluster.

* `[wsrep_gtid_domain_id](../galera-cluster-system-variables.md#wsrep_gtid_domain_id)` needs to be set to the same value on all nodes in the cluster, so that each cluster node uses the same domain when assigning [GTIDs](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) for Galera Cluster's write sets.
...
* `[log_bin](/en/replication-and-binary-log-server-system-variables/#log_bin)` needs to be set to the same path on all nodes in the cluster. See [MDEV-9856](https://jira.mariadb.org/browse/MDEV-9856) about that.
...
* Set [gtid_domain_id](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_domain_id) to a value that is different than the [wsrep_gtid_domain_id](../galera-cluster-system-variables.md#wsrep_gtid_domain_id) and [gtid_domain_id](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_domain_id) values that the cluster nodes are using.

* Set [log_bin](/en/replication-and-binary-log-server-system-variables/#log_bin) and [log_slave_updates=ON](../../standard-replication/replication-and-binary-log-system-variables.md#log_slave_updates) if you want the replica to log the transactions that it replicates.

#

# Setting up Replication

Our process to set up replication is going to be similar to the process described at [Setting up a Replication Slave with Mariabackup](/en/setting-up-a-replication-slave-with-mariabackup/), but it will be modified a bit to work in this context.

#

## Start the Cluster

The very first step is to start the nodes in the first cluster. The first node will have to be [bootstrapped](../getting-started-with-mariadb-galera-cluster.md#bootstrapping-a-new-cluster). The other nodes can be [started normally](/en/starting-and-stopping-mariadb-starting-and-stopping-mariadb/).
...
Now that the backup has been restored to the MariaDB Server replica, you can [start the MariaDB Server process](/en/starting-and-stopping-mariadb-starting-and-stopping-mariadb/).
...
Mariabackup dumps replication coordinates in two forms: [GTID strings](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) and [binary log](../../../programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) file and position coordinates, like the ones you would normally see from [SHOW MASTER STATUS](/en/show-master-status/) output. In this case, it is probably better to use the [GTID](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) coordinates.
...
Regardless of the coordinates you use, you will have to set up the primary connection using [CHANGE MASTER TO](../../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md) and then start the replication threads with [START SLAVE](/en/start-slave/).

#

### GTIDs

If you want to use GTIDs, then you will have to first set [gtid_slave_pos](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_slave_pos) to the [GTID](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) coordinates that we pulled from the [xtrabackup_binlog_info](../../../../server-management/backing-up-and-restoring-databases/mariabackup/files-created-by-mariabackup.md#xtrabackup_binlog_info) file, and we would set `MASTER_USE_GTID=slave_pos` in the [CHANGE MASTER TO](../../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md) command. For example:
...
You should be done setting up the replica now, so you should check its status with [SHOW SLAVE STATUS](/en/show-slave-status/). For example:
...
How this is done would depend on whether you want to use the [GTID](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md) coordinates or the [binary log](../../../programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) file and position coordinates.
...
To get the GTID coordinates on the MariaDB Server you can check `[gtid_current_pos](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_current_pos)` by executing:

```
SHOW GLOBAL VARIABLES LIKE 'gtid_current_pos';
```

Then on the node acting as replica in the cluster, you can set up replication by setting [gtid_slave_pos](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#gtid_slave_pos) to the GTID that was returned and then executing [CHANGE MASTER TO](../../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md):
...
To get the [binary log](../../../programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) file and position coordinates on the MariaDB Server, you can execute [SHOW MASTER STATUS](/en/show-master-status/):

```
SHOW MASTER STATUS
```

Then on the node acting as replica in the cluster, you would set `master_log_file` and `master_log_pos` in the [CHANGE MASTER TO](../../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/replication-commands/change-master-to.md) command. For example:
...
You should be done setting up the circular replication on the node in the first cluster now, so you should check its status with [SHOW SLAVE STATUS](/en/show-slave-status/). For example:
