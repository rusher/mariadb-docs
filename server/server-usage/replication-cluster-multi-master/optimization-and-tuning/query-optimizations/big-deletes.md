...
* [MyISAM](../../../../clients-and-utilities/myisam-clients-and-utilities/myisamchk-table-information.md) will lock the table during the entire operation, thereby nothing else can be done with the table.
* [InnoDB](../../../../security/securing-mariadb/securing-mariadb-encryption/encryption-data-at-rest-encryption/innodb-encryption/innodb-encryption-troubleshooting.md) won't lock the table, but it will chew up a lot of resources, leading to sluggishness.
* InnoDB has to write the undo information to its transaction logs; this significantly increases the I/O required.
* [Replication](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/mariadb-releases/compatibility-differences/replication-compatibility-between-mariadb-and-mysql), being asynchronous, will effectively be delayed (on Slaves) while the DELETE is running.
