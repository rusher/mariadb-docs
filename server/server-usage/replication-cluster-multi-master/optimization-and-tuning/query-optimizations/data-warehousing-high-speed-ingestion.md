...
Since we are probably talking about a billion-row table, shrinking the width of the Fact table by normalizing (as mentioned here). Changing an [INT](../../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/intvar_event.md) to a [MEDIUMINT](../../../../reference/data-types/data-types-numeric-data-types/mediumint.md) will save a GB. Replacing a string by an id (normalizing) saves many GB. This helps disk space and cacheability, hence speed.
...
The companion [Data Warehouse article](data-warehousing-techniques.md) folds SQL #2 into the INSERT INTO Fact. But you may need host_id for further normalization steps and/or Summarization steps, so this explicit UPDATE shown here is often better.
...
`Fact` table -- [InnoDB](../../../../security/securing-mariadb/securing-mariadb-encryption/encryption-data-at-rest-encryption/innodb-encryption/innodb-encryption-troubleshooting.md), if for no other reason than that a system crash would not need a REPAIR TABLE. (REPAIRing a billion-row [MyISAM](../../../../clients-and-utilities/myisam-clients-and-utilities/myisamchk-table-information.md) table can take hours or days.)
...
This is mostly covered here: [Summary Tables](data-warehousing-summary-tables.md)
...
* [companion Data Warehouse blog](https://mariadb.com/kb/en/mariadb/data-warehousing-techniques/)
* [companion Summary Table blog](https://mariadb.com/kb/en/mariadb/data-warehousing-summary-tables/)
