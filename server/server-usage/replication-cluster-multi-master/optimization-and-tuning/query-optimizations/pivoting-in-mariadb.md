...
The best solution is probably to do it in some form of client code (PHP, etc). MySQL and MariaDB do not have a syntax for SELECT that will do the work for you. The code provided here uses a [stored procedure](/en/stored-procedures/) to generate code to pivot the data, and then runs the code.
