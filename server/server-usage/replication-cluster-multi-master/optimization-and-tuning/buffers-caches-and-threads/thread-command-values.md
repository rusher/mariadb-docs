...
| --- | --- |
| Value | Description |
| Binlog Dump | Master thread for sending [binary log](../../../programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md) contents to a slave. |
| Change user | Executing a change user operation. |
| Close stmt | Closing a [prepared statement](/en/prepared-statements/). |
| Connect | [Replication](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/mariadb-releases/compatibility-differences/replication-compatibility-between-mariadb-and-mysql) slave is connected to its master. |
...
| Error | Error. |
| Execute | Executing a [prepared statement](/en/prepared-statements/). |
| Fetch | Fetching the results of an executed [prepared statement](/en/prepared-statements/). |
| Field List | Retrieving table column information. |
| Init DB | Selecting default database. |
| Kill | Killing another thread. |
| Long Data | Retrieving long data from the result of executing a [prepared statement](/en/prepared-statements/). |
| Ping | Handling a server ping request. |
| Prepare | Preparing a [prepared statement](/en/prepared-statements/). |
...
| Reset stmt | Resetting a [prepared statement](/en/prepared-statements/). |
