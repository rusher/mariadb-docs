...
Also, an attempt to use an ignored index in a `<code>USE INDEX</code>`, `<code>FORCE INDEX</code>`, or `<code>IGNORE INDEX</code>` hint will result in an error - the same what would have if one used a name of a non-existent index.
