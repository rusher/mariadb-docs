...
([LOOP](loop.md), [REPEAT](repeat-loop.md), [WHILE](while.md)). In [Stored Procedures](../stored-routines/stored-procedures/), [Triggers](../triggers-events/triggers/) and [Events](../triggers-events/event-scheduler/events.md), LEAVE can refer to the outmost BEGIN ... END construct; in that case, the program exits the procedure. In [Stored Functions](../stored-routines/stored-functions/), [RETURN](return.md) can be used instead.
