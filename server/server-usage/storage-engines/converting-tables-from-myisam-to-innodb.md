...
Before [MariaDB 5.5](broken-reference), the DATA DIRECTORY [table option](../sql-statements/data-definition/create/create-table.md#table-options) was not supported for InnoDB. Since [MariaDB 5.5](broken-reference) it is supported, but only in CREATE TABLE. INDEX DIRECTORY has no effect, since InnoDB does not use separate files for indexes. To better balance the workload through several disks, the paths of some InnoDB log files can also be changed.
