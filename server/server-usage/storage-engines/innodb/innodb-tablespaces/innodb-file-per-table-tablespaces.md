...
* Then, use [ALTER TABLE ... DISCARD TABLESPACE](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md#discard-tablespace>) to discard the new table's tablespace:
...
* Then, once the files are in the proper directory on the target server, use [ALTER TABLE ... IMPORT TABLESPACE](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md#import-tablespace>) to import the new table's tablespace:
...
* First, we need to use [ALTER TABLE ... DISCARD TABLESPACE](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md#discard-tablespace>) to discard the placeholder table's tablespace:
...
* Then, once the files are in the proper directory on the target server, we need to use [ALTER TABLE ... IMPORT TABLESPACE](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md#import-tablespace>) to import the new table's tablespace:
...
* Then, it's time to transfer the partition from the placeholder to the target table. This can be done with an [ALTER TABLE... EXCHANGE PARTITION](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md#exchange-partition>) statement:
