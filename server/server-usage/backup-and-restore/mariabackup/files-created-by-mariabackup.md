...
If the backup is a non-prepared [full backup](full-backup-and-restore-with-mariabackup.md) or a non-prepared [partial backup](broken-reference), then `backup_type` is set to `full-backuped`.
