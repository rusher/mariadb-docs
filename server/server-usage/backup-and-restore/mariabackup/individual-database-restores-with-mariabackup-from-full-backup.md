...
Firstly, we will need the table structure from a mariadb-dump backup with the [--no-data](../../../clients-and-utilities/backup-restore-and-import-clients/mariadb-dump.md#options) option. I recommend this is done at least once per day or every six hours via a cronjob. As it is just the structure, it will be very fast.
