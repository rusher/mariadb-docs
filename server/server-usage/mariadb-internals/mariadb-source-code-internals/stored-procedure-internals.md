...
interest to the [stored procedure](../../stored-routines/stored-procedures/) implementation:
...
it's also used at [CALL](../../../reference/sql-statements/stored-routine-statements/call.md) time to find the parameters mode (IN, OUT or INOUT)\
...
* sp\_instr\_stmt\
  ...
  like a [SELECT](../../../reference/sql-statements/data-manipulation/selecting-data/select.md), [INSERT](../../../reference/sql-statements/data-manipulation/inserting-loading-data/insert.md) etc. It contains the Lex structure for the\
...
procedure in the [mysql.proc table](../../../reference/sql-statements/administrative-sql-statements/system-tables/the-mysql-database-tables/mysql-proc-table.md) (or the internal cache).

#### Parsing CREATE PROCEDURE

When parsing a [CREATE PROCEDURE](../../stored-routines/stored-procedures/create-procedure.md) the parser first initializes the`sphead` and `spcont` (runtime context) fields in the Lex.\
...
* SET var:\
  ...
  and the type.
* Flow control:\
  Flow control constructs such as [IF](../../../reference/sql-statements/programmatic-compound-statements/if.md), [WHILE](../../../reference/sql-statements/programmatic-compound-statements/while.md), etc, generate a conditional\
...
[Creating a function](../../../reference/sql-statements/data-definition/create/create-function.md) is essentially the same thing as for a PROCEDURE,\
...
A PROCEDURE is just stored in the [mysql.proc table](../../../reference/sql-statements/administrative-sql-statements/system-tables/the-mysql-database-tables/mysql-proc-table.md). A FUNCTION has an\
...
A [CALL](../../../reference/sql-statements/stored-routine-statements/call.md) is parsed just like any statement. The resulting Lex has the\
...
database (if any). If this was changed during execution (i.e. a [USE](../../../reference/sql-statements/administrative-sql-statements/use-database.md)\
...
The problem is that we already have [User Defined Functions](../../user-defined-functions/) (UDFs) which\
...
* We can't read and parse the FUNCTION from the [mysql.proc table](../../../reference/sql-statements/administrative-sql-statements/system-tables/the-mysql-database-tables/mysql-proc-table.md) at the\
  ...
  of the [mysql.proc table](../../../reference/sql-statements/administrative-sql-statements/system-tables/the-mysql-database-tables/mysql-proc-table.md) during a query execution, but this it not\
...
[DROP PROCEDURE](../../stored-routines/stored-procedures/drop-procedure.md)/[DROP FUNCTION](../../stored-routines/stored-functions/drop-function.md) also supports the non-standard "IF EXISTS", analogous to other [DROP](../../../reference/sql-statements/data-definition/drop/) statements in MariaDB.
...
This is the [mysql.proc table](../../../reference/sql-statements/administrative-sql-statements/system-tables/the-mysql-database-tables/mysql-proc-table.md) used in [MariaDB 10.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/community-server/old-releases/release-notes-mariadb-10-4-series/what-is-mariadb-104):
