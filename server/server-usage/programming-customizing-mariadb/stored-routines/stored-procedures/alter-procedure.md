...
procedure](README.md). More than one change may be specified in an `<code>ALTER PROCEDURE</code>`
...
You must have the `<code>ALTER ROUTINE</code>` privilege for the procedure. By default, that privilege is granted automatically to the procedure creator. See [Stored Routine Privileges](../stored-functions/stored-routine-privileges.md).
