...
Creates a [stored procedure](/kb/en/stored-procedures/). By default, a routine is
...
`MODIFIES SQL DATA` means that the function contains statements that may modify data stored in databases. This happens if the function contains statements like [DELETE](../../../../reference/sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/delete.md), [UPDATE](../../../../reference/sql-statements-and-structure/sql-statements/data-manipulation/changing-deleting-data/update.md), [INSERT](../../views/inserting-and-updating-with-views.md), [REPLACE](../../../../clients-and-utilities/replace-utility.md) or DDL.

`READS SQL DATA` means that the function reads data stored in databases, but does not modify any data. This happens if [SELECT](../../../replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) statements are used, but there no write operations are executed.

`CONTAINS SQL` means that the function contains at least one SQL statement, but it does not read or write any data stored in a database. Examples include [SET](../../../replication-cluster-multi-master/standard-replication/setting-up-replication.md) or [DO](../../../../server-management/getting-installing-and-upgrading-mariadb/downgrading-between-major-versions-of-mariadb.md).
...
be a simple statement such as [SELECT](../../../replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) or [INSERT](../../views/inserting-and-updating-with-views.md), or it can be a
...
DROP. MariaDB also allows [stored procedures](/kb/en/stored-procedures/) (but not [stored functions](/kb/en/stored-functions/))
...
For information about invoking [stored procedures](/kb/en/stored-procedures/) from within programs written in a language that has a MariaDB/MySQL interface, see [CALL](../../../../reference/sql-statements-and-structure/sql-statements/stored-routine-statements/call.md).
