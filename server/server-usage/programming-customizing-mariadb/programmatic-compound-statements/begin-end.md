...
`NOT ATOMIC` is required when used outside of a [stored procedure](/kb/en/stored-procedures/). Inside stored procedures or within an anonymous block, `BEGIN` alone starts a new anonymous block.
...
Use of multiple statements requires that a client is able to send statement strings containing the ; statement delimiter. This is handled in the [mysql](mysql-command-line_client) command-line client with the [DELIMITER](../../../clients-and-utilities/mariadb-client/delimiters.md) command.
...
In this example, a [TINYINT](../../../reference/data-types/data-types-numeric-data-types/tinyint.md) variable, `x` is declared in the outter block. But in the inner block `x` is re-declared as a [CHAR](../../replication-cluster-multi-master/optimization-and-tuning/query-optimizations/charset-narrowing-optimization.md) and an `y` variable is declared. The inner [SELECT](../../replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) shows the "new" value of `x`, and the value of `y`. But when x is selected in the outer block, the "old" value is returned. The final [SELECT](../../replication-cluster-multi-master/standard-replication/selectively-skipping-replication-of-binlog-events.md) doesn't try to read `y`, because it doesn't exist in that context.
