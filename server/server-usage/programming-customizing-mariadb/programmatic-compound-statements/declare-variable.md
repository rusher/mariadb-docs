...
Variables declared with `ROW TYPE OF` will have the same features as implicit [ROW](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/rows_event_v1v2-rows_compressed_event_v1.md) variables. It is not possible to use `ROW TYPE OF` variables in a [LIMIT](../../../security/user-account-management/limiting-size-of-created-disk-temporary-files-and-tables/limiting-size-of-created-disk-temporary-files-and-tables-overview.md) clause.
...
The tables referenced in `TYPE OF` and `ROW TYPE OF` declarations will be checked for existence at the beginning of the stored routine call. [CREATE PROCEDURE](../stored-routines/stored-procedures/create-procedure.md) or [CREATE FUNCTION](../user-defined-functions/create-function-udf.md) will not check the referenced tables for existence.
...
`TYPE OF` and `ROW TYPE OF` from [MariaDB 10.3](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-103):
