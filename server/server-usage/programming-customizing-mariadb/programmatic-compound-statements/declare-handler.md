...
Handlers must be declared after local variables, a `CONDITION` and a [CURSOR](programmatic-compound-statements-cursors/README.md).
...
(or for a `NOT FOUND` condition). An example is shown in [Cursor Overview](programmatic-compound-statements-cursors/cursor-overview.md). This condition also occurs for [SELECT ... INTO](selectinto.md) var_list statements that retrieve no
rows.
