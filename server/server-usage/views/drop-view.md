...
The [DROP privilege](../../reference/sql-statements/account-management-sql-commands/grant.md#table-privileges) is required to use `DROP TABLE` on non-temporary tables. For temporary tables, no privilege is required, because such tables are only visible for the current session.
