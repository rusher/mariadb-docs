...
* Stabilize: Set `new_mode = ""` to keep query planning closer to 10.6 and prevent unpredictable execution plan changes on Day 1. `new_mode` is a `SET` variable and does not accept a literal `OFF` value — setting `OFF` aborts startup (including during Galera/wsrep recovery). Note: `""` is not the 11.8 default (the default is `FIX_DISK_TMPTABLE_COSTS`), so this is a deliberate opt-out.
...
# Set new_mode empty to keep 10.6-like query planning ("" is not the default; the default is FIX_DISK_TMPTABLE_COSTS).
new_mode                      = ""
