...
* On systems where systemd is not enabled/supported: The timeout can be increased by setting MYSQLD\_STARTUP\_TIMEOUT either directly in the script or via the command line. In [MariaDB 10.1.13](https://github.com/mariadb-corporation/docs-server/blob/test/server/server-management/install-and-upgrade-mariadb/installing-mariadb/troubleshooting-installation-issues/installation-issues-on-debian-and-ubuntu/broken-reference/README.md) and later versions, the init script also sources /etc/default/mariadb, so it can also be used to set MYSQLD\_STARTUP\_TIMEOUT to persistently change the startup timeout. The default timeout has been increased from 30s to 60s in [MariaDB 10.1.13](https://github.com/mariadb-corporation/docs-server/blob/test/server/server-management/install-and-upgrade-mariadb/installing-mariadb/troubleshooting-installation-issues/installation-issues-on-debian-and-ubuntu/broken-reference/README.md).
