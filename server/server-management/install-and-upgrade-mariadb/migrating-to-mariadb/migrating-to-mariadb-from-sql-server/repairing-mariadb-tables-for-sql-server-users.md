...
For this reason, `CHECK TABLE` and `REPAIR TABLE` don't work on partitioned tables. Instead, use [ALTER TABLE](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md>) to check or repair a single partition.
...
Indexes can get corrupted. However, as long as data is not corrupted, indexes can always be dropped and rebuilt with [ALTER TABLE](<../../../../reference/sql-statements/data-definition/alter/alter-table/README (1).md>):
