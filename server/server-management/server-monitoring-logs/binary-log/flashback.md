...
The real work of Flashback is done by [mariadb-binlog](../../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md) with `--flashback`. This causes events to be translated: INSERT to DELETE, DELETE to INSERT, and for UPDATEs, the before and after images are swapped.
...
* [mariadb-binlog](../../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md) has the option `--flashback` or `-B` that will let it work in flashback mode.
...
* Invoke [mariadb-binlog](../../../clients-and-utilities/mariadb-binlog/mariadb-binlog-options.md) to find the exact log position of the first offending operation after the state you want to revert to.
