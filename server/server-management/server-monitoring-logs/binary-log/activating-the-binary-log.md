...
To enable binary logging, start the server with the `[--log-bin[=name](/kb/en/replication-and-binary-log-server-system-variables/#log_bin)]` option.
...
The binary log index file contains a master list of all the binary logs, in order. From [MariaDB 11.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-114), if [GTID binlog indexing](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#binlog-indexing) is enabled (the default), an additional index file (`.idx`) is present.
...
* [GTID binlog indexing](../../../clients-and-utilities/server-client-software/client-libraries/clientserver-protocol/replication-protocol/gtid_event.md#binlog-indexing)