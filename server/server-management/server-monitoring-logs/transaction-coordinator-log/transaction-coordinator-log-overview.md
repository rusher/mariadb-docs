...
This transaction coordinator uses the [binary log](../../../server-usage/programming-customizing-mariadb/stored-routines/binary-logging-of-stored-routines.md), which is enabled by the `[log_bin](../../../server-usage/replication-cluster-multi-master/standard-replication/replication-and-binary-log-system-variables.md#log_bin)` server option.

#

## Memory-Mapped File-Based Transaction Coordinator Log

This transaction coordinator uses the memory-mapped file defined by the `[--log-tc](/kb/en/mysqld-options/#-log-tc)` server option. The size is defined by the `[log_tc_size](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#log_tc_size)` system variable.
...
To recover from this error, delete the file defined by the `[--log-tc](/kb/en/mysqld-options/#-log-tc)` server option, and then restart the server with the `[--tc-heuristic-recover](/kb/en/mysqld-options/#-tc-heuristic-recover)` option set.
...
This means that the header of the memory-mapped file-based transaction coordinator log is corrupt. To recover from this error, delete the file defined by the `[--log-tc](/kb/en/mysqld-options/#-log-tc)` server option, and then restart the server with the `[--tc-heuristic-recover](/kb/en/mysqld-options/#-tc-heuristic-recover)` option set.
