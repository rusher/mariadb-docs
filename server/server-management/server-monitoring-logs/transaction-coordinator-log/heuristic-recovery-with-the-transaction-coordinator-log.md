...
* Manual heuristic recovery when [--tc-heuristic-recover](../../starting-and-stopping-mariadb/mariadbd-options.md#tc-heuristic-recover) is set to some value other than `OFF`.

## Automatic Crash Recovery

Automatic crash recovery occurs during startup when MariaDB needs to recover from a crash and [--tc-heuristic-recover](../../starting-and-stopping-mariadb/mariadbd-options.md#tc-heuristic-recover) is set to `OFF`, which is the default value.
