...
The `root@localhost` user account can be altered to use [unix_socket](../../ref/plugins/authentication-plugins/authentication-plugin-unix-socket.md) authentication with the [ALTER USER](../../ref/sql-statements-and-structure/sql-statements/account-management-sql-commands/alter-user.md) statement. For example:
