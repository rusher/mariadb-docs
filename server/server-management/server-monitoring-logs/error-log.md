...
SQL errors can also be logged in a separate file using the [SQL_ERROR_LOG plugin](sql-error-log-plugin.md).
...
On Windows, if the [console](../getting-installing-and-upgrading-mariadb/starting-and-stopping-mariadb/mariadbd-options.md) option is specified, and if the [log_error](../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#log_error) system variable is not used, then errors are written to the console. If both options are specified, then the last option takes precedence.
...
* Messages related to the [event scheduler](../../server-usage/programming-customizing-mariadb/triggers-events/event-scheduler/README.md):
