...
control the server behavior. These can be set up when starting mysqld ([mysqld options](../../starting-and-stopping-mariadb/mariadbd-options.md)), and the vast majority are also accessible as [server system variables](../../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md). These can be classified in these ways:
