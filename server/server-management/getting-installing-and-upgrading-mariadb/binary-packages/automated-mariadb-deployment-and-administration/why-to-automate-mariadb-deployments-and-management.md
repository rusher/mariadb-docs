...
MariaDB includes a powerful [configuration system](../../configuring-mariadb-with-option-files.md). This is enough when we need to deploy a single MariaDB instance, or a small number of instances. But many modern organisations have many database servers. Deploying and upgrading them manually could require too much time, and would be error-prone.
...
Once [replication](../../../../server-usage/replication-cluster-multi-master/standard-replication/README.md) is in place, two important aspects to automate are load balancing and failover.
