...
![vagrant-virtalbox-ansible](/en/vagrant-overview-for-mariadb-users/+image/vagrant-virtalbox-ansible "vagrant-virtalbox-ansible")
...
* [Docker](/en/docker-and-mariadb/) allows one to create Docker containers. On non-Linux systems, Vagrant will create a VM to run Docker.
...
Other providers use automation software to provision the machine. There are provisioners that allow one to use [Ansible](/en/ansible-and-mariadb/), [Puppet](/en/automated-mariadb-deployment-and-administration-puppet-and-mariadb/), Chef or Salt. Where relevant, there are different provisioners allowing the use of these technologies in a distributed way (for example, using Puppet apply) or in a centralized way (for example, using a Puppet server).
