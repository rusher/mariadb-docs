...
![Welcome dialog](/kb/en/installing-mariadb-msi-packages-on-windows/+image/WelcomeDialog_New "Welcome dialog")

#

## License Agreement

![License Agreement](/kb/en/installing-mariadb-msi-packages-on-windows/+image/LicenseAgreementDialog_New "License Agreement")
...
![Custom Setup](/kb/en/installing-mariadb-msi-packages-on-windows/+image/CustomSetupDialog_New "Custom Setup")
...
instances after the installation, with the `[mysql_install_db.exe](/kb/en/mysql_install_dbexe/)` utility.
...
![Database security properties](/kb/en/installing-mariadb-msi-packages-on-windows/+image/DatabaseProperties_1_New "Database security properties")
...
![Other database properties](/kb/en/installing-mariadb-msi-packages-on-windows/+image/DatabaseProperties_2_New "Other database properties")
...
simplifies database management. In [MariaDB 10.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-104) and later, the default service name used by the MSI installer is "MariaDB". In 10.3 and before, the default service name used by the MSI installer is "MySQL". Note that the default service name for the `[--install](/kb/en/mysqld-options/#-install)` and `[--install-manual](/kb/en/mysqld-options/#-install-manual)` options for `mysqld.exe` is "MySQL" in all versions of MariaDB.

* Enable Networking

* Whether to enable TCP/IP (recommended) and which port MariaDB should
listen to. If security is a concern, you can change the [bind-address](../../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/server-system-variables.md#bind_address)
...
![Ready Dialog](/kb/en/installing-mariadb-msi-packages-on-windows/+image/ReadyDialog_New "Ready Dialog")
...
![Finish](/kb/en/installing-mariadb-msi-packages-on-windows/+image/FinishDialog_New "Finish")
...
![Start Menu](/kb/en/installing-mariadb-msi-packages-on-windows/+image/StartMenu_New "Start Menu")
...
![UninstallChangeDialog_New](/kb/en/installing-mariadb-msi-packages-on-windows/+image/UninstallChangeDialog_New "UninstallChangeDialog_New")
...
![KeepOrRemoveDataDialog_New](/kb/en/installing-mariadb-msi-packages-on-windows/+image/KeepOrRemoveDataDialog_New "KeepOrRemoveDataDialog_New")
...
*Feature* is a Windows installer term for a unit of installation. Features
