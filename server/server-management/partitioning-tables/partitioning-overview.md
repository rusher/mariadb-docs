...
It is possible to create a new partitioned table using [CREATE TABLE](../../reference/sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md).
...
#### MariaDB starting with [10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107)

```
CONVERT PARTITION partition_name TO TABLE tbl_name
CONVERT TABLE normal_table TO partition_definition
```
`[ALTER TABLE](../../reference/sql-statements-and-structure/sql-statements/data-definition/alter/alter-table.md) ... CONVERT PARTITION` can, from [MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107), be used to convert partitions in an existing table to a standalone table:
...
From [MariaDB 11.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-114), it is possible to disable this validation by specifying the `WITHOUT VALIDATION` option.
...
An alternative (and the only method prior to [MariaDB 10.7](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-107)) to convert partitions to tables is to use `[ALTER TABLE](../../reference/sql-statements-and-structure/sql-statements/data-definition/alter/alter-table.md) ... EXCHANGE PARTITION`. This requires having to manually do the following steps:
...
<= [MariaDB 11.3](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-113)
...
* Any existing row in t2 must match the conditions for storage in the exchanged partition p1 unless, from [MariaDB 11.4](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-114), the `WITHOUT VALIDATION` option is specified.
...
Some MariaDB [storage engines](https://app.gitbook.com/s/iJPrPCGi329TSR8WIXJW/learning-and-training/video-presentations-and-screencasts/storage-engines-and-plugins-videos) allow more interesting uses for partitioning.

The [MERGE](../../reference/storage-engines/merge.md) storage engine allows one to:

* Treat a set of identical defined [MyISAM](../../clients-and-utilities/myisam-clients-and-utilities/myisamchk-table-information.md) tables as one.
* A MyISAM table can be in many different MERGE sets and also used separately.

[SPIDER](../../server-usage/replication-cluster-multi-master/optimization-and-tuning/system-variables/spider-status-variables.md) allows one to:
...
[CONNECT](../../security/user-account-management/catalogs/connecting-to-a-server-configured-for-catalogs.md) allows one to:
