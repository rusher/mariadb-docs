...
Log filenames and locations may be overridden in the server configuration. The default location of logs is the data directory. The data directory is specified by the [datadir](https://mariadb.com/docs/server/ref/mdb/system-variables/datadir/) system variable.
...
MaxScale can be configured using several methods. These methods make use of MaxScale's [REST API](https://mariadb.com/docs/server/service-management/admin-tools/maxscale/rest-api/).
...
Navigation in the procedure ["Deploy Primary/Replica Topology"](https://mariadb.com/docs/server/deploy/topologies/primary-replica/enterprise-server-11-4/#TOP):
