...
* The `bucket` option must be set to the name of the bucket that you created in "[Create an S3 Bucket](step-1-prepare-systems-for-enterprise-columnstore-nodes.md#create-an-s3-bucket)".
...
1. Create the user account with the [CREATE USER](../../../../reference/sql-statements/account-management-sql-statements/create-user.md) statement:
...
2. Grant the user account `SELECT` privileges on all databases with the [GRANT](../../../../reference/sql-statements/account-management-sql-statements/grant.md) statement:
