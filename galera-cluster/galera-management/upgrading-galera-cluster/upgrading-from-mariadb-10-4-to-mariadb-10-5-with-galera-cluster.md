...
1. Next, make sure that the Galera version numbers are compatible.
  ...
  1. See [What is MariaDB Galera Cluster?: Galera wsrep provider Versions](https://mariadb.com/kb/en/what-is-mariadb-galera-cluster/#galera-wsrep-provider-versions) for information on which MariaDB releases uses which Galera wsrep provider versions.
...
1. Run `[mariadb-upgrade](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/clients-and-utilities/mariadb-upgrade)` with the `--skip-write-binlog` option.
