...
In binary input files, the cpimport tool expects [DATE](data-types-date) columns to be in the following format:
...
In binary input files, the cpimport tool expects [DATETIME](data-types-datetime) columns to be in the following format:
...
The cpimport tool requires [DATE](data-types-date) values to be specified in the format YYYY-MM-DD.
...
The cpimport tool does not write bulk data loads to the binary log, so they cannot be replicated using [MariaDB replication](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/replication-cluster-multi-master/standard-replication/).
