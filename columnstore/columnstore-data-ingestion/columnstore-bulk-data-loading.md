...
1. [cpimport modes "cpimport modes"](#cpimport-modes) 

  1. [Mode 1: Bulk Load from a central location with single data source file "Mode 1: Bulk Load from a central location with single data source file"](#mode-1-bulk-load-from-a-central-location-with-single-data-source-file)
  1. [Mode 2: Bulk load from central location with distributed data source files "Mode 2: Bulk load from central location with distributed data source files"](#mode-2-bulk-load-from-central-location-with-distributed-data-source-files)
  1. [Mode 3: Parallel distributed bulk load "Mode 3: Parallel distributed bulk load"](#mode-3-parallel-distributed-bulk-load)
1. [Bulk loading data from STDIN "Bulk loading data from STDIN"](#bulk-loading-data-from-stdin)
1. [Bulk loading from AWS S3 "Bulk loading from AWS S3"](#bulk-loading-from-aws-s3)
1. [Bulk loading data from S3 bucket directly into SkySQL "Bulk loading data from S3 bucket directly into SkySQL"](#bulk-loading-data-from-s3-bucket-directly-into-skysql)
1. [Bulk loading output of SELECT FROM Table(s) "Bulk loading output of SELECT FROM Table(s)"](#bulk-loading-output-of-select-from-tables)
1. [Bulk loading from JSON "Bulk loading from JSON"](#bulk-loading-from-json)
1. [Bulk loading into multiple tables "Bulk loading into multiple tables"](#bulk-loading-into-multiple-tables) 

  1. [colxml syntax "colxml syntax"](#colxml-syntax)
  1. [Example usage of colxml "Example usage of colxml"](#example-usage-of-colxml)
1. [Handling Differences in Column Order and Values "Handling Differences in Column Order and Values"](#handling-differences-in-column-order-and-values)
1. [Binary Source Import "Binary Source Import"](#binary-source-import) 

  1. [Date Struct "Date Struct"](#date-struct)
  1. [DateTime Struct "DateTime Struct"](#datetime-struct)
1. [Working Folders & Logging "Working Folders & Logging"](#working-folders-logging)
...
1. Run the cpimport utility to perform the data import.
...
![cpimport-mode1](../../.gitbook/assets/columnstore-bulk-data-loading/+image/cpimport-mode1.png "cpimport-mode1")
...
![cpimport-mode2](../../.gitbook/assets/columnstore-bulk-data-loading/+image/cpimport-mode2.png "cpimport-mode2")
...
![cpimport-mode3](../../.gitbook/assets/columnstore-bulk-data-loading/+image/cpimport-mode3.png "cpimport-mode3")
...
Standard in can also be used to directly pipe the output from an arbitrary SELECT statement into cpimport. The select statement may select from non-columnstore tables such as [MyISAM](../../server/reference/storage-engines/myisam-storage-engine/myisam-system-variables.md) or [InnoDB](../../general-resources/learning-and-training/training-and-tutorials/advanced-mariadb-articles/development-articles/quality/innodb-upgrade-tests/README.md). In the example below, the db2.source_table is selected from, using the -N flag to remove non-data formatting. The -q flag tells the mysql client to not cache results which will avoid possible timeouts causing the load to fail.
...
1. Run colxml for the load job for the ‘tpch2’ database as shown here:
