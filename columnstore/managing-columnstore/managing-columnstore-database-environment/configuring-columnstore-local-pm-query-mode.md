...
3. [Enable/disable local PM query at the session level "Enable/disable local PM query at the session level"](configuring-columnstore-local-pm-query-mode.md#enabledisable-local-pm-query-at-the-session-level)
4. [Local PM Query Examples "Local PM Query Examples"](configuring-columnstore-local-pm-query-mode.md#local-pm-query-examples)
5. [Example 1 - SELECT from a single table on local PM to import back on local PM: "Example 1 - SELECT from a single table on local PM to import back on local PM:"](configuring-columnstore-local-pm-query-mode.md#example-1-select-from-a-single-table-on-local-pm-to-import-back-on-local-pm)
6. [Example 2 - SELECT involving a join between a fact table on the PM node and dimension table across all the nodes to import back on local PM: "Example 2 - SELECT involving a join between a fact table on the PM node and dimension table across all the nodes to import back on local PM:"](configuring-columnstore-local-pm-query-mode.md#example-2-select-involving-a-join-between-a-fact-table-on-the-pm-node-and-dimension-table-across-all-the-nodes-to-import-back-on-local-pm)
