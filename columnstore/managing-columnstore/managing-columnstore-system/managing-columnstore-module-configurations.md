...
After the _addDbroot_ command, dbroots 3 and 4 have been created. You can see that they have been created by using the _getStorageConfig_ command. Along with other information, towards the bottom of the output you will see information that reflects the additional, unassigned dbroots.
...
Before a modules can be added, the system must be ACTIVE or OFFLINE. Once added, they can be placed in-service with the _alterSystem-Enable_ command.
...
* To create IDs manually one at a time and have it install on existing Instance\
  addModule module\_ID instance-i\
