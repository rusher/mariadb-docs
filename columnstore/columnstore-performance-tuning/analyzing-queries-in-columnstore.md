...
1. [Determining active queries "Determining active queries"](#determining-active-queries) 

  1. [show processlist "show processlist"](#show-processlist)
  1. [getActiveSQLStatements "getActiveSQLStatements"](#getactivesqlstatements)
1. [Analysis of individual queries "Analysis of individual queries"](#analysis-of-individual-queries) 

  1. [Query statistics "Query statistics"](#query-statistics)
1. [Query plan / trace "Query plan / trace"](#query-plan-trace) 

  1. [Viewing the ColumnStore query plan "Viewing the ColumnStore query plan"](#viewing-the-columnstore-query-plan)
1. [Cache clearing to enable cold testing "Cache clearing to enable cold testing"](#cache-clearing-to-enable-cold-testing)
1. [Query statistics history "Query statistics history"](#query-statistics-history) 

  1. [Query statistics table "Query statistics table"](#query-statistics-table)
  1. [Query statistics viewing "Query statistics viewing"](#query-statistics-viewing)
...
The *calGetStats* function provides statistics about resources used on the [User Module](../columnstore-architecture/columnstore-user-module.md) (UM) node, PM node, and network by the last run query.
Example:
...
* MaxMemPct - Peak memory utilization on the [User Module](../columnstore-architecture/columnstore-user-module.md), likely in support of a large (User Module) based hash join operation.
...
In MariaDB ColumnStore there is a set of SQL tracing stored functions provided to see the distributed query execution plan between the [UM](../columnstore-architecture/columnstore-user-module.md) and the [PM](../columnstore-architecture/columnstore-performance-module.md).
...
1. Execute the SQL statement in question.
1. Review the trace collected for the statement.
...
Cross Engine Support must also be enabled before enabling Query Statistics. See the [Cross Engine Configuration](../managing-columnstore/managing-columnstore-database-environment/configuring-columnstore-cross-engine-joins.md) section.
...
* Memory Utilization (maxMemPct) - This field shows memory utilization for the [User Module](../columnstore-architecture/columnstore-user-module.md) (UM) in support of any UM join, group by, aggregation, distinct, or other operation.
