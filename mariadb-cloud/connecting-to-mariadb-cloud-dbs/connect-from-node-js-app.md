...
| port           | The Read-Write Port or Read-Only Port from the "Connect" window in the MariaDB Cloud portal                                                         |
...
* The `end()` function takes a callback function that defines one implicit argument for the `Error` object if thrown in closing the connection as an argument. If no error is generated in closing a connection, the `Error` object is `null`.
...
| Option         | Description                                                                                                                                         |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| host           | The fully Qualified Domain Name from the "Connect" window in the MariaDB Cloud portal                                                               |
| port           | The Read-Write Port or Read-Only Port from the "Connect" window in the MariaDB Cloud portal                                                         |
| user           | The desired username, which might be the default username in the Service Credentials view                                                           |
| password       | The user's password, which might be the default password in the Service Credentials view if it was not yet customized                               |
| database       | Database name to establish a connection to. No default is configured.                                                                               |
...
* The async function name `main` is arbitrary and does not have a special meaning as in some other programming languages.
