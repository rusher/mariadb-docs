...
[mysql.func system table](/en/mysqlfunc-table/) that records the function's name,
...
For an example, see `sql/udf_example.cc` in the source tree. For a collection of existing UDFs see [https://github.com/pluots/sql-udf](https://github.com/pluots/sql-udf).

Statements making use of user-defined functions are not
[safe for replication](/en/unsafe-statements-for-replication/).

For creating a stored function as opposed to a user-defined function, see
[CREATE FUNCTION](create-function-udf.md).
...
return value, and can be one of [STRING](string), [INTEGER](../../../reference/data-types/data-types-numeric-data-types/integer.md), [REAL](/en/real/) or [DECIMAL](../../../reference/data-types/data-types-numeric-data-types/decimal.md). `DECIMAL` functions currently return string values and should be written like [STRING](/en/string-data-types/) functions.
...
[DROP FUNCTION](drop-function-udf.md) statement, then upgrade the shared library and
...
* [DROP FUNCTION](drop-function-udf.md)
* [CREATE FUNCTION](create-function-udf.md)