...
| MESSAGE_TEXT | VARCHAR(512) | Before [MariaDB 10.3.6](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/release-notes-mariadb-10-3-series/mariadb-1036-release-notes), was VARCHAR(128) |
...
MYSQL_ERRNO is the error code. Each built-in condition has a unique numeric code. 0 indicates success, but it cannot be explicitly set or read via SQL. For a list of built-in error codes, refer to [MariaDB Error Codes](/en/mariadb-error-codes/). The API function to read it is mysql_errno().
