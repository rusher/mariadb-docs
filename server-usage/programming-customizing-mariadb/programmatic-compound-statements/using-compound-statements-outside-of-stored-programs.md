...
Compound statements can also be used outside of [stored programs](/en/stored-routines/).
...
* Only [BEGIN](begin-end.md), [IF](/en/if-statement/), [CASE](case-statement.md), [LOOP](loop.md), [WHILE](while.md), [REPEAT](repeat-loop.md) statements may start a compound statement outside of stored programs.
...
* A compound statement is parsed completely—note "2 warnings" in the above example, even if the condition was false (InnoDB was, indeed, disabled), and the [CREATE TABLE](../../../reference/sql-statements-and-structure/sql-statements/data-definition/create/create-tablespace.md) statement was not executed, it was still parsed and the parser produced "Unknown storage engine" warning.
