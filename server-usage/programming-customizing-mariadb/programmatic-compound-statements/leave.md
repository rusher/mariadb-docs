...
([LOOP](loop.md), [REPEAT](repeat-loop.md), [WHILE](while.md)). In [Stored Procedures](/en/stored-procedures/), [Triggers](../triggers-events/triggers/triggers-and-implicit-locks.md) and [Events](../triggers-events/event-scheduler/events.md), LEAVE can refer to the outmost BEGIN ... END construct; in that case, the program exits the procedure. In [Stored Functions](/en/stored-functions/), [RETURN](return.md) can be used instead.
