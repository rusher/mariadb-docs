...
The `mariabackup` SST method uses the [Mariabackup](/en/backup-restore-and-import-clients-mariadb-backup/) utility for performing SSTs. It is one of the methods that does not block the donor node. [Mariabackup](/en/backup-restore-and-import-clients-mariadb-backup/) was originally forked from [Percona XtraBackup](/en/backup-restore-and-import-clients-percona-xtrabackup/), and similarly, the `mariabackup` SST method was originally forked from the `[xtrabackup-v2](/en/xtrabackup-v2-sst-method/)` SST method.

Note that if you use the `mariabackup` SST method, then you also need to have `[socat](#socat-dependency)` installed on the server. This is needed to stream the backup from the donor node to the joiner node. This is a limitation that was inherited from the `[xtrabackup-v2](/en/xtrabackup-v2-sst-method/)` SST method.

#

# Choosing Mariabackup for SSTs

To use the `mariabackup` SST method, you must set the `[wsrep_sst_method=mariabackup](../galera-cluster-system-variables.md#wsrep_sst_method)` on both the donor and joiner node. It can be changed dynamically with `[SET GLOBAL](../../standard-replication/setting-up-replication.md#global-session)` on the node that you intend to be a SST donor. For example:
...
To use the `mariabackup` SST method, [Mariabackup](/en/backup-restore-and-import-clients-mariadb-backup/) needs to be able to authenticate locally on the donor node, so that it can create a backup to stream to the joiner. You can tell the donor node what username and password to use by setting the `[wsrep_sst_auth](../galera-cluster-system-variables.md#wsrep_sst_auth)` system variable. It can be changed dynamically with `[SET GLOBAL](../../standard-replication/setting-up-replication.md#global-session)` on the node that you intend to be a SST donor. For example:
...
The user account would have to have the same name as the operating system user account that is running the `mysqld` process. On many systems, this is the user account configured as the `[user](/en/mysqld-options/#-user)` option, and it tends to default to `mysql`.
...
By default, on the donor node, it logs to `mariabackup.backup.log`. This log file is located in the `[datadir](../../optimization-and-tuning/system-variables/server-system-variables.md#datadir)`.

By default, on the joiner node, it logs to `mariabackup.prepare.log` and `mariabackup.move.log` These log files are also located in the `[datadir](../../optimization-and-tuning/system-variables/server-system-variables.md#datadir)`.

By default, before a new SST is started, existing `mariabackup` SST log files are compressed and moved to `/tmp/sst_log_archive`. This behavior can be disabled by setting `sst-log-archive=0` in the `[sst]` [option group](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md#option-groups) in an [option file](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md). Similarly, the archive directory can be changed by setting `sst-log-archive-dir`. For example:
...
You can redirect the SST logs to the syslog instead by setting the following in the `[sst]` [option group](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md#option-groups) in an [option file](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md):
...
You can also redirect the SST logs to the syslog by setting the following in the `[mysqld_safe]` [option group](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md#option-groups) in an [option file](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md):
...
If you are performing Mariabackup SSTs with IPv6 addresses, then the `socat` utility needs to be passed the `pf=ip6` option. This can be done by setting the `sockopt` option in the `[sst]` [option group](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md#option-groups) in an [option file](../../../../server-management/getting-installing-and-upgrading-mariadb/configuring-mariadb-with-option-files.md). For example:
