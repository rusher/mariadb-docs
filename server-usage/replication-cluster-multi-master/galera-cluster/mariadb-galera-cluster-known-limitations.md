...
* The [general query log](../../../server-management/server-monitoring-logs/general-query-log.md) and the [slow query log](../optimization-and-tuning/query-optimizations/statistics-for-optimizing-queries/slow-query-log-extended-statistics.md) cannot be directed to a table. If you enable these logs, then you must forward the log to a file by setting `[log_output=FILE](../optimization-and-tuning/system-variables/server-system-variables.md#log_output)`.
...
* Transaction size. While Galera does not explicitly limit the transaction
 ...
 result, extremely large transactions (e.g. [LOAD DATA](../standard-replication/obsolete-replication-information/load-data-from-master-removed.md)) may adversely affect
...
* If you are using [mysqldump](../../../clients-and-utilities/legacy-clients-and-utilities/mysqldumpslow.md) for state transfer, and it failed for whatever
...
* While [binlog_format](/kb/en/replication-and-binary-log-server-system-variables/#binlog_format) is checked on startup and can only be ROW (see [Binary Log Formats](../../../server-management/server-monitoring-logs/binary-log/binary-log-formats.md)), it can be changed at runtime. Do NOT change binlog_format at runtime, it is likely not only cause replication failure, but make all other nodes crash.
...
* Nodes may have different table definitions, especially temporarily during [rolling schema upgrade](galera-cluster-system-variables.md#wsrep_osu_method) operations, but the same [schema compatibility restrictions](/kb/en/replication-when-the-master-and-slave-have-different-table-definitions/) apply as they do for ROW based replication