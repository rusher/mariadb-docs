...
# [LIMIT](../../standard-replication/selectively-skipping-replication-of-binlog-events.md#limit)
...
The [LIMIT](../../standard-replication/selectively-skipping-replication-of-binlog-events.md#limit) clause restricts the number of returned rows.
...
[sql_select_limit](../system-variables/server-system-variables.md#sql_select_limit) acts as an automatic `LIMIT row_count` to any [SELECT](../../standard-replication/selectively-skipping-replication-of-binlog-events.md) query.
...
If the [max_statement_time](../system-variables/server-system-variables.md#max_statement_time) variable is set, any query (excluding stored procedures) taking longer than the value of `max_statement_time` (specified in seconds) to execute will be aborted. This can be set globally, by session, as well as per user and per query. See [Aborting statements that take longer than a certain time to execute](/kb/en/aborting-statements-that-take-longer-than-a-certain-time-to-execute/).
