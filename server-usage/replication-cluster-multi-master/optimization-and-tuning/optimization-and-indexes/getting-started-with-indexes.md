...
Full-text indexes support full-text indexing and searching. See the [Full-Text Indexes](/kb/en/full-text-indexes/) section.
...
Using the [EXPLAIN](../../../../clients-and-utilities/explain-analyzer.md) statement on your queries can help you decide which columns need indexing.
...
If [user statistics](../query-optimizations/statistics-for-optimizing-queries/user-statistics.md) are enabled, the [Information Schema](../../../programming-customizing-mariadb/views/information-schema-views-table.md) [INDEX_STATISTICS](../../../../reference/sql-statements-and-structure/sql-statements/administrative-sql-statements/system-tables/information-schema/information-schema-tables/information-schema-index_statistics-table.md) table stores the index usage.

If the [slow query log](../query-optimizations/statistics-for-optimizing-queries/slow-query-log-extended-statistics.md) is enabled and the `[log_queries_not_using_indexes](../system-variables/server-system-variables.md#log_queries_not_using_indexes)` server system variable is `ON`, the queries which do not use indexes are logged.

*The initial version of this article was copied, with permission, from [http://hashmysql.org/wiki/Proper_Indexing_Strategy](http://hashmysql.org/wiki/Proper_Indexing_Strategy) on 2012-10-30.*
