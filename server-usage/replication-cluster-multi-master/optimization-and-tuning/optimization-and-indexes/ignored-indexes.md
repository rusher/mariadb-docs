...
Ignored indexes were added in [MariaDB 10.6](https://app.gitbook.com/s/aEnK0ZXmUbJzqQrTjFyb/mariadb-community-server/what-is-mariadb-106).
...
1. Check if everything continues to work
1. If not, mark the index as not ignored.
1. If everything continues to work, one can safely drop the index.
...
The optimizer does not make use of an index when it is ignored, while if the index is not ignored (the default), the optimizer will consider it in the optimizer plan, as shown in the [EXPLAIN](../../../../clients-and-utilities/explain-analyzer.md) output.
