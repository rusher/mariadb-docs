---
name: gitbook-canonical
description: Canonical GitBook syntax and configuration reference, vendored from gitbook.com/docs/skill.md. Comprehensive catalog of GitBook custom blocks, frontmatter options, `.gitbook.yaml`, `SUMMARY.md` format spec, variables, and expressions. Use when the user asks about stepper / columns / updates / cards / embeds / files / buttons / icons / expandable blocks, GitBook variables and expressions, `.gitbook.yaml` configuration, or `SUMMARY.md` format — anything beyond the daily-use block set. For daily-use blocks (hints, tabs, code, content-refs, alias links), `gitbook-format` and `dev-docs/gitbook-syntax.md` take precedence.
allowed-tools: Read, Grep, Glob
source: https://gitbook.com/docs/skill.md
last_fetched: 2026-06-12
owners: [shinz]
...
> **mariadb-docs overrides take precedence.** `.claude/skills/gitbook-format/SKILL.md`, `dev-docs/gitbook-syntax.md`, and `dev-docs/link-aliases.md` are authoritative for this repo when they disagree with the canonical reference below. Most importantly: **cross-space links must use `{alias}/path`** (never raw `app.gitbook.com` URLs or relative `.md` paths to other spaces, despite what the "Inline links" section below suggests). See `dev-docs/link-aliases.md` for the alias table and rules.
>
> The content below is **vendored verbatim** from <https://gitbook.com/docs/skill.md>. Refresh from upstream quarterly or whenever GitBook publishes a notable update; update the `last_fetched:` date in the frontmatter when you do.

---

> For the complete documentation index, see [llms.txt](https://gitbook.com/docs/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://gitbook.com/docs/skill.md).
