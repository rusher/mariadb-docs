...
myisamchk is a commandline tool for checking, repairing and optimizing non-partitioned [MyISAM](myisamchk-table-information.md) tables.
...
The following options can be set while passed as commandline options to myisamchk, or set with a [myisamchk] section in your [my.cnf](/en/configuring-mariadb-with-mycnf/) file.
...
| [myisam_sort_buffer_size](../../reference/storage-engines/myisam-storage-engine/myisam-system-variables.md#myisam_sort_buffer_size) | 134216704 |
...
It is also possible to repair [MyISAM](myisamchk-table-information.md) tables by using [REPAIR TABLE](../../reference/sql-statements-and-structure/sql-statements/table-statements/repair-table.md).
...
| -r, --recover | Can fix almost anything except unique keys that aren't unique (a rare occurrence). Usually this is the best option to try first. Increase [myisam_sort_buffer_size](../../reference/storage-engines/myisam-storage-engine/myisam-system-variables.md#myisam_sort_buffer_size) for better performance. |
...
| --character-sets-dir=directory_name | Directory where the [character sets](/en/data-types-character-sets-and-collations/) are installed. |
| --set-collation=name | Change the collation (and by implication, the [character set](/en/data-types-character-sets-and-collations/)) used by the index. |
