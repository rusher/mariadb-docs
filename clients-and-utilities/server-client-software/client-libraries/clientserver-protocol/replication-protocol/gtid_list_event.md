...
Logged in every binlog to record the current [replication](/en/standard-replication/) state. Consists of the last [GTID](gtid_event.md) seen for each replication domain.

The Global Transaction ID, [GTID](gtid_event.md) for short, consists of three components:
...
**Note**: In case of encrypted binlog files ([encrypt_binlog](/en/replication-and-binary-log-server-system-variables/#encrypt_binlog) is set to ON), this event is written just after the [START_ENCRYPTION_EVENT](start_encryption_event.md)
...
From the [mysqlbinlog](/en/mysqlbinlog/) utility:
